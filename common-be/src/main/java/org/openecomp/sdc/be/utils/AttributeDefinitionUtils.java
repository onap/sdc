...
public class AttributeDefinitionUtils {
    ...
    public static Map<String, List<AttributeDataDefinition>> resolveGetOutputAttributes(Map<String, List<AttributeDataDefinition>> attributes) {
        if (attributes == null) {
            return emptyMap();
        }
        return attributes.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, entry -> filterGetOutputAttributes(entry.getValue())));
    }
