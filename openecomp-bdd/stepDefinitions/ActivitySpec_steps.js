...
When('I want to create a ActivitySpec', function () {
...
When('I want to list ActivitySpec with status {string}', function (string) {
...
When('I want to get ActivitySpec', function () {
  let path = '/activity-spec/'+ this.context.item.id+'/versions/'+this.context.item.versionId ;
  return util.request(this.context, 'GET', path, null, false, 'activity_spec').then((result)=> {
  });
});

Then('I want to {string} ActivitySpec', function(string)  {
