...
public class TryTest {
    ...
    private static final Try<Integer> FAILURE = raise(new SomeException());
    ...
    @Test
    public void testMapErrorWithShouldThrowClassCastExceptionWhenNotMappedProperly() {
        try {
            FAILURE
                .mapErrorWith((MappedException e) -> raise(e))
                .rethrow();
            Assert.fail();
        } catch (Exception ex) {
            assertThat(ex, is(instanceOf(MatchException.class)));
        }
    }

    @Test
    public void testSuccessRaiseIfNull() {
        assertThat(SUCCESS.raiseIfNull(mappedException).get(identity(), e -> 0), is(42));
    }

    @Test
    public void testRaiseIfNull() {
        try {
            Try.success((Integer) null)
                .raiseIfNull(mappedException)
                .rethrow();
            Assert.fail();
        } catch (Exception ex) {
            assertThat(ex, is(mappedException));
    ...
    @Test
    public void testFailureRaiseIfFailed() {
        try {
            FAILURE
                .raiseIfFailed(mappedException)
                .rethrow();
            Assert.fail();
        } catch (Exception ex) {
            assertThat(ex, is(mappedException));
    ...
    @Test
    public void testRaiseIfFalsePredicate() {
        assertThat(Try.raiseIf(false, mappedException)
            .get(v -> true, e -> mappedException == e), is(true));
    }

    @Test
    public void testRaiseIfTruePredicate() {
        assertThat(Try.raiseIf(true, mappedException)
            .get(v -> false, e -> mappedException == e), is(true));
    }
    ...
    static private class SomeException extends RuntimeException {
