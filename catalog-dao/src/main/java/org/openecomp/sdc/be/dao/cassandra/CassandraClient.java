...
public class CassandraClient {
    ...
    public CassandraClient() {
        ...
        try {
            cassandraHosts = ConfigurationManager.getConfigurationManager().getConfiguration().getCassandraConfig()
                .getCassandraHosts();
            Integer cassandraPort = ConfigurationManager.getConfigurationManager().getConfiguration()
                .getCassandraConfig()
                .getCassandraPort();
            ...
            logger
                .debug("creating cluster to hosts:{} port:{} with reconnect timeout:{}", cassandraHosts, cassandraPort,
                    reconnectTimeout);
    ...
    private void setSocketOptions(Cluster.Builder clusterBuilder) {
        ...
        Integer socketConnectTimeout = ConfigurationManager.getConfigurationManager().getConfiguration()
            .getCassandraConfig().getSocketConnectTimeout();
        if (socketConnectTimeout != null) {
            logger.info("SocketConnectTimeout was provided, setting Cassandra client to use SocketConnectTimeout: {} .",
                socketConnectTimeout);
            socketOptions.setConnectTimeoutMillis(socketConnectTimeout);
        }
        Integer socketReadTimeout = ConfigurationManager.getConfigurationManager().getConfiguration()
            .getCassandraConfig().getSocketReadTimeout();
        if (socketReadTimeout != null) {
            logger.info("SocketReadTimeout was provided, setting Cassandra client to use SocketReadTimeout: {} .",
                socketReadTimeout);
    ...
    public <T> Try<T> getByIdTry(String id, Class<T> clazz, MappingManager manager) {
        return Try
            .raiseIf(isConnected, CassandraClientException.clusterNotConnected())
            .andThen(v -> mapClazz(id, clazz, manager))
            .raiseIfNull(CassandraClientException.artifactNotFound());
    }

    private <T> Try<T> mapClazz(String id, Class<T> clazz, MappingManager manager) {
        return Try.make(() -> manager.mapper(clazz).get(id))
            .evalIfFailed(e -> logger.debug("Failed to get by Id [{}], error :", id, e))
            .raiseIfFailed(CassandraClientException.unknownError());
    }
