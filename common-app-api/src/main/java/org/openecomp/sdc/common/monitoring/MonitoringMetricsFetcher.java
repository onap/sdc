...
public class MonitoringMetricsFetcher {
    ...
    private static synchronized MonitoringMetricsFetcher init() {
        if (instance == null) {
            ...
            if (appName.contains("fe")) {
                ...
                monitoringLogger.warn("Couldn't determine jvmName, appName is expected to contain \"be\" or \"fe\" string");
            }
        }
        return instance;
    }
    ...
    private long getJvmCpuTime() {
        long cpuTime = -1;
        try {
            cpuTime = (long) platformMBeanServer.getAttribute(new ObjectName(ManagementFactory.OPERATING_SYSTEM_MXBEAN_NAME), PROCESS_CPU_TIME_ATTR);
        } catch (Exception e) {
