...
public class ExportImportMenu {
	...
	enum ExportImportEnum {
		...
		EXPORT("Usage: export <titan.properties> <output directory>", "export"){
			@Override
			void handle(String[] args) {
		...
		},EXPORT_AS_GRAPH("Usage: export-as-graph-ml <titan.properties> <output directory>", "export-as-graph-ml"){
			@Override
			void handle(String[] args) {
				if (verifyParamsLength(args, 3)) {
					usage();
		...
		},EXPORT_USERS("Usage: exportusers <titan.properties> <output directory>", "exportusers"){
			@Override
			void handle(String[] args) {
				if (verifyParamsLength(args, 3)) {
					usage();
		...
		},EXPORT_WITH_REPORT("Usage: export-as-graph-ml-with-data-report <titan.properties> <output directory>", "export-as-graph-ml-with-data-report"){
			@Override
			void handle(String[] args) {
				if (verifyParamsLength(args, 3)) {
					usage();
		...
		},FIND_PROBLEM("Usage: findproblem <titan.properties> <graph file location>", "findproblem"){
			@Override
			void handle(String[] args) {
				if (verifyParamsLength(args, 3)) {
					usage();
		...
		},IMPORT("Usage: import <titan.properties> <graph file location>", "import"){
			@Override
			void handle(String[] args) {
				if (verifyParamsLength(args, 3)) {
					usage();
