...
public class CLIToolTest {
    ...
    private CLIToolImplTest CLIToolImplTest = new CLIToolImplTest();

    @Test
    public void testInit() {
        // when
        final CLIToolData init = CLIToolImplTest.init(new String[]{"-t", ARG});

        // then
        final CommandLine commandLine = init.getCommandLine();
        final Option option = commandLine.iterator().next();

        assertNull(null, commandLine.getOptionValue(OPT));
