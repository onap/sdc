...
public class ActivitySpecManagerImpl implements ActivitySpecManager {
  ...
  private void updateVersionStatus(String activitySpecId, ActivitySpecAction action,
    ...
    switch (version.getStatus()) {
      case Certified:
        String errMsg = String.format("Certify ActivitySpec With Id %s failed since it is not in "
            + "Draft status", activitySpecId);
        validateStatus(Objects.nonNull(retrievedVersion) ? retrievedVersion.getStatus() : null,
            Draft, errMsg);
        prevVersionStatus = Draft;
        break;
      case Deprecated:
        errMsg = String.format("Deprecate ActivitySpec With Id %s failed since it is not in "
            + "Certified status", activitySpecId);
        validateStatus(Objects.nonNull(retrievedVersion) ? retrievedVersion.getStatus() : null,
            Certified, errMsg);
        prevVersionStatus = Certified;
        break;
      case Deleted:
        errMsg = String.format("Delete ActivitySpec With Id %s failed since it is not in Deprecated"
            + " status", activitySpecId);
        validateStatus(Objects.nonNull(retrievedVersion) ? retrievedVersion.getStatus() : null,
            VersionStatus.Deprecated,errMsg);
