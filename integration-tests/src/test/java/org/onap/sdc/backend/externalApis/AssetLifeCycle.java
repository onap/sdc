...
public class AssetLifeCycle extends ComponentBaseTest {
	...
	protected static User defaultUser = new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER);
	...
	public void createResourceCheckInThenCheckOut() throws Exception {
		...
		ResourceExternalReqDetails defaultResource = new ElementFactory().getDefaultResourceByType("ci", ResourceCategoryEnum.TEMPLATE_MONITORING_TEMPLATE, defaultUser.getUserId(), ResourceTypeEnum.VF.toString());
		...
		Component resourceDetails = new AtomicOperationUtils().getResourceObjectByNameAndVersion(UserRoleEnum.DESIGNER, defaultResource.getName(), defaultResource.getVersion());
		...
		ExpectedResourceAuditJavaObject expectedResourceAuditJavaObject = new ElementFactory().getDefaultChangeAssetLifeCycleExternalAPI(resourceDetails, defaultUser, LifeCycleStatesEnum.CHECKIN, AssetTypeEnum.RESOURCES);	
		...
		resourceDetails = new AtomicOperationUtils().getResourceObjectByNameAndVersion(UserRoleEnum.DESIGNER, defaultResource.getName(), String.format("%.1f", Double.parseDouble(defaultResource.getVersion()) + 0.1));
		...
		expectedResourceAuditJavaObject = new ElementFactory().getDefaultChangeAssetLifeCycleExternalAPI(resourceDetails, defaultUser, LifeCycleStatesEnum.CHECKOUT, AssetTypeEnum.RESOURCES);	
	...
	public void createServiceCheckInThenCheckOut() throws Exception {
		...
		Either<Service, RestResponse> createdComponent = new AtomicOperationUtils().createDefaultService(UserRoleEnum.DESIGNER, true);
		...
		resourceDetails = new AtomicOperationUtils().getServiceObjectByNameAndVersion(UserRoleEnum.DESIGNER, resourceDetails.getName(), resourceDetails.getVersion());
		Assert.assertEquals(resourceDetails.getLifecycleState().toString(), LifeCycleStatesEnum.CHECKIN.getComponentState().toString(), "Life cycle state not changed.");
		...
		ExpectedResourceAuditJavaObject expectedResourceAuditJavaObject = new ElementFactory().getDefaultChangeAssetLifeCycleExternalAPI(resourceDetails, defaultUser, LifeCycleStatesEnum.CHECKIN, AssetTypeEnum.SERVICES);	
		AuditValidationUtils.validateAuditExternalChangeAssetLifeCycle(expectedResourceAuditJavaObject, action.getName(), body);*/
	
		restResponse = new LifecycleRestUtils().checkOutService(resourceDetails.getUUID(), defaultUser);
		Assert.assertEquals(restResponse.getErrorCode(), (Integer)STATUS_CODE_CREATED, "Fail to check out.");
		
		resourceDetails = new AtomicOperationUtils().getServiceObjectByNameAndVersion(UserRoleEnum.DESIGNER, resourceDetails.getName(), String.format("%.1f", Double.parseDouble(resourceDetails.getVersion()) + 0.1));
		Assert.assertEquals(resourceDetails.getLifecycleState().toString(), LifeCycleStatesEnum.CHECKOUT.getComponentState().toString(), "Life cycle state not changed.");
		...
		body.put(AuditingFieldsKeysEnum.AUDIT_RESOURCE_NAME, resourceDetails.getName());
		body.put(AuditingFieldsKeysEnum.AUDIT_RESOURCE_CURR_STATE, LifecycleStateEnum.NOT_CERTIFIED_CHECKOUT.toString());
		expectedResourceAuditJavaObject = new ElementFactory().getDefaultChangeAssetLifeCycleExternalAPI(resourceDetails, defaultUser, LifeCycleStatesEnum.CHECKOUT, AssetTypeEnum.SERVICES);	
		expectedResourceAuditJavaObject.setCurrVersion("0.2");
		expectedResourceAuditJavaObject.setPrevState(LifecycleStateEnum.NOT_CERTIFIED_CHECKIN.toString());
		expectedResourceAuditJavaObject.setCurrState(LifecycleStateEnum.NOT_CERTIFIED_CHECKOUT.toString());
		AuditValidationUtils.validateAuditExternalChangeAssetLifeCycle(expectedResourceAuditJavaObject, action.getName(), body);*/
	...
	public static Object[][] dataProviderInvalidUserCheckinForCheckedOutService() {
		return new Object[][] {
			{new ElementFactory().getDefaultUser(UserRoleEnum.TESTER)},
			// TODO: remove comment after talk with renana if it is defect or not
//			{new ElementFactory().getDefaultUser(UserRoleEnum.ADMIN)},
			{new ElementFactory().getDefaultUser(UserRoleEnum.GOVERNOR)},
			{new ElementFactory().getDefaultUser(UserRoleEnum.OPS)},
			/*due to those roles are not exists in the system		{new ElementFactory().getDefaultUser(UserRoleEnum.PRODUCT_STRATEGIST1)},
			{new ElementFactory().getDefaultUser(UserRoleEnum.PRODUCT_MANAGER1)},*/
			};
	}
	...
	public void invalidUserCheckinForCheckedOutService(User defaultUser) throws Exception {
		Component resourceDetails = new AtomicOperationUtils().createDefaultService(UserRoleEnum.DESIGNER, true).left().value();
		RestResponse restResponse = new LifecycleRestUtils().checkInService(resourceDetails.getUUID(), defaultUser);
...
//			{new ElementFactory().getDefaultUser(UserRoleEnum.ADMIN)},
	...
	public void invalidUserCheckinForCheckedInService(User defaultUser) throws Exception {
		...
		Either<Service, RestResponse> createdComponent = new AtomicOperationUtils().createDefaultService(UserRoleEnum.DESIGNER, true);
		resourceDetails = createdComponent.left().value();
		resourceDetails = new AtomicOperationUtils().changeComponentState(resourceDetails, UserRoleEnum.DESIGNER, LifeCycleStatesEnum.CHECKIN, true).getLeft();
		RestResponse restResponse = new LifecycleRestUtils().checkInService(resourceDetails.getUUID(), defaultUser);
		Assert.assertEquals(restResponse.getErrorCode(), RESTRICTED_OPERATION, "Expected for restricted operation.");
...
//			{new ElementFactory().getDefaultUser(UserRoleEnum.ADMIN)},
	...
	public void invalidUserCheckinForCheckedOutResource(User defaultUser) throws Exception {
		...
		Component resourceDetails = new AtomicOperationUtils().createResourcesByTypeNormTypeAndCatregory(ResourceTypeEnum.VF, NormativeTypesEnum.ROOT, ResourceCategoryEnum.GENERIC_INFRASTRUCTURE, UserRoleEnum.DESIGNER, true).left().value();
...
//			{new ElementFactory().getDefaultUser(UserRoleEnum.ADMIN)},
	...
	public void invalidUserCheckinForCheckedInResource(User defaultUser) throws Exception {
		Component resourceDetails = null;
		Either<Resource, RestResponse> createdComponent = new AtomicOperationUtils().createResourcesByTypeNormTypeAndCatregory(ResourceTypeEnum.VF, NormativeTypesEnum.ROOT, ResourceCategoryEnum.GENERIC_INFRASTRUCTURE, UserRoleEnum.DESIGNER, true);
		resourceDetails = createdComponent.left().value();
		resourceDetails = new AtomicOperationUtils().changeComponentState(resourceDetails, UserRoleEnum.DESIGNER, LifeCycleStatesEnum.CHECKIN, true).getLeft();
...
//			{new ElementFactory().getDefaultUser(UserRoleEnum.ADMIN)},
	...
	public static Object[][] dataProviderInvalidStatesForService() {
		return new Object[][] {
				{LifeCycleStatesEnum.CHECKIN, LifeCycleStatesEnum.CHECKIN, COMPONENT_ALREADY_CHECKED_IN, LifecycleStateEnum.NOT_CERTIFIED_CHECKIN, LifecycleStateEnum.NOT_CERTIFIED_CHECKIN, "409", ActionStatus.COMPONENT_ALREADY_CHECKED_IN, new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER), null},
				{LifeCycleStatesEnum.CHECKOUT, LifeCycleStatesEnum.CHECKOUT, COMPONENT_IN_CHECKOUT_STATE, LifecycleStateEnum.NOT_CERTIFIED_CHECKOUT, LifecycleStateEnum.NOT_CERTIFIED_CHECKOUT, "403", ActionStatus.COMPONENT_IN_CHECKOUT_STATE, new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER), null},
				{LifeCycleStatesEnum.CERTIFY, LifeCycleStatesEnum.CHECKIN, COMPONENT_ALREADY_CERTIFIED, LifecycleStateEnum.CERTIFIED, LifecycleStateEnum.CERTIFIED, "403", ActionStatus.COMPONENT_ALREADY_CERTIFIED, new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER), new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER)},
				{LifeCycleStatesEnum.CERTIFY, LifeCycleStatesEnum.CERTIFY, COMPONENT_NOT_READY_FOR_CERTIFICATION, LifecycleStateEnum.CERTIFIED, LifecycleStateEnum.CERTIFIED, "403", ActionStatus.COMPONENT_NOT_READY_FOR_CERTIFICATION, new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER), null},
		};
	}
	...
	public void invalidStatesForService(LifeCycleStatesEnum initState, LifeCycleStatesEnum targetState, int errorCode,
		...
		service = new AtomicOperationUtils().changeComponentState(service, UserRoleEnum.DESIGNER, initState, true).getLeft();
	...
	public static Object[][] dataProviderInvalidStatesForResource() {
		return new Object[][] {
			{LifeCycleStatesEnum.CHECKIN, LifeCycleStatesEnum.CHECKIN, COMPONENT_ALREADY_CHECKED_IN, LifecycleStateEnum.NOT_CERTIFIED_CHECKIN, LifecycleStateEnum.NOT_CERTIFIED_CHECKIN, "409", ActionStatus.COMPONENT_ALREADY_CHECKED_IN, new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER), null},
		/*	{LifeCycleStatesEnum.CHECKIN, LifeCycleStatesEnum.STARTCERTIFICATION, COMPONENT_NOT_READY_FOR_CERTIFICATION, LifecycleStateEnum.NOT_CERTIFIED_CHECKIN, LifecycleStateEnum.NOT_CERTIFIED_CHECKIN, "403", ActionStatus.COMPONENT_NOT_READY_FOR_CERTIFICATION, new ElementFactory().getDefaultUser(UserRoleEnum.TESTER), null},*/
			{LifeCycleStatesEnum.CHECKIN, LifeCycleStatesEnum.CERTIFY, COMPONENT_NOT_READY_FOR_CERTIFICATION, LifecycleStateEnum.NOT_CERTIFIED_CHECKIN, LifecycleStateEnum.NOT_CERTIFIED_CHECKIN, "403", ActionStatus.COMPONENT_NOT_READY_FOR_CERTIFICATION, new ElementFactory().getDefaultUser(UserRoleEnum.TESTER), null},
			
			{LifeCycleStatesEnum.CHECKOUT, LifeCycleStatesEnum.CHECKOUT, COMPONENT_IN_CHECKOUT_STATE, LifecycleStateEnum.NOT_CERTIFIED_CHECKOUT, LifecycleStateEnum.NOT_CERTIFIED_CHECKOUT, "403", ActionStatus.COMPONENT_IN_CHECKOUT_STATE, new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER), null},
		/*	{LifeCycleStatesEnum.CHECKOUT, LifeCycleStatesEnum.STARTCERTIFICATION, COMPONENT_NOT_READY_FOR_CERTIFICATION, LifecycleStateEnum.NOT_CERTIFIED_CHECKOUT, LifecycleStateEnum.NOT_CERTIFIED_CHECKOUT, "403", ActionStatus.COMPONENT_NOT_READY_FOR_CERTIFICATION, new ElementFactory().getDefaultUser(UserRoleEnum.TESTER), null},*/
			{LifeCycleStatesEnum.CHECKOUT, LifeCycleStatesEnum.CERTIFY, COMPONENT_NOT_READY_FOR_CERTIFICATION, LifecycleStateEnum.NOT_CERTIFIED_CHECKOUT, LifecycleStateEnum.NOT_CERTIFIED_CHECKOUT, "403", ActionStatus.COMPONENT_NOT_READY_FOR_CERTIFICATION, new ElementFactory().getDefaultUser(UserRoleEnum.TESTER), null},
			
			/*{LifeCycleStatesEnum.CERTIFICATIONREQUEST, LifeCycleStatesEnum.CERTIFICATIONREQUEST, COMPONENT_SENT_FOR_CERTIFICATION, LifecycleStateEnum.READY_FOR_CERTIFICATION, LifecycleStateEnum.READY_FOR_CERTIFICATION, "403", ActionStatus.COMPONENT_SENT_FOR_CERTIFICATION, new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER), null},
			{LifeCycleStatesEnum.CERTIFICATIONREQUEST, LifeCycleStatesEnum.CERTIFY, RESTRICTED_OPERATION, LifecycleStateEnum.READY_FOR_CERTIFICATION, LifecycleStateEnum.READY_FOR_CERTIFICATION, "409", ActionStatus.RESTRICTED_OPERATION, new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER), null},
			
			{LifeCycleStatesEnum.STARTCERTIFICATION, LifeCycleStatesEnum.CHECKIN, COMPONENT_IN_CERT_IN_PROGRESS_STATE, LifecycleStateEnum.CERTIFICATION_IN_PROGRESS, LifecycleStateEnum.CERTIFICATION_IN_PROGRESS, "403", ActionStatus.COMPONENT_IN_CERT_IN_PROGRESS_STATE, new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER), new ElementFactory().getDefaultUser(UserRoleEnum.TESTER)},
			{LifeCycleStatesEnum.STARTCERTIFICATION, LifeCycleStatesEnum.CHECKOUT, COMPONENT_IN_CERT_IN_PROGRESS_STATE, LifecycleStateEnum.CERTIFICATION_IN_PROGRESS, LifecycleStateEnum.CERTIFICATION_IN_PROGRESS, "403", ActionStatus.COMPONENT_IN_CERT_IN_PROGRESS_STATE, new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER), new ElementFactory().getDefaultUser(UserRoleEnum.TESTER)},
			{LifeCycleStatesEnum.STARTCERTIFICATION, LifeCycleStatesEnum.CERTIFICATIONREQUEST, COMPONENT_IN_CERT_IN_PROGRESS_STATE, LifecycleStateEnum.CERTIFICATION_IN_PROGRESS, LifecycleStateEnum.CERTIFICATION_IN_PROGRESS, "403", ActionStatus.COMPONENT_IN_CERT_IN_PROGRESS_STATE, new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER), new ElementFactory().getDefaultUser(UserRoleEnum.TESTER)},
			{LifeCycleStatesEnum.STARTCERTIFICATION, LifeCycleStatesEnum.STARTCERTIFICATION, COMPONENT_IN_CERT_IN_PROGRESS_STATE, LifecycleStateEnum.CERTIFICATION_IN_PROGRESS, LifecycleStateEnum.CERTIFICATION_IN_PROGRESS, "403", ActionStatus.COMPONENT_IN_CERT_IN_PROGRESS_STATE, new ElementFactory().getDefaultUser(UserRoleEnum.TESTER), null},*/

			{LifeCycleStatesEnum.CERTIFY, LifeCycleStatesEnum.CHECKIN, COMPONENT_ALREADY_CERTIFIED, LifecycleStateEnum.CERTIFIED, LifecycleStateEnum.CERTIFIED, "403", ActionStatus.COMPONENT_ALREADY_CERTIFIED, new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER), new ElementFactory().getDefaultUser(UserRoleEnum.TESTER)},
			/*{LifeCycleStatesEnum.CERTIFY, LifeCycleStatesEnum.CERTIFICATIONREQUEST, COMPONENT_ALREADY_CERTIFIED, LifecycleStateEnum.CERTIFIED, LifecycleStateEnum.CERTIFIED, "403", ActionStatus.COMPONENT_ALREADY_CERTIFIED, new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER), new ElementFactory().getDefaultUser(UserRoleEnum.TESTER)},
			{LifeCycleStatesEnum.CERTIFY, LifeCycleStatesEnum.STARTCERTIFICATION, COMPONENT_ALREADY_CERTIFIED, LifecycleStateEnum.CERTIFIED, LifecycleStateEnum.CERTIFIED, "403", ActionStatus.COMPONENT_ALREADY_CERTIFIED, new ElementFactory().getDefaultUser(UserRoleEnum.TESTER), null},*/
			{LifeCycleStatesEnum.CERTIFY, LifeCycleStatesEnum.CERTIFY, COMPONENT_NOT_READY_FOR_CERTIFICATION, LifecycleStateEnum.CERTIFIED, LifecycleStateEnum.CERTIFIED, "403", ActionStatus.COMPONENT_NOT_READY_FOR_CERTIFICATION, new ElementFactory().getDefaultUser(UserRoleEnum.TESTER), null},
		};
	}
	...
	public void invalidStatesForResource(LifeCycleStatesEnum initState, LifeCycleStatesEnum targetState, int errorCode,
		...
		resourceDetails = new AtomicOperationUtils().changeComponentState(resourceDetails, UserRoleEnum.DESIGNER, initState, true).getLeft();
...
/*	// US824692 - Story [BE]: External API for asset lifecycle - submit for test / start testing
	...
	public void BasicFlowForResourceSubmitForTestingStartTesting() throws Exception {
		Either<Resource, RestResponse> createdComponent = new AtomicOperationUtils().createResourcesByTypeNormTypeAndCatregory(ResourceTypeEnum.VF, NormativeTypesEnum.ROOT, ResourceCategoryEnum.GENERIC_INFRASTRUCTURE, UserRoleEnum.DESIGNER, true);
		Component resourceDetails = createdComponent.left().value();
		RestResponse restResponse = new LifecycleRestUtils().checkInResource(resourceDetails.getUUID(), new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER));
		
		// Certification request
		restResponse = new LifecycleRestUtils().certificationRequestResource(resourceDetails.getUUID(), new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER));
	}*/
	...
	public void BasicFlowForServiceSubmitForTestingStartTesting() throws Exception {
		Either<Service, RestResponse> createdComponent = new AtomicOperationUtils().createDefaultService(UserRoleEnum.DESIGNER, true);
		Component resourceDetails = createdComponent.left().value();
		RestResponse restResponse = new LifecycleRestUtils().checkInService(resourceDetails.getUUID(), new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER));
		
		// Certification request
		restResponse = new LifecycleRestUtils().certificationRequestService(resourceDetails.getUUID(), new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER));
		
		// Start testing
		restResponse = new LifecycleRestUtils().startTestingService(resourceDetails.getUUID(), new ElementFactory().getDefaultUser(UserRoleEnum.TESTER));
	...
	public void specialCaseInvalidFlowForVfcmtSubmitForTesting() throws Exception {
		Either<Resource, RestResponse> createdComponent = new AtomicOperationUtils().createResourcesByTypeNormTypeAndCatregory(ResourceTypeEnum.VFCMT, NormativeTypesEnum.ROOT, ResourceCategoryEnum.GENERIC_INFRASTRUCTURE, UserRoleEnum.DESIGNER, true);
		Component resourceDetails = createdComponent.left().value();
		RestResponse restResponse = new LifecycleRestUtils().checkInResource(resourceDetails.getUUID(), new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER));
	...
	public void specialCaseInvalidFlowForVfcmtStartTesting() throws Exception {
		...
		// Certification request
		restResponse = new LifecycleRestUtils().startTestingResource(resourceDetails.getUUID(), new ElementFactory().getDefaultUser(UserRoleEnum.TESTER));
	...
	public void BasicFlowForResourceCertify() throws Exception {
		...
		// Certify
		restResponse = new LifecycleRestUtils().certifyResource(resourceDetails.getUUID(), new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER));
	}
	...
	public void BasicFlowForServiceCertify() throws Exception {
		...
		// Certify
		restResponse = new LifecycleRestUtils().certifyService(resourceDetails.getUUID(), new ElementFactory().getDefaultUser(UserRoleEnum.TESTER));
	...
	public void theFlow() throws Exception {
		...
		restResponse = new ResourceRestUtils().getResourceListFilterByCriteria(new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER), AssetTypeEnum.RESOURCES.getValue(), SearchCriteriaEnum.RESOURCE_TYPE.getValue(), ResourceTypeEnum.VFCMT.toString());
		...
		Component initComponentVersion = new AtomicOperationUtils().getResourceObjectByNameAndVersion(UserRoleEnum.DESIGNER, parsedCreatedResponse.getName(), parsedCreatedResponse.getVersion());
		ArtifactReqDetails artifactReqDetails = new ElementFactory().getArtifactByType("ci", "OTHER", true, false);
...
//		ArtifactDefinition responseArtifact = ArtifactRestUtils.getArtifactDataFromJson(uploadArtifactRestResponse.getResponse());
		initComponentVersion = new AtomicOperationUtils().getResourceObjectByNameAndVersion(UserRoleEnum.DESIGNER, parsedCreatedResponse.getName(), parsedCreatedResponse.getVersion());
		...
		RestResponse checkInRestResponse = new LifecycleRestUtils().checkInResource(initComponentVersion.getUUID(), new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER));
		...
		RestResponse checkOutRestResponse = new LifecycleRestUtils().checkOutResource(initComponentVersion.getUUID(), new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER));
		...
		Component newComponentVersion = new AtomicOperationUtils().getResourceObjectByNameAndVersion(UserRoleEnum.DESIGNER, parsedCreatedResponse.getName(), String.format("%.1f", Double.parseDouble(parsedCreatedResponse.getVersion())));
		...
		initComponentVersion = new AtomicOperationUtils().getResourceObjectByNameAndVersion(UserRoleEnum.DESIGNER, parsedCreatedResponse.getName(), String.format("%.1f", Double.parseDouble("0.1")));
		...
		checkInRestResponse = new LifecycleRestUtils().checkInResource(initComponentVersion.getUUID(), new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER));
		BaseRestUtils.checkCreateResponse(checkInRestResponse);
		parsedCreatedResponse = gson.fromJson(checkInRestResponse.getResponse(), ResourceAssetStructure.class);
		...
		RestResponse certifyRestResponse = new LifecycleRestUtils().certifyResource(initComponentVersion.getUUID(), new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER));
		...
		checkOutRestResponse = new LifecycleRestUtils().checkOutResource(initComponentVersion.getUUID(), new ElementFactory().getDefaultUser(UserRoleEnum.DESIGNER));
		BaseRestUtils.checkCreateResponse(checkOutRestResponse);
		parsedCreatedResponse = gson.fromJson(checkOutRestResponse.getResponse(), ResourceAssetStructure.class);
