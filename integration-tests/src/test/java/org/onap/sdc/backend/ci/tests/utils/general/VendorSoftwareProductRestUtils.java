...
//    if yes remove this method and use general actionOnComponent method
    ...
    public static RestResponse uploadHeatPackage(String filepath, String filename, VendorSoftwareProductObject vendorSoftwareProductObject, User user) throws Exception {
        Config config = Utils.getConfig();
        String url = String.format(Urls.UPLOAD_HEAT_PACKAGE, config.getOnboardingBeHost(), config.getOnboardingBePort(), vendorSoftwareProductObject.getVspId(), vendorSoftwareProductObject.getVersionId());
    ...
    public static RestResponse uploadFile(String filepath, String filename, String url, User user) throws IOException {
        ...
        // Use the old method for consistency
        ...
        for (Map.Entry<String, String> entry : headersMap.entrySet()) {
            // avoid overwriting multipart content type
            if (!entry.getKey().equalsIgnoreCase(HttpHeaders.CONTENT_TYPE)) {
                headers.add(entry.getKey(), entry.getValue());
            }
        }

        // Multipart body (field name must match @RequestPart("upload"))
