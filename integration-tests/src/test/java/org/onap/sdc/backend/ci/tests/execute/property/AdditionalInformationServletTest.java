...
public class AdditionalInformationServletTest extends ComponentBaseTest {
	...
	// new ResourceRestUtils().deleteResourceByNameAndVersion(user, resourceName,
	// "0.1");
	// new ResourceRestUtils().deleteResourceByNameAndVersion(user, resourceName,
	// "0.2");
	// new ResourceRestUtils().deleteResourceByNameAndVersion(user, resourceName,
	...
	public RestResponse createService() {
		...
		try {
			createServiceResponse = new ServiceRestUtils().createService(serviceDetails, user);
	...
	protected RestResponse deleteResource(String resourceName, String resourceVersion, User sdncModifierDetails) throws IOException {
		RestResponse deleteResourceResponse = new ResourceRestUtils().deleteResourceByNameAndVersion(sdncModifierDetails, resourceName, resourceVersion);
	...
	public void createSamePropertyAfterCiCOResource() {
		...
		try {
			...
			RestResponse afterCreateAI = new ResourceRestUtils().getResource(user, newResourceId);
	...
	private void createAndAddCertResourceToService(ServiceReqDetails serviceDetails, User user) throws Exception {
		...
		User sdncTesterUser = new ElementFactory().getDefaultUser(UserRoleEnum.TESTER);
		...
		RestResponse response = new ResourceRestUtils().createResource(resourceDetails, user);
		...
		ArtifactReqDetails heatArtifactDetails = new ElementFactory().getDefaultDeploymentArtifactForType(ArtifactTypeEnum.HEAT.getType());
