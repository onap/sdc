...
public class PathValidations {
    ...
    public static String[] validateComplexExtendedPath(String[] services) throws Exception {
        ...
        GeneralUIUtils.findElementsByXpath(
                "//*[@data-tests-id='" + DataTestIdEnum.ComplexServiceAmdocs.PATH_MENU_BUTTON.getValue() +
                        "']/parent::*").get(0).click();
        ...
        // delete path
        int paths_before_deletion =
                GeneralUIUtils.findElementsByXpath("//*[text()='" + newPathName + "']/parent::*//span").size();
        GeneralUIUtils.findElementsByXpath("//*[text()='" + newPathName + "']/parent::*//span").get(1).click();
        GeneralUIUtils.ultimateWait();

        int paths_after_deletion =
                GeneralUIUtils.findElementsByXpath("//*[text()='" + newPathName + "']/parent::*//span").size();
        assertNotSame("path expected to be deleted but did not", paths_after_deletion, paths_before_deletion);

        GeneralUIUtils
                .clickOnElementByTestId(DataTestIdEnum.PropertiesAssignmentScreen.INPUT_DELETE_DIALOG_CLOSE.getValue());
    ...
    public static void createPathNumOfRows(int numOfPathRows) throws Exception {
        ...
        PathUtilities.openCreatePath();
        PathUtilities.insertValues("extended" + RandomStringUtils.randomAlphanumeric(numberOfRandomCharacters),
                "pathProtocol1", "pathPortNumbers1");
        PathUtilities.selectFirstLineParam();
    ...
    public static void extendPath(int numOfLines) throws Exception {
        for (int i = 0; i < numOfLines; i++) {
            ...
            List<WebElement> linkSrcs = GeneralUIUtils.findElementsByXpath(
                    "//*[@data-tests-id='" + DataTestIdEnum.ComplexServiceAmdocs.LINK_SOURCE.getValue() + "']//select");
            List<WebElement> linkSrcCPs = GeneralUIUtils.findElementsByXpath(
                    "//*[@data-tests-id='" + DataTestIdEnum.ComplexServiceAmdocs.LINK_SOURCE_CP.getValue() +
                            "']//select");
            List<WebElement> linkTargets = GeneralUIUtils.findElementsByXpath(
                    "//*[@data-tests-id='" + DataTestIdEnum.ComplexServiceAmdocs.LINK_TARGET.getValue() + "']//select");
            List<WebElement> linkTargetCPs = GeneralUIUtils.findElementsByXpath(
                    "//*[@data-tests-id='" + DataTestIdEnum.ComplexServiceAmdocs.LINK_TARGET_CP.getValue() +
                            "']//select");
            for (int j = 0; j < i + 2; j++) {
            ...
            List<WebElement> choices = GeneralUIUtils.findElementsByXpath("//*[" + index + "]/*[@data-tests-id='" +
                    DataTestIdEnum.ComplexServiceAmdocs.LINK_TARGET.getValue() + "']//option");
            choices.get((new Random()).nextInt(choices.size())).click();
            choices = GeneralUIUtils.findElementsByXpath("//*[" + index + "]/*[@data-tests-id='" +
                    DataTestIdEnum.ComplexServiceAmdocs.LINK_TARGET_CP.getValue() + "']//option");
            choices.get((new Random()).nextInt(choices.size())).click();
        }
    }
    ...
    public static void validateExtendedPathDisabledButtons(List<WebElement> linkSrcs, int i, String errMsg)
            throws Exception {
    ...
    public static void ValidateAndDeletePathFromPathList(String pathName) throws Exception {
        // count paths
        List<WebElement> path_list_name = GeneralUIUtils
                .getWebElementsListByTestID(DataTestIdEnum.ComplexServiceAmdocs.PATH_LIST_NAME.getValue());
        // delete paths
        List<WebElement> path_delete = GeneralUIUtils
                .getWebElementsListByTestID(DataTestIdEnum.ComplexServiceAmdocs.PATH_LIST_DELETE.getValue());
        ...
        try {
            path_list_name2 = GeneralUIUtils
                    .getWebElementsListByTestID(DataTestIdEnum.ComplexServiceAmdocs.PATH_LIST_NAME.getValue());
    ...
    public static void checkPathFilter(String pathName, boolean isFound) throws Exception {
        List<WebElement> pathFilterList = GeneralUIUtils.findElementsByXpath(
                "//*[@data-tests-id='" + DataTestIdEnum.ComplexServiceAmdocs.SERVICE_PATH_SELECTOR.getValue() +
                        "']//option");
        GeneralUIUtils.ultimateWait();
        if (isFound) {
    ...
    public static void AssertCreatePath() throws Exception {
        String check =
                GeneralUIUtils.getWebElementByTestID(DataTestIdEnum.ComplexServiceAmdocs.CREATE_BUTTON.getValue())
...
//        GeneralUIUtils.ultimateWait();
    ...
    public static void AssertExtendPath() throws Exception {
        String check =
                GeneralUIUtils.getWebElementsListByTestID(DataTestIdEnum.ComplexServiceAmdocs.EXTEND_BUTTON.getValue())
                        .get(0).getAttribute("class");
        assertEquals("Extend button should be disabled", check, "disabled");
    }

    public static Pair<RestResponse, ServiceReqDetails> validateServicePath(String serviceName, String name)
            throws Exception {
    ...
    public static void ValidateThereIsNoErrorMessage() throws Exception {
        try {
            GeneralUIUtils.findElementsByXpath(
                    "//*[@data-tests-id='" + DataTestIdEnum.ComplexServiceAmdocs.OK.getValue() + "']");
    ...
    public static void validateEditToExistingName(String firstPathName, String secondPathName) throws Exception {
        ...
        try {
            ...
            throw new Exception(
                    "space in beggining or end does not count. when creating another path with duplicate name, expected error did not appear");
        }
        PathUtilities.openPathList();
        PathUtilities.editPathName(secondPathName, "           " + firstPathName);
        try {
