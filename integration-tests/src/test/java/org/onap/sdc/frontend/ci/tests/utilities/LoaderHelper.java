...
public class LoaderHelper {
    ...
    private void waitForLoaderVisibility(final int timeout) {
        getWait(timeout)
            .until(webDriver->ExpectedConditions.visibilityOfElementLocated(locator).apply(webDriver));
    }

    private void waitForLoaderInvisibility(int timeout) {
        getWait(timeout).until(webDriver->ExpectedConditions.invisibilityOfElementLocated(locator).apply(webDriver));
    }
