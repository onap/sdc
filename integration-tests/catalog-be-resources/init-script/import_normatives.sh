...
exec > /home/onap/logs/3script.log 2>&1
...
if [ "$BASIC_AUTH_ENABLED" = "true" ] && [ -n "$BASIC_AUTH_USER" ] && [ -n "$BASIC_AUTH_PASS" ]; then
...
echo "End of sdcinit"
echo "SDC initialization completed. Logs can be found at ${ONAP_LOG}/init.log"
