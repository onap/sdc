...
`settles(page)` derives from `AdditionalConditions.pageLoadWait()` in the Selenium suite: readyState,
`jQuery.active`, and — best-effort — every registered Angular testability reporting stable
(`window.getAllAngularTestabilities`, the hook Protractor's `waitForAngular()` used). Prefer it over
`waitForTimeout`.
