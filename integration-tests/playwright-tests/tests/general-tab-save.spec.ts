...
// ---------------------------------------------------------------------------
// Test 3: Service Role / Function dropdowns are populated (code-review #1 guard)
//
//   Before the fix the template bound `*ngFor="let val of component?.serviceRoleValues"` —
//   a property that exists nowhere — so both dropdowns rendered ZERO options. The fix binds
//   them to getMetadataKeyValidValues('Service Role'|'Service Function'), which (for a category
//   that defines those metadata keys) yields the valid values + an "Others" sentinel.
//
//   This guard creates a Service, opens its General tab, and asserts the Service Role and
//   Service Function selects each expose at least one <option> (incl. "Others"). A Service is
//   used deliberately — the VF canary never renders these service-only fields (§P blind spot).
// ---------------------------------------------------------------------------

test('General tab: Service Role/Function dropdowns are populated (review #1 regression guard)', async ({ page }) => {
    await login(page);

    // Create a Service (service-only fields only render for a Service).
    const created = await page.evaluate(() => {
        const $http = (window as any).angular.element('body').injector().get('$http');
        const name = 'PwSvcRoleFn' + Date.now();
        return $http.post('/sdc1/feProxy/rest/v1/catalog/services', {
            name,
            description: 'created by service role/function guard',
            componentType: 'SERVICE',
            categories: [{ name: 'Network Service', normalizedName: 'network service', uniqueId: 'serviceNewCategory.network service' }],
            contactId: 'cs0008',
            icon: 'defaulticon',
            tags: [name]
        }).then(
    ...
    // A service category without the Service Role/Function metadata keys would fall back to a
    // free-text input (also valid). Only assert the dropdown options when the select is present.
    expect(created.id, `Service creation failed: ${JSON.stringify(created)}`).toBeTruthy();

    await page.evaluate((id: string) => {
        (window as any).angular.element('body').injector().get('$state')
            .go('workspace.general', { id, type: 'service', previousState: 'dashboard' });
    }, created.id);

    expect(await settles(page)).toBe(true);
    await expect(page.locator('[data-tests-id="name"]')).toBeVisible({ timeout: 10_000 });

    // If the category defines the Service Role metadata key the dropdown is rendered; assert it has
    // options (the bug rendered it empty). If instead a free-text serviceRole input is shown, that is
    // the documented fallback and equally acceptable — so we require at least one of the two.
    const roleSelect = page.locator('[data-tests-id="roleOption"]');
    const roleInput = page.locator('input[data-tests-id="serviceRole"]');
    if (await roleSelect.count()) {
        const roleOptionCount = await roleSelect.locator('option').count();
        expect(roleOptionCount, 'Service Role dropdown must not be empty').toBeGreaterThan(0);
        const roleOptionTexts = await roleSelect.locator('option').allTextContents();
        expect(roleOptionTexts).toContain('Others');
        console.log(`✓ Service Role dropdown options: ${JSON.stringify(roleOptionTexts)}`);
    } else {
        await expect(roleInput).toBeVisible({ timeout: 5_000 });
        console.log('✓ Service Role free-text fallback rendered (category defines no Role metadata key)');
    }

    const fnSelect = page.locator('[data-tests-id="functionOption"]');
    const fnInput = page.locator('input[data-tests-id="serviceFunction"]');
    if (await fnSelect.count()) {
        const fnOptionCount = await fnSelect.locator('option').count();
        expect(fnOptionCount, 'Service Function dropdown must not be empty').toBeGreaterThan(0);
        console.log('✓ Service Function dropdown populated');
    } else {
        await expect(fnInput).toBeVisible({ timeout: 5_000 });
        console.log('✓ Service Function free-text fallback rendered');
    }
});
