...
public class ArtifactTypeOperation implements IArtifactTypeOperation {
    ...
    public ArtifactTypeDefinition createArtifactType(final ArtifactTypeDefinition artifactType,
        ...
        try {
            artifactType
                    .setUniqueId(UniqueIdBuilder.buildArtifactTypeUid(artifactType.getModel(), artifactType.getType()));
            final ArtifactTypeData artifactTypeData = new ArtifactTypeData(artifactType);
            Either<ArtifactTypeData, JanusGraphOperationStatus> existArtifact = janusGraphGenericDao
            ...
            if (!existArtifact.isLeft()) {
                ...
                if (createNodeResult.isRight()) {
                    ...
                    log.error(EcompLoggerErrorCode.DATA_ERROR,
                            "Failed to add artifact type {} to graph. Operation status {}", artifactType.getType(),
                            operationStatus);
    ...
    @Override
    public ArtifactTypeDefinition getArtifactType(final String id) {
        Either<ArtifactTypeData, JanusGraphOperationStatus> getResult = janusGraphGenericDao
                .getNode(UniqueIdBuilder.getKeyByNodeType(NodeTypeEnum.ArtifactType), id, ArtifactTypeData.class);
        if (getResult.isRight()) {
            JanusGraphOperationStatus janusGraphStatus = getResult.right().value();
            log.debug("Node with id {} was not found in the graph. status: {}", id, janusGraphStatus);
            StorageOperationStatus storageOperationStatus =
                    DaoStatusConverter.convertJanusGraphStatusToStorageStatus(janusGraphStatus);
            throw new OperationException(ActionStatus.GENERAL_ERROR,
                    String.format("Failed to find artifact on JanusGraph with status %s", storageOperationStatus));
        }
        return convertArtifactDataDefinition(getResult.left().value());
    }
    ...
    public Map<String, ArtifactTypeDefinition> getAllArtifactTypes(final String model) {
        final Either<List<ArtifactTypeData>, JanusGraphOperationStatus> artifactTypes =
                janusGraphGenericDao.getByCriteriaForModel(NodeTypeEnum.ArtifactType, Collections.emptyMap(), model,
                        ArtifactTypeData.class);
        ...
        for (ArtifactTypeDefinition type : artifactTypeList) {
            artifactTypeDefinitionTypes.put(type.getUniqueId(), type);
        }
        return artifactTypeDefinitionTypes;
    }

    private ArtifactTypeDefinition convertArtifactDataDefinition(ArtifactTypeData artifactTypeData) {
        log.debug("The object returned after create interface is {}", artifactTypeData);
        final ArtifactTypeDefinition artifactType =
                new ArtifactTypeDefinition(artifactTypeData.getArtifactTypeDataDefinition());
    ...
    private Either<ArtifactTypeData, JanusGraphOperationStatus> getLatestArtifactTypeByType(String type, String model) {
        final Map<String, Object> mapCriteria = new HashMap<>();
        mapCriteria.put(GraphPropertiesDictionary.TYPE.getProperty(), type);
        Either<List<ArtifactTypeData>, JanusGraphOperationStatus> result = janusGraphGenericDao.getByCriteriaForModel(NodeTypeEnum.ArtifactType,
    ...
    private void addDerivedFromRelation(ArtifactTypeDefinition artifactType) {
        ...
        final ArtifactTypeData derivedFromNode =
                getLatestArtifactTypeByType(derivedFrom, artifactType.getModel()).left().value();
        if (derivedFromOperation.addDerivedFromRelation(artifactId, derivedFromNode.getUniqueId(), NodeTypeEnum.ArtifactType)
                .isRight()) {
            throw new OperationException(ActionStatus.GENERAL_ERROR,
                    String.format("Failed creating derivedFrom relation for artifact type %s",
    ...
    private void addPropertiesToArtifactType(final ArtifactTypeDefinition artifactType) {
        ...
        final Either<Map<String, PropertyData>, JanusGraphOperationStatus> addPropertiesToArtifactType =
                propertyOperation.addPropertiesToElementType(artifactType.getUniqueId(), NodeTypeEnum.ArtifactType,
                        properties);
        if (addPropertiesToArtifactType.isRight()) {
            throw new OperationException(ActionStatus.GENERAL_ERROR,
                    String.format("Failed creating properties for artifact type %s with status %s",
                            artifactType.getType(), DaoStatusConverter.convertJanusGraphStatusToStorageStatus(
                                    addPropertiesToArtifactType.right().value())));
        }
    }
            ...
            if (status.isRight()) {
                throw new OperationException(ActionStatus.GENERAL_ERROR,
                        String.format("Failed creating relation between model %s and artifact type %s with status %s",
                                model, artifactType.getType(),
                                DaoStatusConverter.convertJanusGraphStatusToStorageStatus(status.right().value())));
            }
        }
    }
