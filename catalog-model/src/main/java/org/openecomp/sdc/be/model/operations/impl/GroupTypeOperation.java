...
public class GroupTypeOperation implements IGroupTypeOperation {
    ...
    public Either<GroupTypeDefinition, StorageOperationStatus> addGroupType(GroupTypeDefinition groupTypeDefinition) {
        Either<GroupTypeDefinition, StorageOperationStatus> validationRes = validateUpdateProperties(
            groupTypeDefinition);
        if (validationRes.isRight()) {
            log.error("#addGroupType - One or all properties of group type {} not valid. status is {}",
                groupTypeDefinition, validationRes.right().value());
        ...
        try {
            ...
            if (eitherStatus.isRight()) {
                BeEcompErrorManager.getInstance()
                    .logBeFailedCreateNodeError(CREATE_FLOW_CONTEXT, groupTypeDefinition.getType(),
                        eitherStatus.right().value().name());
                result = Either
                    .right(DaoStatusConverter.convertJanusGraphStatusToStorageStatus(eitherStatus.right().value()));
            } else {
    ...
    public Either<GroupTypeDefinition, StorageOperationStatus> validateUpdateProperties(
        GroupTypeDefinition groupTypeDefinition) {
        JanusGraphOperationStatus error = null;
        if (CollectionUtils.isNotEmpty(groupTypeDefinition.getProperties()) && !Strings
            .isNullOrEmpty(groupTypeDefinition.getDerivedFrom())) {
            ...
            if (allPropertiesRes.isRight() && !allPropertiesRes.right().value().equals(
                ...
                log.debug("Couldn't fetch derived from property nodes for group type {}, error: {}",
                    groupTypeDefinition.getType(), error);
            }
            if (error == null && !allPropertiesRes.left().value().isEmpty()) {
                Either<List<PropertyDefinition>, JanusGraphOperationStatus> validatePropertiesRes = propertyOperation
                    .validatePropertiesUniqueness(allPropertiesRes.left().value(),
    ...
    private Either<Map<String, PropertyDefinition>, JanusGraphOperationStatus> getAllGroupTypePropertiesFromAllDerivedFrom(
        ...
        return janusGraphGenericDao
            ...
            .left()
            .bind(parentGroup -> propertyOperation
                .getAllTypePropertiesFromAllDerivedFrom(parentGroup.getUniqueId(), NodeTypeEnum.GroupType,
                    GroupTypeData.class));
    }
    ...
    private StorageOperationStatus mergeCapabilities(GroupTypeDefinition groupTypeDef) {
        ...
        Either<List<CapabilityDefinition>, StorageOperationStatus> oldCapabilitiesRes = getCapablities(
            groupTypeDef.getUniqueId());
        if (oldCapabilitiesRes.isRight()) {
            ...
            for (Map.Entry<String, CapabilityDefinition> oldEntry : oldCapabilities.entrySet()) {
                ...
                CapabilityDefinition newCapDef =
                    updatedGroupTypeCapabilities != null ? updatedGroupTypeCapabilities.get(key) : null;
                CapabilityDefinition oldCapDef = oldEntry.getValue();

                StorageOperationStatus deleteCapResult = deleteOutdatedCapability(newGroupTypeCapabilities, newCapDef,
                    oldCapDef);
        ...
        JanusGraphOperationStatus createCapResult = createCapabilities(new GroupTypeData(groupTypeDef),
            newGroupTypeCapabilities);
    ...
    private boolean isUpdateAllowed(CapabilityDefinition newCapDef, CapabilityDefinition oldCapDef) {
        if (newCapDef == null) {
            log.error("#upsertCapabilities - Failed due to attempt to delete the capability with id {}",
                oldCapDef.getUniqueId());
            return false;
        }

        if (newCapDef.getType() == null || !newCapDef.getType().equals(oldCapDef.getType())) {
            log.error("#upsertCapabilities - Failed due to attempt to change type of the capability with id {}",
                oldCapDef.getUniqueId());
            return false;
        }
    ...
    private Map<String, CapabilityDefinition> collectNewCapabilities(
        ...
        if (updatedGroupTypeCapabilities == null) {
            return new HashMap<>();
        } else {
            return updatedGroupTypeCapabilities.entrySet().stream()
                .filter(entry -> !oldCapabilities.containsKey(entry.getKey()))
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
        }
    }

    private JanusGraphOperationStatus createCapabilities(GroupTypeData groupTypeData,
        Map<String, CapabilityDefinition> groupCapabilities) {
    ...
    private Either<GraphRelation, JanusGraphOperationStatus> createCapability(GroupTypeData groupTypeData,
        ...
        return eitherCapData
            ...
            .left()
            .bind(capTypeData -> capabilityOperation
                .addCapabilityToGraph(groupTypeData.getUniqueId(), capTypeData, capabilityDef))
            .left()
    ...
    private Either<List<CapabilityDefinition>, StorageOperationStatus> getCapablities(String groupTypeId) {
        Either<List<ImmutablePair<CapabilityData, GraphEdge>>, JanusGraphOperationStatus> groupCapabilitiesOnGraph =
            janusGraphGenericDao.getChildrenNodes(UniqueIdBuilder.getKeyByNodeType(NodeTypeEnum.GroupType), groupTypeId,
                GraphEdgeLabels.GROUP_TYPE_CAPABILITY, NodeTypeEnum.Capability, CapabilityData.class, true);
    ...
    private Either<GraphRelation, JanusGraphOperationStatus> connectToCapability(GroupTypeData groupTypeData,
        CapabilityData capabilityData, String capabilityName) {
        Map<String, Object> properties = new HashMap<>();
        properties.put(GraphEdgePropertiesDictionary.NAME.getProperty(), capabilityName);

        return janusGraphGenericDao
            .createRelation(groupTypeData, capabilityData, GraphEdgeLabels.GROUP_TYPE_CAPABILITY, properties);
    }


    public List<GroupTypeDefinition> getAllGroupTypes(Set<String> excludedGroupTypes) {
        Map<String, Map.Entry<JanusGraphPredicate, Object>> predicateCriteria = buildNotInPredicate(
            GraphPropertiesDictionary.TYPE.getProperty(), excludedGroupTypes);
    ...
    public Either<GroupTypeDefinition, StorageOperationStatus> getGroupTypeByCriteria(String type,
        ...
        try {
            ...
            result = groupTypeEither.left().bind(this::getGroupTypeDefinitionFromList);
    ...
    private Either<GroupTypeDefinition, StorageOperationStatus> getGroupTypeDefinitionFromList(List<GroupTypeData> list) {
        return list.stream()
            .map(GroupTypeData::getGroupTypeDataDefinition)
            .findFirst()
            .map((GroupTypeDataDefinition definition) -> getGroupTypeByUid(definition.getUniqueId()))
            .orElseGet(() -> Either.right(StorageOperationStatus.GENERAL_ERROR));
    }

    private Either<GroupTypeDefinition, StorageOperationStatus> buildGroupTypeDefinition(String uniqueId,
        GroupTypeData groupTypeNode) {
    ...
    private Either<GroupTypeDefinition, StorageOperationStatus> fillCapabilities(String uniqueId,
    ...
    private Either<GroupTypeData, StorageOperationStatus> fillDerivedFrom(String uniqueId,
        GroupTypeDefinition groupType) {
    ...
    private Either<List<PropertyDefinition>, StorageOperationStatus> fillProperties(String uniqueId,
        GroupTypeDefinition groupType, GroupTypeData derivedFromNode) {
        ...
        return propertyOperation.findPropertiesOfNode(NodeTypeEnum.GroupType, uniqueId)
            ...
            .left()
            .bind(
                propsMap -> fillDerivedFromProperties(groupType, derivedFromNode, new ArrayList<>(propsMap.values())));
    }

    Either<Map<String, PropertyDefinition>, StorageOperationStatus> handleGroupTypeHasNoProperties(
    ...
    private Either<List<PropertyDefinition>, StorageOperationStatus> fillDerivedFromProperties(
        ...
        log.debug("#fillDerivedFromProperties - fetching all properties of derived from chain for group type {}",
            groupType.getType());
        return propertyOperation
            .getAllPropertiesRec(derivedFromNode.getUniqueId(), NodeTypeEnum.GroupType, GroupTypeData.class)
            .left()
     ...
     * @return
     */
    private Either<GroupTypeData, JanusGraphOperationStatus> addGroupTypeToGraph(
        GroupTypeDefinition groupTypeDefinition) {

        log.debug("Got group type {}", groupTypeDefinition);

        String ctUniqueId = UniqueIdBuilder
            .buildGroupTypeUid(groupTypeDefinition.getType(), groupTypeDefinition.getVersion(), "grouptype");
        ...
        if (createGTResult.isRight()) {
            ...
            log.error("Failed to add group type {} to graph. status is {}", groupTypeDefinition.getType(),
                operationStatus);
        ...
        if (derivedFrom != null) {
            Either<GraphRelation, JanusGraphOperationStatus> createRelation = connectToDerivedFrom(ctUniqueId,
                derivedFrom);
    ...
    private Either<GraphRelation, JanusGraphOperationStatus> connectToDerivedFrom(String ctUniqueId,
        String derivedFrom) {
        ...
        if (derivedFromGroupTypeResult.isLeft()) {
            ...
            log.debug("After create relation between Group Type with id {} to its parent {}, status is {}.", ctUniqueId,
                derivedFrom, createRelation);
    ...
    private Either<GroupTypeDefinition, StorageOperationStatus> updateGroupTypeOnGraph(
        GroupTypeDefinition updatedGroupType, GroupTypeDefinition currGroupType) {
        ...
        return janusGraphGenericDao.updateNode(new GroupTypeData(updatedGroupType), GroupTypeData.class)
            ...
            .left()
            .bind(
                updatedNode -> updateGroupProperties(updatedGroupType.getUniqueId(), updatedGroupType.getProperties()))
            .left()
            ...
            .left()
            .bind(
                updatedDerivedFrom -> TypeOperations.mapOkStatus(mergeCapabilities(updatedGroupType), updatedGroupType))
            .left()
            .bind(def -> getGroupTypeByUid(def.getUniqueId()));
    }


    private Either<Map<String, PropertyData>, StorageOperationStatus> updateGroupProperties(String groupId,
    ...
    private Either<GraphRelation, StorageOperationStatus> updateGroupDerivedFrom(GroupTypeDefinition updatedGroupType,
        ...
        StorageOperationStatus status = isLegalToReplaceParent(currDerivedFromGroupType,
            updatedGroupType.getDerivedFrom(), updatedGroupType.getType());
        ...
        StorageOperationStatus deleteDerivedRelationStatus = deleteDerivedFromGroupType(groupTypeId,
            currDerivedFromGroupType);
    ...
    private StorageOperationStatus isLegalToReplaceParent(String oldTypeParent, String newTypeParent,
        String childType) {
        return derivedFromOperation
            .isUpdateParentAllowed(oldTypeParent, newTypeParent, childType, NodeTypeEnum.GroupType, GroupTypeData.class,
                t -> t.getGroupTypeDataDefinition().getType());
    }

    private Either<GraphRelation, StorageOperationStatus> addDerivedFromRelation(GroupTypeDataDefinition groupTypeDef,
        String gtUniqueId) {
        String derivedFrom = groupTypeDef.getDerivedFrom();
        if (derivedFrom == null) {
            return Either.left(null);
        }
        log.debug("#addDerivedFromRelationBefore - adding derived from relation between group type {} to its parent {}",
            groupTypeDef.getType(), derivedFrom);
        return this.getLatestGroupTypeByType(derivedFrom, true)
            .left()
            .bind(derivedFromGroup -> derivedFromOperation
                .addDerivedFromRelation(gtUniqueId, derivedFromGroup.getUniqueId(), NodeTypeEnum.GroupType));
    }
    ...
    private StorageOperationStatus deleteDerivedFromGroupType(String groupTypeId, String derivedFromType) {
        ...
        log.debug(
            "#deleteDerivedFromGroupType - deleting derivedFrom relation for group type with id {} and its derived type {}",
            groupTypeId, derivedFromType);
        return getLatestGroupTypeByType(derivedFromType, true)
            .either(derivedFromNode -> derivedFromOperation
                    .removeDerivedFromRelation(groupTypeId, derivedFromNode.getUniqueId(), NodeTypeEnum.GroupType),
                err -> err);
    }

    private void updateGroupTypeData(GroupTypeDefinition updatedTypeDefinition,
        GroupTypeDefinition currTypeDefinition) {
