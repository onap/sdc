...
class SoftwareProductValidationResultsView extends React.Component {
    ...
    refreshValidationResult(thisObj) {
        ...
        var vspTestResults = this.props.softwareProductValidation
            .vspTestResults;
        refreshValidationResults(
            this.props.softwareProductId,
            this.props.version.id
        );
        if (vspTestResults && vspTestResults.length > 0) {
            this.props.softwareProductValidationResult.vspTestResults = vspTestResults;
        }
        delete this.props.softwareProductValidation.vspTestResults;
    }
    getTestResultToDisplay() {
        var vspTestResult = this.props.softwareProductValidation
            ? this.props.softwareProductValidation.vspTestResults
            : null;
        var vspTestRefreshedResult = this.props.softwareProductValidationResult
            ? this.props.softwareProductValidationResult.vspTestResults
            : null;
        return vspTestResult && vspTestResult.length > 0
            ? vspTestResult
            : vspTestRefreshedResult === ''
              ? ''
              : vspTestRefreshedResult && vspTestRefreshedResult.length > 0
                ? vspTestRefreshedResult
                : [];
    }
    render() {
        let results = this.getTestResultToDisplay();
