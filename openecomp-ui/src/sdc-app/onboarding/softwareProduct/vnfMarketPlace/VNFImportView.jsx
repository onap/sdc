...
function VNFAction({action, isHeader, downloadCSAR}) {
	if (isHeader) {
		return <span>{action}</span>;
	}
	return (
		<span>
			<SVGIcon name='download' color='positive' onClick={() => downloadCSAR()}/>
		</span>
	);
}
...
export function VNFItemList({vnf, isHeader, isDes, onSort, activeSortColumn, downloadCSAR, selectTableRow, selectedRow}) {
	...
	return (
		<li className={`vnfBrowse-list-item ${isHeader ? 'header' : ''} ${csarId === selectedRow ? 'selectedRow' : ''}`} 
			...
			<div className='table-cell vnftable-action' data-test-id='vnftable-action'><VNFAction isHeader={isHeader} action={action} downloadCSAR= {downloadCSAR}/></div>		</li>
	);
}
					...
					<GridSection className='vnf-grid-section'>
						<GridItem colSpan='4'>
							<ListEditorView
								...
								<VNFItemList
									isHeader={true}
									vnf={{csarId: 0, name: 'Name', version: 'Verison', provider: 'Vendor', shortDesc: 'Description', action: 'Action'}}
								...
								{this.sortVNFItems(this.filterVNFItems(), 
									this.state.sortDescending, this.state.sortCrit).map(vnf => <VNFItemList key={vnf.id} vnf={vnf} 
										downloadCSAR={this.downloadCSAR} selectTableRow={(selID) => 
											{this.setState({selectedRow: selID});this.selectTableRow(selID);}} selectedRow={this.state.selectedRow}/>)}
							</ListEditorView>
						</GridItem>
						<GridItem colSpan='4'>
							<div className='vnf-modal'>
	            				<Button className='vnf-submit' type='button' btnType='default' onClick={() => 
	            					onSubmit(this.state.selectedRow)}>{i18n('OK')}
	            				</Button>
