...
function VNFAction({action, isHeader, downloadCSAR, id}) {
	if (isHeader) {
		return <span>{action}</span>;
	}
	return (
		<span>
			<SVGIcon name='download' color='positive' onClick={() => {downloadCSAR(id);}}/>
		</span>
	);
}
	...
	return (
		<li className={`vnfBrowse-list-item ${isHeader ? 'header' : ''} ${csarId === selectedRow ? 'selectedRow' : ''}`} 
			...
			<div className='table-cell vnftable-action' data-test-id='vnftable-action'>
				<VNFAction isHeader={isHeader} action={action} downloadCSAR= {downloadCSAR} id={csarId}/>
			</div>
		</li>
	);
}
...
class VNFImportView extends React.Component {
	...
	downloadCSAR(id) {
		VNFImportActionHelper.download(id);
	}
