...
export class HeadersInterceptor implements HttpInterceptor {
    ...
    constructor(private injector: Injector, private cookieService: Cookie2Service, private credentialService: CredentialService, private httpHelperService: HttpHelperService) {}
    ...
    intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        if (this.credentialService.basicAuthEnabled()) {
            authReq = authReq.clone({ headers: authReq.headers.set('Authorization', this.credentialService.getBasicAuth())});
        }
