...
export class WorkflowServiceNg2 {
    ...
    public getWorkflows(filterCertified: boolean = true): Observable<any> {
        return this.http.get(this.baseUrl + '/workflows' + (filterCertified ? '?versionState=CERTIFIED' : ''))
            .map((res:Response) => {
    ...
    public getWorkflowVersions(workflowId: string, filterCertified: boolean = true): Observable<any> {
        return this.http.get(this.baseUrl + '/workflows/' + workflowId + '/versions' + (filterCertified ? '?state=CERTIFIED' : ''))
            .map((res:Response) => {
                return res.json().results;
            });
    }
    ...
    public associateWorkflowArtifact(resourceUuid, operationId, workflowId, workflowVersionId, artifactUuid): Observable<any> {
        return this.http.post(this.baseUrl + '/workflows/' + workflowId + '/versions/' + workflowVersionId + '/associates',
            {
                associationUrl: this.catalogBaseUrl + '/resources/' + resourceUuid + '/interfaces/' + operationId + '/artifacts/' + artifactUuid
            })
            .map((res:Response) => {
                return res.json();
            });
