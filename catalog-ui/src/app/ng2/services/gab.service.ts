...
export interface GabRequest {
  fields: string[];
  parentId: string;
  artifactUniqueId: string;
}
...
export class GabService {
  ...
  public getArtifact(artifactUniqueId: string, resourceId: string, columns: string[]): Observable<Response> {
    ...
    let request: GabRequest = new class implements GabRequest {
      artifactUniqueId: string;
      fields: string[];
      parentId: string;
    };

    request.artifactUniqueId = artifactUniqueId;
    request.fields = columns;
    request.parentId = resourceId;
