...
export class DynamicAttributeComponent {
  ...
  derivedAttributeType = DerivedAttributeType; //http://stackoverflow.com/questions/35835984/how-to-use-a-typescript-enum-value-in-an-angular2-ngswitch-statement
  ...
  ngDoCheck() {
    ...
    if (this.mapKeyInput) {
      ...
      const mapKeyError = (<DerivedFEAttribute>this.attribute).mapKeyError;
  ...
  onClickPropertyRow = (property, event) => {
    // Because DynamicAttributeComponent is recrusive second time the event is fire event.stopPropagation = undefined
  ...
  updateMapKeyValueOnMainParent(childrenProps: Array<DerivedFEAttribute>) {
    if (this.attribute instanceof AttributeFEModel) {
      const property: AttributeFEModel = <AttributeFEModel>this.attribute;
      //Update only if all this property parents has key name
      if (property.getParentNamesArray(childrenProps[0].attributesName, []).indexOf('') === -1) {
        ...
        let parentNames = (<AttributeFEModel>property).getParentNamesArray(childrenProps[0].attributesName, []);
