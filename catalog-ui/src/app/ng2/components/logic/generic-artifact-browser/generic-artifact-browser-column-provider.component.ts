...
export class GenericArtifactBrowserColumnProviderComponent {
  ...
  private columnsContainsName = (name: string): boolean => {
    let columnDefinitions = this.pathsandnames.filter(column => column.friendlyName.toLowerCase() === name.toLowerCase());
    return columnDefinitions.length > 0;
  };

  private columnsContainsPath = (path: string): boolean => {
    let columnDefinitions = this.pathsandnames.filter(column => column.path.toLowerCase() === path.toLowerCase());
    return columnDefinitions.length > 0;
  };
