...
export class TypeWorkspaceGeneralComponent implements OnInit {
  ...
  ngOnInit(): void {
      ...
      if (!this.isViewOnly) {
          ...
          new ServiceDataTypeReader().read(this.importedFile).then(
              (serviceType) => {
                  ...
                  this.dataType.uniqueId = this.dataType.model ? this.dataType.model + "." + this.dataType.name : this.DEFAULT_MODEL_NAME +
    ...
    onModelChange(): void {
        ...
        this.dataType.uniqueId = this.dataType.model.name === "DEFAULT_MODEL_NAME" ? "" : this.dataType.model.name + "." + this.dataType.name;
        this.derivedFromName = "";
        this.derivedFrom.setValue("Select...")
