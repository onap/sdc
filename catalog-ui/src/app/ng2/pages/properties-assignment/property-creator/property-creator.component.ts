...
export class PropertyCreatorComponent {
    ...
    onTypeChange(): void {
        ...
        if(this.propertyModel.type === PROPERTY_TYPES.MAP){
            this.typesSchemaProperties = _.filter(typeList,(dropdownList: DropdownValue) => {
                return dropdownList.label != 'list' && dropdownList.label != 'map';
            });
        }
        if(this.propertyModel.type === PROPERTY_TYPES.LIST){
            this.typesSchemaProperties = _.filter(typeList,(dropdownList: DropdownValue) => {
                return dropdownList.label != 'list';
            });
        }
    }
    ...
    public filterDataTypesByModel = (modelName: string) => {
        ...
        const typesSimpleProperties = _.filter(this.typesProperties,(dropdownList: DropdownValue) => {
            return dropdownList.label != 'list';
        });
