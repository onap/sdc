...
export class ConstraintsComponent implements OnInit {
  ...
  constraintValidators: ValidatorFn;
  ...
  ngOnInit() {
    ...
    this.constraintValidators = Validators.compose([
      Validators.required,
      constraintValidator(this.propertyType)
    ]);

    this.validationMessages = {
      constraint: [
        { type: 'required', message: 'Constraint value is required'},
        { type: 'constraintInvalid', message: 'Constraint value is invalid'}
      ],
      type : [
  ...
  ngOnChanges(changes): void {
    ...
    if (changes.propertyType) {
      ...
      // When property type changes, validators need to be re-composed with new type
      this.constraintValidators = Validators.compose([
        Validators.required,
        constraintValidator(this.propertyType)
      ]);
    ...
    console.log('form', this.constraintForm);
  ...
  onChangeConstraintValue(constraintIndex: number, newValue: any) {
    console.log('onChangeConstraintValue.newValue', newValue);
  ...
  onChangeConstrainValueIndex(constraintIndex: number, newValue: any, valueIndex: number) {
    console.log('value', this.constraintsArray.at(constraintIndex).get('value').value);
    this.constraintValuesArray(constraintIndex).controls[valueIndex].setValue(newValue);
    console.log('updated value', this.constraintsArray.at(constraintIndex).get('value').value);
    this.emitOnConstraintChange();
  }
  ...
  private getConstraintsFormat(): any[] {
    ...
    this.constraintsArray.controls.forEach((control: AbstractControl) => {
      ...
      if (type != ConstraintTypes.valid_values && type != ConstraintTypes.in_range) {
        ...
        console.log('value', control.get('value'));
        const valArray = [];

        control.get('value').value.forEach((val) => {
          console.log('value to push', val);
          valArray.push(val);
        });
  ...
  private validateConstraints(): void {
    this.valid = this.constraintsArray.controls.every((control: AbstractControl) => {
      ...
      if (type == ConstraintTypes.pattern) {
        ...
        this.validateNonStringVal(control);
      }
  ...
  private validateNonStringVal(control: AbstractControl) {
    if (!this.valid) {
      return;
    }
    switch (this.propertyType) {
      case PROPERTY_TYPES.INTEGER:
        try {
          ...
          const parsed = parseInt(control.get('value').value);
          console.log('Parsed', parsed);
          control.get('value').setValue(parsed);
        } catch (e) {
      ...
      case PROPERTY_TYPES.FLOAT:
        try {
          // tslint:disable-next-line:radix
          const parsed = parseFloat(control.get('value').value);
          console.log('Parsed', parsed);
          control.get('value').setValue(parsed);
        } catch (e) {
  ...
  private emitOnConstraintChange(): void {
    console.log('constraintForm', this.constraintForm);
    console.log('constraints', this.constraintsArray);
...
export function constraintValidator(propertyType): ValidatorFn {
  return (control: AbstractControl): ValidationErrors | null => {
    console.group('constraintValidator');
    ...
    if (!value || (value && value.empty)) {
      console.group('value failed');
      console.warn('Could not retrieve value', value);
      console.groupEnd();
      return null;
    }
    ...
    switch (propertyType) {
      case PROPERTY_TYPES.INTEGER:
        // Check for full stop which gets ignored by parse check
        if (value.includes('.')) {
          console.groupEnd();
          return {constraintInvalid: true};
        }

        try {
          // Check that string is a valid int
          // tslint:disable-next-line:radix
      ...
      default:
        console.warn('No validation for property type ', propertyType);
    }
  };
}
