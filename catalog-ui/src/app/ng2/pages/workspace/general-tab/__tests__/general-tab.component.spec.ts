...
describe('GeneralTabComponent - Task 8c categories', () => {
    ...
    it('Resource categories load from cache into comp.categories (non-empty)', () => {
        ...
        // Categories are sorted by name (RC-C fix, mirrors the old |orderBy:'name'); given
        // RESOURCE_CATEGORIES = [Common Network Resources, Application Layer 4+], "Application
        // Layer 4+" sorts first. (Both must still be present — see names assertion below.)
        const names = (comp as any).categories.map((c: any) => c.name);
        expect(names).toContain('Common Network Resources');
        expect(names).toContain('Application Layer 4+');
        expect((comp as any).categories[0].name).toBe('Application Layer 4+');
    });
...
// ─── C5: model '' -> null coercion (fixes BE "Unsupported datatype found for property") ──────
// The empty <option value=""> (no-model / default SDC AID choice) yields '' from the reactive
// control. The BE keys its datatype cache by model name; getAll("") resolves an EMPTY datatype
// scope, so a component persisted with model="" later fails property validation on even basic
// string/boolean types. The component must always send null, never '', for "no model".
describe('GeneralTabComponent - C5 model empty-string coercion', () => {

    // Regression guard for CI ui-verify (CR 145593): VF created through the General tab carried
    // model="" -> addComponentProperty(string/boolean) 400s with "Unsupported datatype found".
    it('coerces an empty model form value to null on the component (not "") on form change', () => {
        const resourceComp = makeComponent({model: undefined, tags: [], name: 'MyVF'});
        const {comp} = createComp({component: resourceComp, stateParams: {}}); // CREATE mode
        comp.ngOnInit();
        // The model control defaults to '' (the blank option). Trigger a sync via another edit.
        comp.form.get('description').setValue('trigger sync');
        expect(comp.component.model).toBeNull();
        expect(comp.component.model).not.toBe('');
    });

    it('keeps a real model value when one is selected', () => {
        const serviceComp = makeComponent({
            componentType: 'SERVICE', isService: () => true, isResource: () => false,
            getComponentSubType: () => 'SERVICE', model: 'SDC AID', tags: [], name: 'MySvc'
        });
        const {comp} = createComp({component: serviceComp});
        comp.ngOnInit();
        comp.form.get('description').setValue('trigger sync');
        expect(comp.component.model).toBe('SDC AID');
    });

    it('onModelChange coerces a cleared model selection to null', () => {
        const serviceComp = makeComponent({
            componentType: 'SERVICE', isService: () => true, isResource: () => false,
            getComponentSubType: () => 'SERVICE', model: 'SDC AID', tags: [], name: 'MySvc',
            categories: [{name: 'Network Service'}]
        });
        const {comp} = createComp({component: serviceComp});
        comp.ngOnInit();
        comp.form.get('model').setValue('');
        comp.onModelChange();
        expect(comp.component.model).toBeNull();
    });
});

// ─── C6: init-default fields must reach the form control + survive a sync ──────────────────
// initEnvironmentContext / initInstantiationTypes / single-VSP-model set a default on the
// component, but the old ng-model auto-reflected it into the <select>; with formControlName the
// control kept its '' default and the NEXT syncFormToComponent overwrote the component default
// back to ''. For a Service create that produced an empty Environment Context -> backend
// "Invalid Environment context" (CI: EtsiModelUiTests.createServiceWithModel).
describe('GeneralTabComponent - C6 init defaults reflected into the form', () => {
    function makeServiceForInit(overrides: any = {}) {
        return makeComponent(Object.assign({
            componentType: 'SERVICE', isService: () => true, isResource: () => false,
            getComponentSubType: () => 'SERVICE',
    ...
    it('environmentContext default reaches the form control and survives a later form change (regression for "Invalid Environment context")', () => {
        ...
        cacheService.get = jest.fn((k: string) => {
            if (k === 'user') { return {userId: 'cs0008', role: 'DESIGNER'}; }
            if (k === 'UIConfiguration') {
                return {environmentContext: {defaultValue: 'General_Revenue-Bearing',
                    validValues: ['General_Revenue-Bearing', 'General_Non-Revenue-Bearing']}};
            }
            if (k === 'serviceCategories') { return [{name: 'Network Service', subcategories: []}]; }
            return null;
        });
        comp.ngOnInit();
        // The default must be on BOTH the component and the form control
        expect(comp.component.environmentContext).toBe('General_Revenue-Bearing');
        expect(comp.form.get('environmentContext').value).toBe('General_Revenue-Bearing');
        // A subsequent edit (which triggers syncFormToComponent) must NOT wipe it back to ''
        comp.form.get('description').setValue('edited');
        expect(comp.component.environmentContext).toBe('General_Revenue-Bearing');
    });
    ...
    it('instantiationType default reaches the form control and survives a later form change', () => {
        ...
        comp.ngOnInit();
        expect(comp.component.instantiationType).toBe('A-la-carte');
        expect(comp.form.get('instantiationType').value).toBe('A-la-carte');
        comp.form.get('description').setValue('edited');
        expect(comp.component.instantiationType).toBe('A-la-carte');
    });
...
// ─── C7: vendorName/vendorRelease required ONLY for Resources ───────────────────────────────
// The old template wrapped these in ng-if="component.isResource()", so a Service never showed
// them and they never affected validity. The reactive form required them unconditionally, so a
// Service's form was permanently invalid -> registerLifecycleSaveHandler aborts on !form.valid
// -> the certify modal never opens (CI: EtsiModelUiTests.createServiceWithModel timed out at the
// checkindialog textarea on certify). NOTE: makeComponent() pre-fills vendorName/vendorRelease,
// which is precisely why the existing suite never caught this — the Service tests below leave
// them EMPTY to reproduce the real Service shape.
describe('GeneralTabComponent - C7 vendor fields required for resources only', () => {
    function makeServiceNoVendor(overrides: any = {}) {
        return makeComponent(Object.assign({
            componentType: 'SERVICE', isService: () => true, isResource: () => false,
            getComponentSubType: () => 'SERVICE',
            vendorName: '', vendorRelease: '', resourceVendorModelNumber: '',
            tags: [], name: 'CI-Service', selectedCategory: 'Network Service'
        }, overrides));
    }

    it('Service with empty vendorName/vendorRelease has a VALID form (regression guard for certify block)', () => {
        const {comp, workspaceService} = createComp({component: makeServiceNoVendor()});
        comp.ngOnInit();
        expect(comp.component.isService()).toBe(true); // sanity: component is a Service
        expect(comp.form.get('vendorName').value).toBe('');
        expect(comp.form.get('vendorRelease').value).toBe('');
        expect(comp.form.valid).toBe(true);
        expect(workspaceService.isValidForm).toBe(true);
    });

    it('Service certify reaches handleChangeLifecycleState (does NOT abort on !form.valid)', async () => {
        const handleChangeLifecycleState = jest.fn();
        const startProgress = jest.fn();
        const stopProgress = jest.fn();
        ...
        expect(call).toBeDefined();
        await call[1]('certify');
        // The certify must proceed: save() then delegate to the shell (which opens the modal).
        expect(svc.updateComponent).toHaveBeenCalled();
        expect(handleChangeLifecycleState).toHaveBeenCalledWith('certify');
    });

    it('Resource with empty vendorName/vendorRelease stays INVALID (vendor remains required for VFs)', () => {
        // default makeComponent is a Resource; null out the vendor fields it pre-fills
        const res = makeComponent({vendorName: '', vendorRelease: '', tags: [], name: 'MyVF'});
        const {comp} = createComp({component: res});
        comp.ngOnInit();
        expect(comp.form.get('vendorName').errors && comp.form.get('vendorName').errors.required).toBe(true);
        expect(comp.form.valid).toBe(false);
    });
});

// ─── C8: resource categories are sorted by name (RC-C) ───────────────────────
// Regression for ImportVfcUiTest.checkEtsiMetadata: exported main category was "Generic"
// instead of "Application L4+". Root cause: the old template sorted category optgroups with
// |orderBy:'name'; the migrated template renders them in raw array order. Several main
// categories share a "Database" subcategory, and Selenium's selectByVisibleText("Database")
// picks the FIRST matching <option> — so without the sort the wrong main category is chosen.
// Sorting by name puts "Application L4+" first among {Application L4+, DCAE Component, Generic}.
describe('GeneralTabComponent - C8 categories sorted by name', () => {
    // Three main categories that all define a "Database" subcategory, supplied in NON-sorted order.
    const RESOURCE_CATS_UNSORTED = [
        {name: 'DCAE Component', subcategories: [{name: 'Database'}]},
        {name: 'Generic', subcategories: [{name: 'Database'}]},
        {name: 'Application L4+', subcategories: [{name: 'Database'}]}
    ];

    it('sorts resource main categories alphabetically so the first "Database" is under Application L4+', () => {
        const resourceComp = makeComponent({model: undefined, selectedCategory: undefined, tags: []});
        const {comp, cacheService} = createComp({component: resourceComp, stateParams: {}});
        cacheService.get = jest.fn((k: string) => {
            if (k === 'user') { return {userId: 'cs0008', role: 'DESIGNER'}; }
            if (k === 'resourceCategories') { return JSON.parse(JSON.stringify(RESOURCE_CATS_UNSORTED)); }
            return null;
        });
        comp.ngOnInit();
        const names = (comp as any).categories.map((c: any) => c.name);
        expect(names).toEqual(['Application L4+', 'DCAE Component', 'Generic']);
        // The first category exposing a "Database" subcategory must be Application L4+
        // (this is exactly what selectByVisibleText("Database") resolves to).
        const firstWithDatabase = (comp as any).categories
            .find((c: any) => (c.subcategories || []).some((s: any) => s.name === 'Database'));
        expect(firstWithDatabase.name).toBe('Application L4+');
    });

    it('does not mutate the shared CacheService categories array (sorts a copy)', () => {
        const cached = JSON.parse(JSON.stringify(RESOURCE_CATS_UNSORTED));
        const resourceComp = makeComponent({model: undefined, selectedCategory: undefined, tags: []});
        const {comp, cacheService} = createComp({component: resourceComp, stateParams: {}});
        cacheService.get = jest.fn((k: string) => {
            if (k === 'user') { return {userId: 'cs0008', role: 'DESIGNER'}; }
            if (k === 'resourceCategories') { return cached; }
            return null;
        });
        comp.ngOnInit();
        // The original cached array order must be untouched.
        expect(cached.map((c: any) => c.name)).toEqual(['DCAE Component', 'Generic', 'Application L4+']);
    });
});

// ─── C9: special name tag chip (RC-B) ────────────────────────────────────────
// Regression for (1) onapOnboardVNFflow / onboardCNFTest timing out on i-sdc-tag-text — an
// onboarded VF with only the name tag must still expose exactly one tag (its name); and
// (2) Service.updateService — after a rename the OLD name must not leak as an editable chip.
describe('GeneralTabComponent - C9 special name tag', () => {
    it('strips the component name from the editable tag list on load (mirrors old _.without)', () => {
        // tags arrive from the BE containing the name (handleTags persisted it)
        const res = makeComponent({name: 'MyVF', tags: ['MyVF', 'userTag']});
        const {comp} = createComp({component: res});
        comp.ngOnInit();
        // The name must be removed from component.tags; only the user tag remains as editable.
        expect(comp.component.tags).toEqual(['userTag']);
        expect(comp.displayedTags).toEqual(['userTag']);
    });

    it('displayedTags is empty for a component whose only tag is its name (onboarded VF)', () => {
        const res = makeComponent({name: 'EtsiVnf', tags: ['EtsiVnf']});
        const {comp} = createComp({component: res});
        comp.ngOnInit();
        // No editable chips — but the template still renders the name as the special chip,
        // so the Selenium tag reader sees exactly one i-sdc-tag-text (the name).
        expect(comp.displayedTags).toEqual([]);
        expect(comp.component.tags).toEqual([]);
    });

    it('after a rename the OLD name does not reappear as an editable chip', () => {
        const res = makeComponent({name: 'OldName', tags: ['OldName', 'keepTag']});
        const {comp} = createComp({component: res});
        comp.ngOnInit();
        // user renames the component
        comp.component.name = 'NewName';
        // displayedTags must show only the real user tag — never the stale old name.
        expect(comp.displayedTags).toEqual(['keepTag']);
        expect(comp.displayedTags).not.toContain('OldName');
    });
});

// ─── C10: category-specific metadata subsystem (RC-A) ────────────────────────
// Regression for EtsiNetworkServiceUiTests.createService timing out waiting for the
// <select data-tests-id="ETSI Version">. Root cause: onCategoryChange never populated
// categorySpecificMetadata from the category's metadataKeys, so the field never rendered.
describe('GeneralTabComponent - C10 category-specific metadata', () => {
    // A service category exposing an "ETSI Version" metadata key with valid values.
    const ETSI_SERVICE_CATEGORIES = [
        {
            name: 'ETSI NFV Network Service',
            metadataKeys: [
                {name: 'ETSI Version', displayName: 'ETSI Version', mandatory: true, validValues: ['2.5.1', '2.7.1'], defaultValue: '2.5.1'}
            ],
            subcategories: []
        }
    ];

    function makeEtsiService() {
        return makeComponent({
            componentType: 'SERVICE', isService: () => true, isResource: () => false,
            getComponentSubType: () => 'SERVICE', name: 'EtsiNs', tags: [],
            selectedCategory: undefined, categories: undefined, categorySpecificMetadata: {}
        });
    }

    function etsiCache(cacheService: any) {
        cacheService.get = jest.fn((k: string) => {
            if (k === 'user') { return {userId: 'cs0008', role: 'DESIGNER'}; }
            if (k === 'serviceCategories') { return JSON.parse(JSON.stringify(ETSI_SERVICE_CATEGORIES)); }
            return null;
        });
    }

    it('onCategoryChange seeds categorySpecificMetadata from the category metadataKeys (ETSI Version appears)', () => {
        const {comp, cacheService} = createComp({component: makeEtsiService(), stateParams: {}});
        etsiCache(cacheService);
        comp.ngOnInit();
        comp.form.get('category').setValue('ETSI NFV Network Service');
        comp.onCategoryChange();
        // The ETSI Version key must now exist in categorySpecificMetadata (seeded with its default).
        expect(comp.component.categorySpecificMetadata['ETSI Version']).toBe('2.5.1');
        // And it must surface in the entries the template renders.
        const keys = comp.categorySpecificMetadataEntries.map((e: any) => e.key);
        expect(keys).toContain('ETSI Version');
    });

    it('exposes the metadata-key helpers the template needs for a <select>', () => {
        const {comp, cacheService} = createComp({component: makeEtsiService(), stateParams: {}});
        etsiCache(cacheService);
        comp.ngOnInit();
        comp.form.get('category').setValue('ETSI NFV Network Service');
        comp.onCategoryChange();
        expect(comp.isMetadataKeyForComponentCategory('ETSI Version')).toBe(true);
        expect(comp.getMetadataKeyValidValues('ETSI Version')).toEqual(['2.5.1', '2.7.1']);
        expect(comp.isMetadataKeyMandatory('ETSI Version')).toBe(true);
        expect(comp.getMetadataDisplayName('ETSI Version')).toBe('ETSI Version');
        // ETSI Version is NOT one of the dedicated service-metadata keys → rendered generically.
        expect(comp.isCategoryServiceMetadataKey('ETSI Version')).toBe(false);
        // The dedicated keys must be recognised so they are NOT double-rendered.
        expect(comp.isCategoryServiceMetadataKey('Service Role')).toBe(true);
    });

    it('does not overwrite an already-loaded metadata value on re-selection', () => {
        const svc = makeEtsiService();
        svc.categorySpecificMetadata = {'ETSI Version': '2.7.1'};
        const {comp, cacheService} = createComp({component: svc, stateParams: {}});
        etsiCache(cacheService);
        comp.ngOnInit();
        comp.form.get('category').setValue('ETSI NFV Network Service');
        comp.onCategoryChange();
