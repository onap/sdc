...
export class WorkspaceContainerComponent implements OnInit, OnDestroy {
    ...
    getLifecycleButtonEntries(): Array<{key: string, value: any}> {
        if (!this.changeLifecycleStateButtons) { return []; }
        return Object.keys(this.changeLifecycleStateButtons).map(key => ({
            key: key,
            value: this.changeLifecycleStateButtons[key]
        }));
    }
