...
export class OperationCreatorComponent {
    ...
    operationTypeEnum: Array<String> = ['Create', 'Delete', 'Instantiate', 'Start', 'Stop'];
    propertyTooltipText: String = "VNF properties are defined by the input parameter type. In case you can't find a certain parameter, it might be due to a wrong type selection.";

    constructor(private workflowServiceNg2: WorkflowServiceNg2) {}
    ...
    onSelectWorkflow(selectedVersionId?: string): Subscription {
        ...
        return this.workflowServiceNg2.getWorkflowVersions(this.operation.workflowId).subscribe((versions: Array<any>) => {
            ...
            this.workflowVersions = _.map(
                _.filter(versions, version => version.state === 'CERTIFIED'),
                (version: any) => {
