...
export class AttributesOutputsComponent {
  ...
  ngOnInit() {
    ...
    .subscribe(response => {
      _.forEach(response.outputs, (output: OutputBEModel) => {
        ...
        this.outputs.push(newOutput); //only push items that were declared via SDC
      });
  ...
  processInstanceAttributesResponse = (instanceBeAttributesMap: InstanceBeAttributesMap, originTypeIsVF: boolean) => {
    this.instanceFeAttributesMap = this.attributesUtils.convertAttributesMapToFEAndCreateChildren(instanceBeAttributesMap, originTypeIsVF, this.outputs); //create flattened children, disable declared props, and init values
  ...
  onAttributeSelectedUpdate = ($event) => {
    console.log("==>" + this.constructor.name + ": onAttributeSelectedUpdate");
  ...
  selectAttributeRow = (attributeRowSelectedEvent: AttributeRowSelectedEvent) => {
    console.log("==>" + this.constructor.name + ": selectAttributeRow " + attributeRowSelectedEvent.attributeModel.name);
    ...
    // Build hirarchy tree for the navigation and update attributesNavigationData with it.
  ...
  declareAttributes = (): void => {
    console.log("==>" + this.constructor.name + ": declareAttributes");
    ...
    .subscribe((response) => {
      ...
      _.forEach(response, (output: OutputBEModel) => {
        const newInput: OutputFEModel = new OutputFEModel(output);
        this.outputsUtils.resetOutputDefaultValue(newInput, output.defaultValue);
        this.outputs.push(newInput);
        this.updateAttributeValueAfterDeclare(newInput);
      });
    }, error => {
    }); //ignore error
  };
  ...
  saveChangedData = (): Promise<(AttributeBEModel | OutputBEModel)[]> => {
    return new Promise((resolve, reject) => {
      ...
      let changedOutputsAttribuyes = [];
      if (this.isAttributesTabSelected) {
        const changedAttributes: AttributeBEModel[] = this.changedData.map((changedAttrib) => {
          changedAttrib = <AttributeFEModel>changedAttrib;
          const propBE = new AttributeBEModel(changedAttrib);
          propBE.toscaPresentation = new ToscaPresentationData();
          propBE.toscaPresentation.ownerId = changedAttrib.parentUniqueId;
          propBE.value = changedAttrib.getJSONValue();
          propBE.name = changedAttrib.origName || changedAttrib.name;
        ...
        handleSuccess = (response) => {
          ...
          response.forEach((resOutput) => {
            const changedInput = <OutputFEModel>this.changedData.shift();
            this.outputsUtils.resetOutputDefaultValue(changedInput, resOutput.defaultValue);
            changedInput.required = resOutput.required;
            // changedInput.requiredOrig = resOutput.required;
          });
  ...
  updateAttributeValueAfterDeclare = (output: OutputFEModel) => {
    if (this.instanceFeAttributesMap[output.instanceUniqueId]) {
      const instanceName = output.instanceUniqueId.slice(output.instanceUniqueId.lastIndexOf('.') + 1);
      const attributeForUpdatindVal = _.find(this.instanceFeAttributesMap[output.instanceUniqueId], (feAttribute: AttributeFEModel) => {
      ...
      const inputPath = (output.outputPath && output.outputPath != attributeForUpdatindVal.name) ? output.outputPath : undefined;
      attributeForUpdatindVal.setAsDeclared(inputPath); //set prop as declared before assigning value
      this.attributesService.disableRelatedAttributes(attributeForUpdatindVal, inputPath);
      this.attributesUtils.resetAttributeValue(attributeForUpdatindVal, output.relatedAttributeValue, inputPath);
    }
  }
  ...
  updateCheckedAttributeCount = (increment: boolean): void => {
    ...
    console.log("checkedAttributes count is now.... " + this.checkedAttributesCount);
  };
  ...
  deleteOutput = (output: OutputFEModel) => {
    ...
    .subscribe(response => {
      this.outputs = this.outputs.filter(input => input.uniqueId !== response.uniqueId);
        ...
        [
          new ButtonModel('Save', 'blue', () => {
            ...
            .subscribe((response) => {
              ...
              const newProp: AttributeFEModel = this.attributesUtils.convertAddAttributeBEToAttributeFE(response);
              this.instanceFeAttributesMap[this.component.uniqueId].push(newProp);
