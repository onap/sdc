...
export class AttributesOutputsComponent {
  ...
  updateAttributeValueAfterDeclare = (output: OutputFEModel) => {
    ...
    if (attributeList) {
      ...
      const inputPath = (output.outputPath && output.outputPath != attributeForUpdatingVal.name) ? output.outputPath : undefined;
      attributeForUpdatingVal.setAsDeclared(inputPath); //set prop as declared before assigning value
      this.attributesService.disableRelatedAttributes(attributeForUpdatingVal, inputPath);
      this.attributesUtils.resetAttributeValue(attributeForUpdatingVal, output.relatedAttributeValue, inputPath);
    }
  }
