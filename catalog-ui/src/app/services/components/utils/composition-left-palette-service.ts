    ...
    private updateLeftPalette = (componentInternalType:string):void => {
        ...
        if (authConfig.enabled) {
            httpOptions = {
                ...
                params: { 'internalComponentType': componentInternalType}
            };
        } else {
            httpOptions = {params: {'internalComponentType': componentInternalType}};
        }

        this.$http.get(leftPaletteUrl, httpOptions).then((res) => res.data).then((leftPaletteComponentMetadata:Array<ComponentMetadata>) => {
