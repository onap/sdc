...
public final class ConfigurationRepository {
    ...
    private static Set<String> validCallers = Collections.unmodifiableSet(new HashSet<>(
               Arrays.asList(ConfigurationChangeNotifier.NotificationData.class.getName(), ConfigurationUtils.class.getName(),
                          CliConfigurationImpl.class.getName(), ConfigurationChangeNotifier.class.getName(), ConfigurationDataSource.class.getName(),
    ...
    private LinkedHashMap<String, ConfigurationHolder> store = new LinkedHashMap<String, ConfigurationHolder>(16, 0.75f, true) {
        @Override
    ...
    public void initTenantsAndNamespaces() {
        try {
            Collection<String> collection = ConfigurationUtils.executeSelectSql(
                       getConfigurationFor(Constants.DEFAULT_TENANT, Constants.DB_NAMESPACE).getString("fetchnamescql"), new String[] {});
    ...
    public Configuration getConfigurationFor(String tenant, String namespace) throws Exception {
        ...
        if (config == null) {
            config = new ConfigurationHolder(ConfigurationUtils.getDbConfigurationBuilder(tenant + Constants.KEY_ELEMENTS_DELEMETER + namespace));
    ...
    public LinkedHashMap<String, ConfigurationHolder> getStore() {
        return store;
    }
    ...
    private class ConfigurationHolder {
        ...
        private Map<String, FileBasedConfigurationBuilder<FileBasedConfiguration>> overrideConfiguration = new LinkedHashMap<>();
        ...
        public Configuration getConfiguration(String namespace) throws Exception {
            if (config == null) {
                ...
                if ((temp != null) && (lastConfigChangeTimestamp == null || temp.getTime() > lastConfigChangeTimestamp.getTime())) {
                    builder.resetResult();
                    config = builder.getConfiguration();
        ...
        public Timestamp getLastUpdateTimestampFor(String namespace) {
            ...
            try {
                Collection<String> collection = ConfigurationUtils.executeSelectSql(
                           getConfigurationFor(Constants.DEFAULT_TENANT, Constants.DB_NAMESPACE).getString("fetchlastchangecql"),
