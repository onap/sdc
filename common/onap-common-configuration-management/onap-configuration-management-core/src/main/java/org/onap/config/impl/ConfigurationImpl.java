...
public class ConfigurationImpl implements org.onap.config.api.Configuration {
    ...
    public <T> T get(String tenant, String namespace, String key, Class<T> clazz, Hint... hints) {
        ...
        T returnValue;
        returnValue = (T) getInternal(tenant, namespace, key, clazz, hints);
        if ((returnValue == null || ConfigurationUtils.isZeroLengthArray(clazz, returnValue))
                && !Constants.DEFAULT_TENANT.equals(tenant)) {
            returnValue = (T) getInternal(Constants.DEFAULT_TENANT, namespace, key, clazz, hints);
        }
        if ((returnValue == null || ConfigurationUtils.isZeroLengthArray(clazz, returnValue))
                && !Constants.DEFAULT_NAMESPACE.equals(namespace)) {
            returnValue = (T) getInternal(tenant, Constants.DEFAULT_NAMESPACE, key, clazz, hints);
        }
        if ((returnValue == null || ConfigurationUtils.isZeroLengthArray(clazz, returnValue))
                && !Constants.DEFAULT_NAMESPACE.equals(namespace) && !Constants.DEFAULT_TENANT.equals(tenant)) {
            returnValue = (T) getInternal(Constants.DEFAULT_TENANT, Constants.DEFAULT_NAMESPACE, key, clazz, hints);
        }
    ...
    public Map generateMap(String tenantId, String namespace, String key) {
        ...
        try {
            if (isBlank(key)) {
                keys = ConfigurationRepository.lookup().getConfigurationFor(calculatedTenantId, calculatedNamespace).getKeys();
            } else {
                keys = ConfigurationRepository.lookup().getConfigurationFor(calculatedTenantId, calculatedNamespace).getKeys(key);
            }
            ...
            keys.forEachRemaining(k -> {
                ...
                String value = getAsString(calculatedTenantId, calculatedNamespace, k);
                if (!isBlank(key) && k.startsWith(key + ".")) {
                ...
                while (k.contains(".")) {
                    ...
                    if (!map.get().containsKey(subkey)) {
                        Map tmp = new HashMap<>();
                        map.get().put(subkey, tmp);
                        map.set(tmp);
                    } else {
                        map.set((Map) map.get().get(subkey));
                    }
                }
                map.get().put(k, value);
            });
