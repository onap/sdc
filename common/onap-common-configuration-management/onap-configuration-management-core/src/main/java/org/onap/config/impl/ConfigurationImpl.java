...
public class ConfigurationImpl implements org.onap.config.api.Configuration {
    ...
    public ConfigurationImpl() throws Exception {
                    ...
                    for (File tenanatFileRoot : tenantsRoot) {
                        if (file.getAbsolutePath().startsWith(tenanatFileRoot.getAbsolutePath())) {
                            moduleName = ConfigurationUtils.getConfigurationRepositoryKey(
                                       (tenanatFileRoot.getName().toUpperCase() + Constants.TENANT_NAMESPACE_SAPERATOR + moduleName)
        ...
        if (nodeConfigLocation != null && nodeConfigLocation.trim().length() > 0) {
            ...
            for (File file : filesystemResources) {
                if (filePredicate.test(file)) {
                    ConfigurationRepository.lookup().populateOverrideConfigurtaion(ConfigurationUtils
                                          .getConfigurationRepositoryKey(ConfigurationUtils.getNamespace(file).split(Constants.TENANT_NAMESPACE_SAPERATOR)),
                               file);
                }
            }
        }
    ...
    @Override
    public void addConfigurationChangeListener(String tenant, String namespace, String key, ConfigurationChangeListener myself) {
    ...
    private void populateFinalConfigurationIncrementally(Map<String, AggregateConfiguration> configs) throws Exception {
        boolean isDbAccessible = false;
        if (configs.get(Constants.DEFAULT_TENANT + Constants.KEY_ELEMENTS_DELEMETER + Constants.DB_NAMESPACE) != null) {
            ConfigurationRepository.lookup().populateConfigurtaion(
                       ...
                       configs.remove(Constants.DEFAULT_TENANT + Constants.KEY_ELEMENTS_DELEMETER + Constants.DB_NAMESPACE).getFinalConfiguration());
            isDbAccessible = ConfigurationUtils.executeDdlSql(
                       ConfigurationRepository.lookup().getConfigurationFor(Constants.DEFAULT_TENANT, Constants.DB_NAMESPACE)
                                  .getString("createtablecql"));
            if (isDbAccessible) {
                ConfigurationUtils.executeDdlSql(
                           ConfigurationRepository.lookup().getConfigurationFor(Constants.DEFAULT_TENANT, Constants.DB_NAMESPACE)
        ...
        for (String module : modules) {
            if (isDbAccessible) {
                DatabaseConfiguration config = ConfigurationUtils.getDbConfigurationBuilder(module).getConfiguration();
                Configuration currentConfig = configs.get(module).getFinalConfiguration();
                Iterator<String> keys = currentConfig.getKeys();
                while (keys.hasNext()) {
                    String currentKey = keys.next();
                    if (!(Constants.MODE_KEY.equals(currentKey) || Constants.NAMESPACE_KEY.equals(currentKey) || Constants.LOAD_ORDER_KEY.equals(
                               currentKey))) {
                ...
                ConfigurationRepository.lookup().populateConfigurtaion(module, configs.get(module).getFinalConfiguration());
            }
        }
    }
    ...
    public <T> T get(String tenant, String namespace, String key, Class<T> clazz, Hint... hints) {
        ...
        if (tenant == null && namespace != null && (tenantNamespaceArrayy = namespace.split(Constants.TENANT_NAMESPACE_SAPERATOR)).length > 1) {
        ...
        if ((returnValue == null || ConfigurationUtils.isZeroLengthArray(clazz, returnValue)) && !Constants.DEFAULT_TENANT.equals(tenant)) {
            returnValue = (T) getInternal(Constants.DEFAULT_TENANT, namespace, key, clazz.isPrimitive() ? getWrapperClass(clazz) : clazz,
                       hints == null || hints.length == 0 ? new Hint[] { Hint.EXTERNAL_LOOKUP, Hint.NODE_SPECIFIC } : hints);
        }
        if ((returnValue == null || ConfigurationUtils.isZeroLengthArray(clazz, returnValue)) && !Constants.DEFAULT_NAMESPACE.equals(namespace)) {
            returnValue = (T) getInternal(tenant, Constants.DEFAULT_NAMESPACE, key, clazz.isPrimitive() ? getWrapperClass(clazz) : clazz,
                       hints == null || hints.length == 0 ? new Hint[] { Hint.EXTERNAL_LOOKUP, Hint.NODE_SPECIFIC } : hints);
        }
        if ((returnValue == null || ConfigurationUtils.isZeroLengthArray(clazz, returnValue)) && !Constants.DEFAULT_NAMESPACE.equals(namespace)
    ...
    protected <T> T getInternal(String tenant, String namespace, String key, Class<T> clazz, Hint... hints) {
        ...
        try {
            if (ConfigurationUtils.isWrapperClass(clazz) || clazz.isPrimitive()) {
                Object obj = ConfigurationUtils.getProperty(ConfigurationRepository.lookup().getConfigurationFor(tenant, namespace), key,
                           processingHints);
                if (obj != null) {
            ...
            } else if (clazz.isArray() && (clazz.getComponentType().isPrimitive() || ConfigurationUtils.isWrapperClass(clazz.getComponentType()))) {
                Object obj = ConfigurationUtils.getProperty(ConfigurationRepository.lookup().getConfigurationFor(tenant, namespace), key,
                           processingHints);
                if (obj != null) {
                ...
                return read(tenant, namespace, clazz, (key == null || key.trim().length() == 0) ? "" : (key + "."), hints);
            } else {
                throw new IllegalArgumentException("Only pimitive classes, wrapper classes, corresponding array classes and any "
    ...
    private <T> T read(String tenant, String namespace, Class<T> clazz, String keyPrefix, Hint... hints) throws Exception {
        ...
        for (Field field : clazz.getDeclaredFields()) {
            ...
            if (fieldConfAnnot != null) {
                if (field.getType().isPrimitive() || ConfigurationUtils.isWrapperClass(field.getType()) || (field.getType().isArray() && (
                           field.getType().getComponentType().isPrimitive() || ConfigurationUtils.isWrapperClass(field.getType().getComponentType())))
                           || field.getType().getAnnotation(Config.class) != null) {
                    field.set(objToReturn, get(tenant, namespace, keyPrefix + fieldConfAnnot.key(), field.getType(), hints));
                    ...
                    if (obj != null) {
                        ...
                        if (field.getType().isInterface()) {
                            ...
                            clazzToInstantiate = ConfigurationUtils.getCompatibleCollectionForAbstractDef(field.getType()).getClass();
                        } else {
                        ...
                        } else if ((construct = getConstructorWithArguments(clazzToInstantiate, Integer.class, Boolean.class, Collection.class))
                                   != null) {
                            construct.setAccessible(true);
    ...
    private <T> T getValue(Object obj, Class<T> clazz, int processingHint) {
        ...
        if (String.class.equals(clazz)) {
            if (obj.toString().startsWith("@") && ConfigurationUtils.isExternalLookup(processingHint)) {
                String contents = ConfigurationUtils.getFileContents(NonConfigResource.locate(obj.toString().substring(1).trim()));
                if (contents == null) {
    ...
    @Override
    public void removeConfigurationChangeListener(String tenant, String namespace, String key, ConfigurationChangeListener myself) {
        tenant = ConfigurationRepository.lookup().isValidTenant(tenant) ? tenant.toUpperCase() : Constants.DEFAULT_TENANT;
        namespace = ConfigurationRepository.lookup().isValidNamespace(namespace) ? namespace.toUpperCase() : Constants.DEFAULT_NAMESPACE;
        if (key == null || key.trim().length() == 0) {
            throw new IllegalArgumentException("Key can't be null.");
    ...
    public <T> Map<String, T> populateMap(String tenantId, String namespace, String key, Class<T> clazz) {
        ...
        try {
            if (ConfigurationRepository.lookup().isDBAccessible()) {
                keys = ConfigurationUtils.executeSelectSql(
                           ConfigurationRepository.lookup().getConfigurationFor(Constants.DEFAULT_TENANT, Constants.DB_NAMESPACE)
                                      .getString("fetchkeysql"), new String[] { tenantId + Constants.KEY_ELEMENTS_DELEMETER + namespace }).iterator();
            } else {
