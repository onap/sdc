...
public final class CliConfigurationImpl extends ConfigurationImpl implements ConfigurationManager {
  ...
  private void handleUnregisteredNotification(MBeanServerNotification mbs) {
    try {
      ...
      LOGGER.error("Error while setting unregistered notification properties value", exception);
    }
  }
  ...
  private String getConfigurationValue(ConfigurationQuery queryData) {
    try {
      if (queryData.isFallback()) {
        return ConfigurationUtils.getCommaSaperatedList(getConfigurationFallbackValue(queryData));
      } else {
        String[] list =
      ...
      LOGGER.error("Error while getting configuration value", exception);
    }
    return null;
  }
  ...
  private void updateConfigurationValue(ConfigurationUpdate updateData) {
    ...
    try {
      ...
      LOGGER.error("Error while updating configuration value", exception);
    }
  }
  ...
  private void validateTenantAndNamespace(ConfigurationUpdate updateData) {
    try {
      ...
      LOGGER.error("Value for Tenant or Namespace missing", e);
    }
  }
  ...
  private boolean isKeyExists(String tenant, String namespace, String key) {
    ...
    try {
      ...
      LOGGER.error("Error while checking if key exists in configuration", exception);
    }
    return keyExist;
  }
  ...
  private Map<String, String> listConfiguration(ConfigurationQuery query) {
    ...
    try {
      ...
      LOGGER.error("Error while listing configuration", exception);
      return null;
    }
    return map;
  }
  ...
  private Object getInput(Map<String, Object> input) {
    ...
    try {
      ...
      LOGGER.error("Error while getting configuration input", exception);
    }
  ...
  private ArrayList<String> getInMemoryKeys(String tenant, String namespace) {
    ...
    try {
      ...
      LOGGER.error("Error while getting in memory keys", exception);
    }
  ...
  public Collection<String> getKeys(String tenant, String namespace) {
    ...
    try {
      ...
      LOGGER.error("Error while getting all configuration keys", exception);
