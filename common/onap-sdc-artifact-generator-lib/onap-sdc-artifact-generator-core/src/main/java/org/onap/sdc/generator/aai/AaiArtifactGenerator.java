...
public class AaiArtifactGenerator implements ArtifactGenerator {
    ...
    @Override
    @Audit
    public GenerationData generateArtifact(List<Artifact> input, Map<String, String> additionalParams) {
        try {
     ...
     * @return Translated/Error data as a {@link GenerationData} object
     */
    private GenerationData generateArtifactInternal(List<Artifact> input, Map<String, String> additionalParams) {
        ...
        if (serviceVersion == null) {
            ...
            if (!(serviceVersion.matches(versionRegex))) {
                throw new IllegalArgumentException(String.format(GeneratorConstants.GENERATOR_AAI_INVALID_SERVICE_VERSION));
            }
        }
        ...
        if (serviceTosca.getTopology_template() != null && serviceTosca.getTopology_template().getNode_templates() != null) {
        ...
        while (toscas.size() > 0) {
            ...
            if (mapValue == null) {
                log.warn("Additional tosca file found with resource version id : " + resourceUuId);
                continue;
            }
            ...
            model.populateModelIdentificationInformation(resourceTemplate.getMetadata());  //Get base resource metadata information
            //Found model from the type store so removing the same
            idTypeStore.remove(model.getModelNameVersionId());
            if (resourceTemplate.getTopology_template() != null && resourceTemplate.getTopology_template().getNode_templates() != null) {
                processVfTosca(idTypeStore, resourceTemplate, model);
            }

            //Process group information from tosca for vfModules
            if (resourceTemplate.getTopology_template() != null && resourceTemplate.getTopology_template().getGroups() != null) {
                processVfModule(resources, currentToscaResources, resourceTemplate, model);
            } else {
                model.getWidgets().clear();
            }

            if ("Tunnel XConnect".equals(resourceTemplate.getMetadata().get("subcategory")) && "Allotted Resource".equals(
                       resourceTemplate.getMetadata().get("category"))) {
                model.addWidget(new TunnelXconnectWidget());
            }

            resources.add((Resource) model);
            currentToscaResources.clear();    //Clearing the current tosca resource list for the next iteration
            counter = 0;
        }
    ...
    private void validateResourceToscaAgainstService(Map<String, String> idTypeStore, List<ToscaTemplate> toscas) {
    ...
    private ToscaTemplate preProcessingTosca(ToscaTemplate tosca) {
        ...
        if (tosca.getTopology_template() != null && tosca.getTopology_template().getNode_templates() != null) {
            Collection<NodeTemplate> coll = processedTosca.getTopology_template().getNode_templates().values();
            for (NodeTemplate node : coll) {

                if (node.getType().contains("org.openecomp.resource.vf.") && node.getMetadata().get("category").equals("Allotted Resource")) {
                    node.setType("org.openecomp.resource.vf.allottedResource");
                }
                if (node.getType().contains("org.openecomp.resource.vfc.") && node.getMetadata().get("category").equals("Allotted Resource")) {
    ...
    private void processVfTosca(Map<String, String> idTypeStore, ToscaTemplate resourceTemplate, Model model) {
        ...
        for (String key : keys) {
            ...
            if (resourceNode != null) {
                if (resourceNode instanceof ProvidingService) {
                    ...
                    if (nodeProperties.get("providing_service_uuid") == null || nodeProperties.get("providing_service_invariant_uuid") == null) {
                        throw new IllegalArgumentException(
                                   String.format(GeneratorConstants.GENERATOR_AAI_PROVIDING_SERVICE_METADATA_MISSING, model.getModelId()));
                    }
        ...
        if (model instanceof AllotedResource) {
            if (!flag) {
                throw new IllegalArgumentException(String.format(GeneratorConstants.GENERATOR_AAI_PROVIDING_SERVICE_MISSING, model.getModelId()));
            }
        }
    }

    private void processVfModule(List<Resource> resources, List<Resource> currentToscaResources, ToscaTemplate resourceTemplate, Model model) {
        log.debug("Inside Resource artifact generation for group/vfModule");
        Collection<GroupDefinition> groups = resourceTemplate.getTopology_template().getGroups().values();
                ...
                if (group instanceof VfModule && !currentToscaResources.contains(group)) {
                    if (gd.getMembers() != null && !gd.getMembers().isEmpty()) {
                        ...
                        for (String member : groupMembers) {
                            NodeTemplate node = resourceTemplate.getTopology_template().getNode_templates().get(member);
                            if (node != null) {
                                Model resourceNode;
                    ...
                    currentToscaResources.add((Resource) group); //Adding the VfModule group to file specific resources
        ...
        model.getWidgets().removeIf(widget -> widget.allInstancesUsed(nodeNameListForGroups) || true);
    }

    private void processServiceTosca(Service service, Map<String, String> idTypeStore, Map<String, String> resourcesVersion,
                                     ToscaTemplate serviceTosca, List<Resource> resources) {
        Collection<NodeTemplate> coll = serviceTosca.getTopology_template().getNode_templates().values();
    ...
    private Artifact getServiceArtifact(Model serviceModel, String aaiServiceModel) {
        return getArtifact(serviceModel, aaiServiceModel);
    }

    private Artifact getArtifact(Model serviceModel, String aaiServiceModel) {
    ...
    private Artifact getResourceArtifact(Model resourceModel, String aaiResourceModel) {
        return getArtifact(resourceModel, aaiResourceModel);
    }
    ...
    private void validateVersion(String version, String uuId) {
        ...
        if (null == version || version == "") {
            throw new IllegalArgumentException(String.format(GeneratorConstants.GENERATOR_AAI_ERROR_NULL_RESOURCE_VERSION_IN_SERVICE_TOSCA, uuId));
        } else if (version.equals("0.0") || !(version.matches(versionRegex))) {
            throw new IllegalArgumentException(String.format(GeneratorConstants.GENERATOR_AAI_ERROR_INVALID_RESOURCE_VERSION_IN_SERVICE_TOSCA, uuId));
        }
    }
     ...
     *
     * @param input
     *            Input tosca file and its metadata information as {@link Artifact} object
    ...
    private ToscaTemplate getToscaModel(Artifact input, String serviceVersion) {
        ...
        if (checksum.equalsIgnoreCase(input.getChecksum())) {
            try {
                ...
                throw new IllegalArgumentException(String.format(GeneratorConstants.GENERATOR_AAI_ERROR_INVALID_TOSCA, input.getName()), exception);
            }
        } else {
    ...
    private void validateTosca(ToscaTemplate tosca, Artifact input) {
        ...
        if (tosca.getMetadata().containsKey("invariantUUID")) {
            if (tosca.getMetadata().get("invariantUUID") == null || tosca.getMetadata().get("invariantUUID") == "") {
                throw new IllegalArgumentException(
                           String.format(GeneratorConstants.GENERATOR_AAI_ERROR_MANDATORY_METADATA_DEFINITION, "invariantUUID", input.getName()));
            } else if (tosca.getMetadata().get("invariantUUID").length() != GeneratorConstants.ID_LENGTH) {
                throw new IllegalArgumentException(
                           String.format(GeneratorConstants.GENERATOR_AAI_ERROR_INVALID_ID, "invariantUUID", input.getName()));
            }
        }
        ...
        if (tosca.getMetadata().containsKey("UUID")) {
            if (tosca.getMetadata().get("UUID") == null || tosca.getMetadata().get("UUID") == "") {
                throw new IllegalArgumentException(
                           String.format(GeneratorConstants.GENERATOR_AAI_ERROR_MANDATORY_METADATA_DEFINITION, "UUID", input.getName()));
            } else if (tosca.getMetadata().get("UUID").length() != GeneratorConstants.ID_LENGTH) {
                throw new IllegalArgumentException(String.format(GeneratorConstants.GENERATOR_AAI_ERROR_INVALID_ID, "UUID", input.getName()));
            }
        ...
        if (tosca.getMetadata().containsKey("name")) {
            if (tosca.getMetadata().get("name") == null || tosca.getMetadata().get("name") == "") {
                throw new IllegalArgumentException(
                           String.format(GeneratorConstants.GENERATOR_AAI_ERROR_MANDATORY_METADATA_DEFINITION, "name", input.getName()));
            }
        }
        ...
        if (tosca.getTopology_template() != null && tosca.getTopology_template().getGroups() != null) {
            ...
            for (GroupDefinition gd : groups) {
                ...
                if (group != null && group instanceof VfModule) {
                    if (gd.getMetadata().containsKey("vfModuleModelName") && gd.getMetadata().get("vfModuleModelName") == null) {
                        throw new IllegalArgumentException(
                                   String.format(GeneratorConstants.GENERATOR_AAI_ERROR_MANDATORY_METADATA_DEFINITION, "vfModuleModelName",
                                              input.getName()));
                    }
                    if (gd.getMetadata().containsKey("vfModuleModelInvariantUUID") && gd.getMetadata().get("vfModuleModelInvariantUUID") == null) {
                        throw new IllegalArgumentException(
                                   String.format(GeneratorConstants.GENERATOR_AAI_ERROR_MANDATORY_METADATA_DEFINITION, "vfModuleModelInvariantUUID",
                                              input.getName()));
                    } else if (gd.getMetadata().get("vfModuleModelInvariantUUID").length() != GeneratorConstants.ID_LENGTH) {
                        throw new IllegalArgumentException(
                                   String.format(GeneratorConstants.GENERATOR_AAI_ERROR_INVALID_ID, "vfModuleModelInvariantUUID", input.getName()));
                    }

                    if (gd.getMetadata().containsKey("vfModuleModelUUID") && gd.getMetadata().get("vfModuleModelUUID") == null) {
                        throw new IllegalArgumentException(
                                   String.format(GeneratorConstants.GENERATOR_AAI_ERROR_MANDATORY_METADATA_DEFINITION, "vfModuleModelUUID",
                                              input.getName()));
                    } else if (gd.getMetadata().get("vfModuleModelUUID").length() != GeneratorConstants.ID_LENGTH) {
                        throw new IllegalArgumentException(
                                   String.format(GeneratorConstants.GENERATOR_AAI_ERROR_INVALID_ID, "vfModuleModelUUID", input.getName()));
                    }
                    if (gd.getMetadata().containsKey("vfModuleModelVersion") && gd.getMetadata().get("vfModuleModelVersion") == null) {
                        throw new IllegalArgumentException(
                                   String.format(GeneratorConstants.GENERATOR_AAI_ERROR_MANDATORY_METADATA_DEFINITION, "vfModuleModelVersion",
                                              input.getName()));
                    }
                }
            }
     ...
     *
     * @param input
     *            List of translated {@link ToscaTemplate tosca} object models
    ...
    private void initWidgetConfiguration() throws IOException {
        ...
        if (configLocation != null) {
            ...
            if (file.exists()) {
                ...
                throw new IllegalArgumentException(String.format(GeneratorConstants.GENERATOR_AAI_CONFIGFILE_NOT_FOUND, configLocation));
            }
        } else {
