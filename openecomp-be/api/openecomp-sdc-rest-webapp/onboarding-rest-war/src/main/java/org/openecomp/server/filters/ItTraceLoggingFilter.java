...
public class ItTraceLoggingFilter implements Filter {
    ...
    public void doFilter(final ServletRequest request, final ServletResponse response, final FilterChain chain)
        ...
        try {
            ...
            System.out.println("[IT_TRACE][ONBOARDING-BE] traceId=" + traceId + " ERROR " + httpRequest.getMethod() + " " + safePath(httpRequest) + " status=" + httpResponse.getStatus());
            throw t;
        } finally {
            final long durationMs = (System.nanoTime() - startNs) / 1_000_000;
            ...
            if (shouldLog) {
                ...
                if (status >= 400) {
                    System.out.println("[IT_TRACE][ONBOARDING-BE] traceId=" + traceId + " " + httpRequest.getMethod() + " " + safePath(httpRequest) + " status=" + status + " durationMs=" + durationMs);
                }
            }
            MDC.remove(MDC_KEY);
        }
