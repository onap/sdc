...
class HttpNotificationTask implements Callable<AsyncNotifier.NextAction> {
    ...
    public synchronized AsyncNotifier.NextAction call() {
        ...
        Collection<String> failedItemIds = this.itemIds;
        try (CloseableHttpClient client = HttpClients.createDefault()) {

            HttpPost request = createPostRequest(endpoint, this.itemIds, userId);

            try (CloseableHttpResponse response = client.execute(request)) {

                StatusLine status = response.getStatusLine();

                LOGGER.debug("Catalog notification on VSP IDs: {}, endpoint: {}, response: {}", this.itemIds, endpoint,
                        status);

                failedItemIds = getFailedIds(this.itemIds, response.getEntity());
                if(status.getStatusCode() == HttpStatus.SC_INTERNAL_SERVER_ERROR) {
                    LOGGER.debug("Catalog notification on VSP IDs {} failed. Endpoint: {}. Retry", failedItemIds, endpoint);
                    return RETRY;
                }
                return DONE;
            }
            ...
            LOGGER.error("Catalog notification on VSP IDs {} failed. Endpoint: {}", failedItemIds, endpoint, e);
            return DONE;
        }
    }
