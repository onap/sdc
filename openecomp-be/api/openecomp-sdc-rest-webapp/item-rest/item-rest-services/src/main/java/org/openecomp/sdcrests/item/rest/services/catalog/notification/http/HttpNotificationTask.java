    ...
    public synchronized AsyncNotifier.NextAction call() {
        ...
        try (CloseableHttpClient client = HttpClients.createDefault()) {
            ...
            try (CloseableHttpResponse response = client.execute(request)) {
                ...
                LOGGER.debug("Catalog notification on VSP IDs: {}, endpoint: {}, response: {}", itemIds, endpoint,
                        status);

                itemIds = getFailedIds(itemIds, response.getEntity());
                return status.getStatusCode() == HttpStatus.SC_INTERNAL_SERVER_ERROR ? RETRY : DONE;
            }
