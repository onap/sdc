...
public class HttpTaskProducer
    ...
    public HttpTaskProducer(HttpConfiguration config) {
        ...
        String protocol = config.getCatalogBeProtocol();
        if (protocol == null) {
            throw new EntryNotConfiguredException("Protocol");
        }

        String host = config.getCatalogBeFqdn();
        if (host == null) {
            throw new EntryNotConfiguredException("Catalog host");
        }

        String url = config.getCatalogNotificationUrl();
        if (url == null) {
            throw new EntryNotConfiguredException("Notification URL");
        }
    ...
    private static String getPortConfiguration(String protocol, HttpConfiguration config) {
        ...
        if (CATALOG_HTTP_PROTOCOL.equalsIgnoreCase(protocol)) {
            return config.getCatalogBeHttpPort();
        } else if (CATALOG_HTTPS_PROTOCOL.equalsIgnoreCase(protocol)) {
            return config.getCatalogBeSslPort();
        } else {
