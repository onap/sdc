...
public class VendorLicenseModelsImpl implements VendorLicenseModels {
    ...
    public Response deleteLicenseModel(String vlmId, String user) {
        ...
        final boolean isVlmArchived = vlm.getStatus() != null && vlm.getStatus().equals(ItemStatus.ARCHIVED);
        if ((isVlmNotCertified || isVlmArchived) && isVlmNotUsedByAnyVsp(vlm.getId())) {
    ...
    private boolean isVlmNotUsedByAnyVsp(final String vlmId) {
        final Collection<VspDetails> vspDetailsList = vendorSoftwareProductInfoDao.list(null);
        return vspDetailsList.stream().noneMatch(vspDetails -> vlmId.equals(vspDetails.getVendorId()));
    }
