...
public interface VendorSoftwareProducts extends VspEntities {
    ...
    ResponseEntity listVsps(@Parameter(description = "Filter to return only Vendor Software Products with at"
            + " least one version at this status. Currently supported values: 'Certified' , 'Draft'") @RequestParam(name = "versionFilter", required = false) String versionStatus,
                            @Parameter(description = "Filter to only return Vendor Software Products at this status."
                                    ...
                                    + "Default value = 'ACTIVE'.") @RequestParam(name = "Status", required = false) String itemStatus,
    ...
    ResponseEntity listPackages(@Parameter(description = "Vendor Software Product status filter. "
            + "Currently supported values: 'ACTIVE', 'ARCHIVED'") @RequestParam(name = "Status", required = false) String status,
                                @Parameter(description = "Category") @RequestParam(name = "category", required = false) String category,
                                @Parameter(description = "Sub-category") @RequestParam(name = "subCategory", required = false) String subCategory,
                                @NotNull(message = USER_MISSING_ERROR_MSG) @RequestHeader(USER_ID_HEADER_PARAM) String user);

    @GetMapping(value = "/{vspId}/versions/{versionId}/orchestration-template",
    ...
    ResponseEntity<Resource> getTranslatedFile(@PathVariable("vspId") String vspId, @RequestParam(name = "versionId", required = false) String versionId,
                                     @RequestHeader(USER_ID_HEADER_PARAM) String user);
