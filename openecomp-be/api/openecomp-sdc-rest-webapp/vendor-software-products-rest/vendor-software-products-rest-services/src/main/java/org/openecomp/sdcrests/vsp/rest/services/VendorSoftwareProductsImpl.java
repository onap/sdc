...
public class VendorSoftwareProductsImpl implements VendorSoftwareProducts {
    ...
    private final ArtifactStorageManager artifactStorageManager = new StorageFactory().createArtifactStorageManager();
    ...
    public Response deleteVsp(String vspId, String user) {
        ...
        if (Objects.isNull(certifiedVersionsCounter) || certifiedVersionsCounter == 0) {
            ...
            List<String> vspsIds = getVspList(null, ItemStatus.ARCHIVED.name(), user).stream().map(Item::getId).collect(Collectors.toList());
            if (vspsIds.contains(vspId)) {
                if (artifactStorageManager.isEnabled()) {
                    try {
                        deleteVspFromStorage(vspId);
                    } catch (Exception e) {
    ...
    private void deleteVspFromStorage(final String vspId) {
        versioningManager.list(vspId).forEach(version -> artifactStorageManager.delete(vspId, version.getId()));
    }

    private Response getResponseDeletionOk(String vspId, String user, Item vsp) {
