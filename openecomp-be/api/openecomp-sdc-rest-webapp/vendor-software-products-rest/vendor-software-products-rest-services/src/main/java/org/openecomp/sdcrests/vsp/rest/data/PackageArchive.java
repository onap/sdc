...
public class PackageArchive {
     ...
     */
    public byte[] getPackageFileContents() {
        try {
            if (isSignatureValid()) {
    ...
    public boolean isSignatureValid() {
        ...
        return certificateFile.map(s -> securityManager.verifySignedData(files.get(signatureFileName.get()),
                files.get(s), files.get(archiveFileName.get()))).orElseGet(() ->
                securityManager.verifySignedData(files.get(signatureFileName.get()),
