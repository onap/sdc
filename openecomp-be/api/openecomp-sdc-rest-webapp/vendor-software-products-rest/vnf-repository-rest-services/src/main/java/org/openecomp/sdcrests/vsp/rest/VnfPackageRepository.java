...
public interface VnfPackageRepository extends VspEntities {
  ...
  @ApiOperation(value = "Get VNF packages from VNF Repository",
      notes = "Call VNF Repostory to get VNF package details",
      response = File.class)
   Response getVnfPackages(
  ...
  @ApiOperation(value = "Download VNF package from VNF Repository",
      notes = "Download VNF package from VNF repository and send to client",
      response = File.class)
   Response downloadVnfPackage(
      @PathParam("vspId") String vspId,
      @ApiParam(value = "Version Id") @PathParam("versionId") String versionId, @PathParam("csarId") String csarId,
      @NotNull(message = USER_MISSING_ERROR_MSG) @HeaderParam(USER_ID_HEADER_PARAM) String user)
      throws Exception; 
