...
public class VnfPackageRepositoryImpl implements VnfPackageRepository {
    ...
    public static final Version VERSION01 = new Version("1.0");
    ...
    private Version getVersion(String vspId, String versionId) {
        ...
        Version versionObj =
                versioningManager.list(vspId).stream().filter(ver -> ver.getId() != versionId).findAny().orElse(null);

        if(null == versionObj) {
            versionObj = new Version(versionId);
        }
