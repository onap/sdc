...
public class ExternalTestingManagerImpl implements ExternalTestingManager {
  ...
  private ExternalTestingException buildTestingException(int statusCode, JsonObject o) {
    ...
    if (o.has("title")) {
      ...
      title = o.get("error").getAsString();
    }
    if (o.has("detail")) {
      detail = o.get("detail").getAsString();
    }
    else if (o.has("message")) {
      detail = o.get("message").getAsString();
    }
    if (o.has("path")) {
      if (detail == null) {
        detail = o.get("path").getAsString();
      }
      else {
        detail = detail + " " + o.get("path").getAsString();
      }
    }
    return new ExternalTestingException(title, statusCode, detail);
  }
