...
import org.springframework.web.client.ResourceAcupdateVtpResultInDBcessException;
...
public class ExternalTestingManagerImpl implements ExternalTestingManager {
  ...
  private void loadConfig() {
    ...
    try (InputStream fileInput = new FileInputStream(file)) {
      ...
      endpoints = accessConfig.getEndpoints().stream().flatMap(this::mapEndpointString).collect(Collectors.toList());
  ...
  public List<VtpTestExecutionResponse> execute(final List<VtpTestExecutionRequest> testsToRun, String vspId,
    ...
    return partitions.entrySet().stream()
                   .flatMap(e -> doExecute(e.getKey(), e.getValue(), vspId, vspVersionId, requestId, fileMap).stream())
  ...
  private String buildEndpointUrl(String format, String endpointName, String[] args) {
    if (endpoints != null) {
      RemoteTestingEndpointDefinition ep =
              endpoints.stream().filter(e -> e.isEnabled() && e.getId().equals(endpointName)).findFirst().orElseThrow(
                      () -> new ExternalTestingException(NO_SUCH_ENDPOINT_ERROR_CODE, 500,
                              "No endpoint named " + endpointName + " is defined"));
  ...
  private void extractMetadata(VtpTestExecutionRequest requestItem, MultiValueMap<String, Object> body, String vspId,
    ...
    if (!ozip.isPresent()) {
      ...
      String knownVersions = versions.stream().map(v -> String.format("%d.%d: %s (%s)", v.getMajor(), v.getMinor(),
