...
public class ExternalTestingManagerImplTests {
  ...
  private ExternalTestingManager configTestManager(boolean loadConfig) throws Exception {
    ...
    problemHeaders.setContentType(MediaType.parseMediaType("application/json"));
    ...
    Mockito
        .when(restTemplate.getForEntity(
            ...
            ArgumentMatchers.eq(org.openecomp.core.externaltesting.api.Test.class)))
        .thenThrow(new HttpClientErrorException(HttpStatus.NOT_FOUND, "Not Found", problemHeaders, notFound.getBytes(), Charset.defaultCharset()));

    Mockito
        .when(restTemplate.getForEntity(
            ArgumentMatchers.endsWith("/servererror"),
            ArgumentMatchers.eq(org.openecomp.core.externaltesting.api.Test.class)))
        .thenThrow(new ResourceAccessException("Simulated IO Exception"));
  ...
  public void testManagerWithMultipleEndpoints() throws Exception {
    ...
    TestExecutionRequestItem t = new TestExecutionRequestItem();
    t.setTestId("foo");
    inputTests.add(t);
    ...
    Assert.assertEquals("Status Match", ter2.getStatus(), "Completed");
  }
  ...
  public void testManager() throws Exception {
    ...
    try {
      m.getTestDefinition("missing");
      Assert.fail("should not have reached this line due to exception");
    }
    catch (ExternalTestingException e) {
      Assert.assertEquals("status code", e.getCode(), 404);
    }

    // test a missing resource...
    try {
      m.getTestDefinition("servererror");
      Assert.fail("should not have reached this line due to exception");
    }
    catch (ExternalTestingException e) {
      Assert.assertEquals("status code", e.getCode(), 500);
    }
