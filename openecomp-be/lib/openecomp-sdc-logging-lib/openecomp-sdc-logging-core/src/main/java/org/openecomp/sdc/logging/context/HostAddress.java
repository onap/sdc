 ...
 * calls, but at the same time pick up any IP or FQDN changes.
 *
 * @author evitaliy
 * @since 26 Mar 2018
 */
...
public class HostAddress {
    ...
    private CacheEntry cashedAddress;
    ...
    public HostAddress(long refreshInterval) {
        ...
        this.cashedAddress = new CacheEntry(System.currentTimeMillis(), read());
    }
    ...
    public synchronized InetAddress get() {
        ...
        if (current - cashedAddress.lastUpdated < interval) {
            return cashedAddress.address;
        }

        InetAddress address = read();
        cashedAddress = new CacheEntry(current, address);
        return address;
    }
