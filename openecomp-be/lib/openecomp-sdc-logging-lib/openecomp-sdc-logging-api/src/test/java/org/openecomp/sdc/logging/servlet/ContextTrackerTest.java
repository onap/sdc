...
public class ContextTrackerTest {
    ...
    private static final Supplier<String> SERVICE_NAME = () -> "Test-Service";

    @Test(expected = NullPointerException.class)
    public void throwExceptionWhenPartnerNamesNull() {
        new ContextTracker(null, REQUEST_ID_HEADER, SERVICE_NAME);
    }

    @Test(expected = NullPointerException.class)
    public void throwExceptionWhenRequestIdsNull() {
        new ContextTracker(PARTNER_NAME_HEADER, null, SERVICE_NAME);
    }

    @Test(expected = NullPointerException.class)
    public void throwExceptionWhenNameSupplierNull() {
        new ContextTracker(PARTNER_NAME_HEADER, REQUEST_ID_HEADER, null);
    }
    ...
    public void requestIdCopiedWhenGiven() {
        ...
        ContextTracker tracker = new ContextTracker(PARTNER_NAME_HEADER, REQUEST_ID_HEADER, SERVICE_NAME);
    ...
    @Test
    public void serviceNameCopied() {

        mockStatic(LoggingContext.class);

        final String serviceName = "temp-service-for-unit-testing";
        ContextTracker tracker = new ContextTracker(PARTNER_NAME_HEADER, REQUEST_ID_HEADER, () -> serviceName);
        tracker.preRequest(mock(HttpServletRequest.class));
        ...
        assertEquals(serviceName, contextDataCaptor.getValue().getServiceName());
    }
