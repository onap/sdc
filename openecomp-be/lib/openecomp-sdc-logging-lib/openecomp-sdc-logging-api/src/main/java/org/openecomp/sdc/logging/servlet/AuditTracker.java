...
public class AuditTracker implements Tracker {
    ...
    public AuditTracker(Logger logger, Function<Integer, Result> statusCodeToMessage) {
        this.logger = Objects.requireNonNull(logger);
        this.statusCodeInterpreter = Objects.requireNonNull(statusCodeToMessage);
    }
    ...
    public synchronized void postRequest(HttpServletRequest request, HttpServletResponse response) {
        ...
        AuditData auditData = AuditData.builder().startTime(started).endTime(end)
                                       .statusCode(result.getStatus())
                                       .responseCode(Integer.toString(responseCode))
                                       .responseDescription(result.getMessage())
