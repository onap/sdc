...
import org.openecomp.sdc.logging.servlet.ServiceNameFormatter;
...
public class LoggingRequestFilter implements ContainerRequestFilter {
    ...
    private static final Function<Integer, AuditTracker.Result> RESPONSE_INTERPRETER = new ResponseStatusInterpreter();
    ...
    public void filter(ContainerRequestContext requestContext) {
        ...
        AuditTracker audit = new AuditTracker(resource.getResourceMethod().getDeclaringClass(), RESPONSE_INTERPRETER);
        ContextTracker context = new ContextTracker(partnerNameHeader, requestIdHeader,
                () -> ServiceNameFormatter.format(httpRequest.getMethod(), httpRequest.getRequestURI()));

        Tracker tracker = new CombinedTracker(context, audit);
        requestContext.setProperty(LOGGING_TRACKER_KEY, tracker);
        tracker.preRequest(httpRequest);
    }

    private static class ResponseStatusInterpreter implements Function<Integer, AuditTracker.Result> {

        @Override
        public AuditTracker.Result apply(Integer status) {
            return new JaxRsMappedResult(status);
        }
    }

    private static class JaxRsMappedResult implements AuditTracker.Result {

        private final int status;

        private JaxRsMappedResult(int status) {
            this.status = status;
        }
