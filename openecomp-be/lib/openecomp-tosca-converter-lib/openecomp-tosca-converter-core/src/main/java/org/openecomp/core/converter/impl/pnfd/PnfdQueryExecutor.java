...
public class PnfdQueryExecutor {
    ...
    private static boolean findMap(final Map query, final Map yamlObject) {
        ...
        for (final Object queryEntryObj : query.entrySet()) {
            final Entry queryEntry = (Entry) queryEntryObj;
            final Object queryValue = queryEntry.getValue();
            final Object yamlObjectValue = yamlObject.get(queryEntry.getKey());
            if (!find(queryValue, yamlObjectValue)) {
                return false;
            }
        }
