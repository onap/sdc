...
public class PnfdTransformationEngine {
    ...
    PnfdTransformationEngine(final ServiceTemplateReaderService templateFrom, final ServiceTemplate templateTo, final String descriptorResourcePath) {
    ...
    private void executeGetInputFunctionTransformations(final Map<String, String> inputsToConvertMap) {
        final List<Transformation> getInputTransformationList = transformationGroupByBlockMap
            .get(TransformationBlock.GET_INPUT_FUNCTION);

        if(MapUtils.isEmpty(inputsToConvertMap) || CollectionUtils.isEmpty(getInputTransformationList)) {
            return;
        }

        final Map<String, List<Transformation>> transformationByName = getInputTransformationList.stream()
