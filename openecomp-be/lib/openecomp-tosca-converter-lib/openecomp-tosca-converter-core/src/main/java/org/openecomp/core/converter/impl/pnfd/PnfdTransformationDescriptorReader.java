...
public class PnfdTransformationDescriptorReader {
    ...
    private List<Object> readDescriptionYaml(final InputStream transformationDescriptionPath) {
        try (final InputStream fileInputStream = transformationDescriptionPath) {
            final LoadSettings settings = new LoadSettingsBuilder().build();
            final Load load = new Load(settings);

            return (List<Object>) load.loadFromInputStream(fileInputStream);
        } catch (final FileNotFoundException e) {
            LOGGER.error(String.format("Could not find the resource on path. %s", e));
        } catch (final IOException e) {
            LOGGER.error(String.format("Could not load resource. %s", e));
        }
