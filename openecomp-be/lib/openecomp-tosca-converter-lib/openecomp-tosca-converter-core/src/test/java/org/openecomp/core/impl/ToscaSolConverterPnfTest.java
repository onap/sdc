...
public class ToscaSolConverterPnfTest {
    ...
    public static Collection input() throws IOException {
        return Files.list(getPathFromClasspath("pnfDescriptor/in"))
    ...
    private String getExpectedResultFor(String inputFilename) throws IOException {
        ServiceTemplate serviceTemplate = toscaExtensionYamlUtil.
                yamlToObject(getOutputFileResourceCorrespondingTo(inputFilename), ServiceTemplate.class);
    ...
    private InputStream getOutputFileResourceCorrespondingTo(String inputFilename) throws IOException {
        String outputFilename = getOutputFilenameFrom(inputFilename);
        return Thread.currentThread().getContextClassLoader().getResourceAsStream(OUTPUT_DIR + outputFilename);
    }
