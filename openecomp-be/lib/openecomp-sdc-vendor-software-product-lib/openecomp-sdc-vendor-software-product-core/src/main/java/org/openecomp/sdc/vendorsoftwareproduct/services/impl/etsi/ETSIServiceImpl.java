...
public class ETSIServiceImpl implements ETSIService {
    ...
    public boolean isEtsiPackage(final FileContentHandler fileContentHandler) throws IOException {
        if (isSol004WithToscaMetaDirectory(fileContentHandler) && fileContentHandler.containsFile(TOSCA_META_PATH_FILE_NAME)) {
            ...
            return !hasOnapCsarEntry(metadata);
        }
        return false;
    }
     ...
     */
    public boolean hasOnapCsarEntry(final ToscaMetadata metadata) {
        final var ONAP_CSAR = "onap_csar";
        final var defaultCsarFormat = Optional.ofNullable(CommonConfigurationManager.getInstance()
