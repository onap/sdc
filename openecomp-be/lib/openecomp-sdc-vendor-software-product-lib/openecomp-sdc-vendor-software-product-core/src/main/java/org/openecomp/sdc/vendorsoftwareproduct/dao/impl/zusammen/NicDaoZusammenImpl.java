...
public class NicDaoZusammenImpl implements NicDao {
    ...
    private Collection<NicEntity> listNics(SessionContext context, ElementContext elementContext, NicEntity nic) {
        return zusammenAdaptor.listElementsByName(context, elementContext, new Id(nic.getComponentId()), ElementType.Nics.name()).stream()
            .map(new ElementToNicConvertor()::convert).map(nicEntity -> {
    ...
    public void create(NicEntity nic) {
        ...
        Element savedElement = zusammenAdaptor.saveElement(context, elementContext, componentElement, "Create nic");
        nic.setId(savedElement.getSubElements().iterator().next().getSubElements().iterator().next().getElementId().getValue());
    }
    ...
    public void update(NicEntity nic) {
        ...
        SessionContext context = createSessionContext();
        zusammenAdaptor.saveElement(context, new ElementContext(nic.getVspId(), nic.getVersion().getId()), nicElement,
    ...
    public NicEntity get(NicEntity nic) {
        ...
        Optional<Element> element = zusammenAdaptor.getElement(context, new ElementContext(nic.getVspId(), nic.getVersion().getId()), nic.getId());
    ...
    @Override
    public NicEntity getQuestionnaireData(String vspId, Version version, String componentId, String nicId) {
        SessionContext context = createSessionContext();
        return getQuestionnaire(context, new ElementContext(vspId, version.getId()), new NicEntity(vspId, version, componentId, nicId));
    }

    private NicEntity getQuestionnaire(SessionContext context, ElementContext elementContext, NicEntity nic) {
        Optional<Element> questionnaireElement = zusammenAdaptor
            .getElementByName(context, elementContext, new Id(nic.getId()), ElementType.NicQuestionnaire.name());
        return questionnaireElement.map(new ElementToNicQuestionnaireConvertor()::convert).map(entity -> {
            entity.setVspId(nic.getVspId());
            entity.setVersion(nic.getVersion());
            entity.setComponentId(nic.getComponentId());
            return entity;
    ...
    @Override
    public void updateQuestionnaireData(String vspId, Version version, String componentId, String nicId, String questionnaireData) {
        ZusammenElement questionnaireElement = nicQuestionnaireToZusammen(questionnaireData, Action.UPDATE);
        ...
        SessionContext context = createSessionContext();
        zusammenAdaptor.saveElement(context, new ElementContext(vspId, version.getId()), nicElement, "Update nic questionnaire");
    }
    ...
    public Collection<NicEntity> listByVsp(String vspId, Version version) {
        ...
        return components.stream()
            .map(component -> listNics(context, elementContext, new NicEntity(vspId, version, component.getId(), null)).stream().map(nic -> {
    ...
    public void deleteByComponentId(String vspId, Version version, String componentId) {
        SessionContext context = createSessionContext();
        ElementContext elementContext = new ElementContext(vspId, version.getId());
        Optional<Element> optionalElement = zusammenAdaptor.getElementByName(context, elementContext, new Id(componentId), ElementType.Nics.name());
        if (optionalElement.isPresent()) {
            ...
            nics.forEach(nic -> {
                ...
                zusammenAdaptor.saveElement(context, elementContext, nicZusammenElement, "Delete nic with id " + nic.getElementId());
            });
        }
    }
    ...
    private ZusammenElement nicToZusammen(NicEntity nic, Action action) {
        ...
        if (action == Action.CREATE) {
            nicElement.setSubElements(Collections.singletonList(nicQuestionnaireToZusammen(nic.getQuestionnaireData(), Action.CREATE)));
        }
        return nicElement;
    }

    private ZusammenElement nicQuestionnaireToZusammen(String questionnaireData, Action action) {
        ZusammenElement questionnaireElement = buildStructuralElement(ElementType.NicQuestionnaire, action);
    ...
    private ZusammenElement buildNicElement(NicEntity nic, Action action) {
        ZusammenElement nicElement = buildElement(nic.getId() == null ? null : new Id(nic.getId()), action);
        Info info = new Info();
