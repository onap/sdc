...
public class ImageDaoZusammenImpl implements ImageDao {
    ...
    public Collection<ImageEntity> list(ImageEntity image) {
        ...
        ElementContext elementContext = new ElementContext(image.getVspId(), image.getVersion().getId());
        return listImages(context, elementContext, image);
    }

    private Collection<ImageEntity> listImages(SessionContext context, ElementContext elementContext, ImageEntity image) {
        return zusammenAdaptor.listElementsByName(context, elementContext, new Id(image.getComponentId()), ElementType.Images.name()).stream()
            .map(new ElementToImageConvertor()::convert).map(imageEntity -> {
    ...
    public void create(ImageEntity image) {
        ...
        ZusammenElement imagesElement = buildStructuralElement(ElementType.Images, Action.IGNORE);
        ...
        SessionContext context = createSessionContext();
        ElementContext elementContext = new ElementContext(image.getVspId(), image.getVersion().getId());
        Element savedElement = zusammenAdaptor.saveElement(context, elementContext, componentElement, "Create image");
        image.setId(savedElement.getSubElements().iterator().next().getSubElements().iterator().next().getElementId().getValue());
    }
    ...
    public void update(ImageEntity image) {
        ...
        SessionContext context = createSessionContext();
        ElementContext elementContext = new ElementContext(image.getVspId(), image.getVersion().getId());
        zusammenAdaptor.saveElement(context, elementContext, imageElement, String.format("Update image with id %s", image.getId()));
    }
    ...
    public void delete(ImageEntity image) {
        ...
        SessionContext context = createSessionContext();
        ElementContext elementContext = new ElementContext(image.getVspId(), image.getVersion().getId());
        zusammenAdaptor.saveElement(context, elementContext, imageElement, String.format("Delete image with id %s", image.getId()));
    }
    ...
    public void deleteByVspId(String vspId, Version version) {
        ...
        Optional<Element> elementOptional = zusammenAdaptor.getElementByName(context, elementContext, null, ElementType.Images.name());
        if (elementOptional.isPresent()) {
            ...
            images.forEach(image -> {
                ...
                zusammenAdaptor.saveElement(context, elementContext, imageZusammenElement, "Delete image " + "with id " + image.getElementId());
            });
        }
    }
    ...
    public Collection<ImageEntity> listByVsp(String vspId, Version version) {
        ...
        return components.stream().map(
            component -> listImages(context, elementContext, new ImageEntity(vspId, version, component.getId(), null)).stream()
                .map(image -> getQuestionnaire(context, elementContext, image)).collect(Collectors.toList())).flatMap(Collection::stream)
            .collect(Collectors.toList());
    }

    @Override
    public ImageEntity getQuestionnaireData(String vspId, Version version, String componentId, String imageId) {
        SessionContext context = createSessionContext();
        ElementContext elementContext = new ElementContext(vspId, version.getId());
        return getQuestionnaire(context, elementContext, new ImageEntity(vspId, version, componentId, imageId));
    }

    private ImageEntity getQuestionnaire(SessionContext context, ElementContext elementContext, ImageEntity image) {
        Optional<Element> questionnaireElement = zusammenAdaptor
            .getElementByName(context, elementContext, new Id(image.getId()), ElementType.ImageQuestionnaire.name());
    ...
    @Override
    public void updateQuestionnaireData(String vspId, Version version, String componentId, String imageId, String questionnaireData) {
        ZusammenElement questionnaireElement = imageQuestionnaireToZusammen(questionnaireData, Action.UPDATE);
        ...
        SessionContext context = createSessionContext();
        ElementContext elementContext = new ElementContext(vspId, version.getId());
        zusammenAdaptor.saveElement(context, elementContext, imageElement, "Update image " + "questionnaire");
    }
    ...
    private ZusammenElement imageToZusammen(ImageEntity image, Action action) {
        ...
        if (action == Action.CREATE) {
            imageElement.setSubElements(Collections.singletonList(imageQuestionnaireToZusammen(image.getQuestionnaireData(), Action.CREATE)));
        }
        return imageElement;
    }

    private ZusammenElement imageQuestionnaireToZusammen(String questionnaireData, Action action) {
        ZusammenElement questionnaireElement = buildStructuralElement(ElementType.ImageQuestionnaire, action);
    ...
    private ZusammenElement buildImageElement(ImageEntity image, Action action) {
        ZusammenElement imageElement = buildElement(image.getId() == null ? null : new Id(image.getId()), action);
        Info info = new Info();
