...
public class ResourceTranslationContrailV2VlanSubInterfaceImpl extends ResourceTranslationBase {
    ...
    private void connectSubInterfaceToInterface(TranslateTo translateTo,
        ...
        if (((List) interfaceRefs).size() > 1) {
            logger.warn("Heat resource: '{}' with type: '{}' include '{}' property with more than one interface "
                    + "values, only the first interface will be connected, all rest will be ignored in TOSCA "
        ...
        if (interfaceResourceId.isPresent()) { // get_resource
            ...
            if (acceptableResourceTypes.contains(interfaceResource.getType())
                ...
                logger.warn("Heat resource: '{}' with type: '{}' include '{}' property which is connect to "
                                + "unsupported/incorrect {} resource '{}' with type '{}', therefore, this connection "
                                + "will be ignored in TOSCA translation.", translateTo.getResourceId(), translateTo
