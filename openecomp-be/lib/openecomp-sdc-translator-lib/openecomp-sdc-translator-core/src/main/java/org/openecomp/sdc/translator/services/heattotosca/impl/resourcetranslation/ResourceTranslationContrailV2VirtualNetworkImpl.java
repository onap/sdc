...
public class ResourceTranslationContrailV2VirtualNetworkImpl extends ResourceTranslationBase {
    ...
    private AttachedResourceId parseNetworkPolicyId(Object propertyValue, TranslateTo translateTo) {
        ...
        if (!translatedPolicyResourceId.isPresent()) {
            logger.warn("Heat resource: '{}' with type: '{}' property network_policy_refs is referenced to an "
    ...
    private String extractResourceId(Object propertyValue, TranslateTo translateTo) {
        ...
        logger.warn("Heat resource: '{}' with type: '{}' invalid format of property value, therefore this resource "
                + "will be ignored in TOSCA translation.", translateTo.getResourceId(),
    ...
    private boolean extractResourceIdFromGetAttrList(TranslateTo translateTo, List<Object> value) {
        if (value.size() == 2 && FQ_NAME.equals(value.get(1))) {
            if (value.get(0) instanceof String) {
                ...
                logger.warn("Heat resource: '{}' with type: '{}' has property with invalid format of "
                        + "'get_attr' function with 'fq_name' value, therefore this property will be "
                        + "ignored in TOSCA translation.", translateTo.getResourceId(),
                        translateTo.getResource().getType());
            }
        }
        return false;
    ...
    private String extractResourceIdFromGetResource(Map propertyValue, TranslateTo translateTo) {
        ...
        if (value instanceof String) {
            ...
            logger.warn("Heat resource: '{}' with type: '{}' has property with invalid format of "
                            + "'get_resource' function, therefore this property will be  ignored in TOSCA "
                            + "translation.", translateTo.getResourceId(), translateTo.getResource().getType());
        }
        return null;
    }
}
