...
public class ResourceTranslationContrailServiceInstanceImpl extends ResourceTranslationBase {
    ...
    public void translate(TranslateTo translateTo) {
        ...
        if (contrailServiceTemplateAttached.isGetResource()) {
            ...
            logger.warn("Heat resource: '{}' with type: '{} include 'service_template' property without "
                    + "'get_resource' function, currently not supported, therefore this resource will be ignored in "
                    + "TOSCA translation.", translateTo.getResourceId(), translateTo.getResource().getType());
        }
    }
    ...
    private void translateContrailServiceInstance(TranslateTo translateTo, Resource serviceInstanceResource,
        ...
        if (!contrailServiceTemplateResource.getType()
            ...
            logger.warn("resource id '{}' with type '{} has reference to resource '{}' with type '{}' in property"
                            + " service_template. Invalid type, resource type should be type of '{}', therefore "
                            + "this resource will be ignored in TOSCA translation.", translateTo
                            .getResourceId(), translateTo.getResource().getType(),
        ...
        if (!contrailServiceTemplateTranslatedId.isPresent()) {
            logger.warn("Resource id '{}' with type '{}' has reference to unsupported resource '{}' with type '{}' in"
                            + " property 'service_template', therefore this resource will be ignored in TOSCA "
                            + "translation.", translateTo.getResourceId(),
        ...
        if (substitutedNodeType.getRequirements() != null
            ...
            logger.warn("More than one ServiceInstance pointing to the same ServiceTemplate '{} ' with different "
                    + "number of interfaces., therefore this resource will be ignored in TOSCA translation.",
                    contrailServiceTemplateResourceId);
            return;
        }
    ...
    private void connectPortToNetwork(TranslateTo translateTo, Object interfacePropertyValue,
        ...
        if (networkResourceId.isPresent()) {
            ...
            if (validNetworksForConnections.contains(networkResource.getType())) {
                ...
                logger.warn("Heat resource {} with type {} has connection to invalid/not supported network resource, "
                        + "therefore, this connection will be ignored in the translation.", translateTo.getResourceId(),
                        translateTo.getResource().getType());
            }
            ...
            logger.warn("Heat resource: '{}' with type: '{}' include 'virtual_network' property with value '{}', the "
                    + "connection to this network wasn't found/not supported therefore this connection will be "
                    + "ignored in TOSCA translation for this property.", translateTo.getResourceId(),
