...
public class ResetOldVersion {
  ...
  public static void reset(SessionContext context, String oldVersion, String emptyOldVersion) {
    ...
    itemVersionMap.entrySet().forEach(entry -> updatElements(context, generalElementMap, entry));

    itemChangeRefMap.entrySet().forEach(entry -> updatElements(context, generalElementMap, entry));

    System.out.println("number of element updated:" + count);

  }

  private static void updatElements(SessionContext context, Map<String, ElementEntity> generalElementMap, Map.Entry<String, List<String>> entry) {
    entry.getValue().stream().filter
            (changeRef -> generalElementMap
                    .containsKey(context.getUser().getUserName() + "_" + entry.getKey()
                            + "_" + changeRef)).
            forEach(changeref -> ElementHandler.update(context,entry.getKey(), changeref, changeref, generalElementMap.
                    get(String.format("%s_%s_%s",context.getUser().getUserName() , entry.getKey() ,changeref))));
