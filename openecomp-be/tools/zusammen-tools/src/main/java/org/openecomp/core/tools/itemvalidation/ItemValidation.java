...
public class ItemValidation {
    ...
    private void accept(VersionElementsEntity versionElementsEntity) {
        printMessage(logger, String
            .format("Found VERSION_ELEMENTS entry. Space: %s, ItemID: %s, VersionId: %s, Revision: %s", versionElementsEntity.getSpace(),
                versionElementsEntity.getItemId(), versionElementsEntity.getVersionId(), versionElementsEntity.getRevisionId()));
        subElementsList.clear();
        if (versionElementsEntity.getElementIds().isEmpty()) {
            validationMessage.add(String.format("Empty field VERSION_ELEMENT.element_ids. Space: %s, ItemID: %s, VersionId: %s, Revision: %s",
                versionElementsEntity.getSpace(), versionElementsEntity.getItemId(), versionElementsEntity.getVersionId(),
                versionElementsEntity.getRevisionId()));
        } else {
            //loop over element_ids stored in version_elements
            versionElementsEntity.getElementIds().forEach(
                (key, value) -> validateElement(versionElementsEntity.getSpace(), versionElementsEntity.getItemId(),
                    versionElementsEntity.getVersionId(), key, value));
            ...
            subElementsList.forEach((String key) -> {
                if (!versionElementsEntity.getElementIds().containsKey(key)) {
                    validationMessage.add(String.format("Element is defined in table ELEMENT but not found in VERSION_ELEMENTS.element_ids."
                            + "  Space:%s, ItemID:%s, VersionID:%s, Version RevisionID:%s, ElementID:%s", versionElementsEntity.getSpace(),
                        versionElementsEntity.getItemId(), versionElementsEntity.getVersionId(), versionElementsEntity.getRevisionId(), key));
                }
            });
        }
    }
