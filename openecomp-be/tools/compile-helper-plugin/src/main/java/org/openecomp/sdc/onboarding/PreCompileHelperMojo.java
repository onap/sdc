...
public class PreCompileHelperMojo extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        byte[] sourceChecksum = new byte[0];
        ...
        try {
            sourceChecksum = getSourceChecksum(mainChecksum + COLON + resourceChecksum, SHA1).getBytes();
        ...
        boolean isFirstBuild = buildState.getBuildTime(moduleCoordinates) == 0 || artifactPath == null;
        ...
        boolean buildDataSameWithPreviousBuild =
                lastTimeModuleBuildData != null && (lastTimeModuleBuildData.get(MAIN) != null && moduleBuildData
                                                                                                         .get(MAIN)
                                                                                                         .equals(lastTimeModuleBuildData
                                                                                                                         .get(MAIN)));
        boolean resourceMainBuildDataSameWithPreviousBuild =
                lastTimeResourceBuildData != null && (lastTimeResourceBuildData.get(MAIN) != null && resourceBuildData
                                                                                                             .get(MAIN)
        ...
        if (!mainToBeCompiled && !instrumented && JAR.equals(project.getPackaging())
                    && resourceMainBuildDataSameWithPreviousBuild) {
            project.getProperties().setProperty("artifactPathToCopy", artifactPath);
        }
    }
    ...
    private static Map<String, String> readCurrentPMDState(String fileName) {
        Map<String, String> val = readState(fileName, HashMap.class);
