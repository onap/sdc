...
public class CnfPackageValidator {
    ...
    private List<String> createErrorMessages(Stats stats) {
        ...
        if (stats.without > 0) {
            messages.add("Definition of 'isBase' is missing in " + stats.without + " charts.");
        }

        if (stats.base == 0) {
            messages.add("None of charts is marked as 'isBase'.");
        } else if (stats.base > 1) {
            messages.add("More than one chart is marked as 'isBase'.");
        }
