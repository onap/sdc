...
public class SecurityManager {
    ...
    public boolean verifySignedData(final byte[] messageSyntaxSignature, final byte[] packageCert,
        ...
        try (ByteArrayInputStream signatureStream = new ByteArrayInputStream(messageSyntaxSignature);
            ...
            if (packageCert == null) {
                cert = readCertsFromCms(signedData, firstSigner);
            } else {
                cert = readCertsFromPackage(packageCert, firstSigner);
            }
    ...
    private X509Certificate readCertsFromCms(final CMSSignedData signedData, final SignerInformation firstSigner)
        ...
        for (X509CertificateHolder crt : allCerts) {
            if (firstSigner.getSID().match(crt)) {
                cert = loadCertificate(crt.getEncoded());
            } else {
        ...
        if (cert == null) {
            throw new SecurityManagerException(
                    "No matching certificate found in cms signature that should contain one!");
        }
        return cert;
    }

    private X509Certificate readCertsFromPackage(final byte[] packageCert, final SignerInformation firstSigner)
            throws IOException, SecurityManagerException {
        ...
        while ((object = pemParser.readObject()) != null) {
            if (object instanceof X509CertificateHolder) {
                if (firstSigner.getSID().match(object)) {
                    cert = loadCertificate((((X509CertificateHolder) object).getEncoded()));
                } else {
                    X509Certificate additionalCert = loadCertificate((((X509CertificateHolder) object).getEncoded()));
                    if (!isSelfSigned(additionalCert))
                        trustedCertificatesFromPackage.add(additionalCert);
                }
            }
