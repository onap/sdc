...
public class SecurityManager {
    ...
    public boolean verifySignedData(final byte[] messageSyntaxSignature, final byte[] packageCert,
        ...
        try (ByteArrayInputStream signatureStream = new ByteArrayInputStream(messageSyntaxSignature);
            ...
            if (packageCert == null) {
                ...
                Set<X509CertificateHolder> certs = parseCertsFromPem(packageCert);
                cert = readSignCert(certs, firstSigner);
                if (cert == null) {
                    throw new SecurityManagerException(
    ...
    private X509Certificate readSignCert(final Collection<X509CertificateHolder> certs, final SignerInformation firstSigner) {
        return certs.stream()
    ...
    private X509Certificate loadCertificate(byte[] certFile) throws SecurityManagerException {
        return loadCertificateFactory(new ByteArrayInputStream(certFile));
    }
