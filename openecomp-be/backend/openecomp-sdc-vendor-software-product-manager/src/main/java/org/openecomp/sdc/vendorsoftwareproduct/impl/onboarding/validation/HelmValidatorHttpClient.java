...
public class HelmValidatorHttpClient {
    ...
    public static final String VERSION_DESIRED = "versionDesired";
    ...
    public HttpResponse<String> execute(String packageFileName, HelmValidatorConfig validatorConfig) {
        LOGGER.info("Trying to execute Helm chart validation. File name: {}", packageFileName);
        final Properties headers = null;
        try {
            File helmChartFile = new File(packageFileName);
            HttpEntity entity = MultipartEntityBuilder.create()
                .addBinaryBody(FILE, helmChartFile, ContentType.DEFAULT_BINARY, helmChartFile.getName())
            ...
            HttpResponse<String> httpResponse = httpRequestHandler.post(validatorConfig.getValidationEndpoint(),
                null, entity, new HttpClientConfig(new Timeouts(TIMEOUT_MS, TIMEOUT_MS)));
            LOGGER.info("Received response from Helm chart validator: {}", httpResponse);

            return httpResponse;
        } catch (Exception e) {
            LOGGER.info("Exception during call to Helm validator {}", e);
        }
        return null;
    }
