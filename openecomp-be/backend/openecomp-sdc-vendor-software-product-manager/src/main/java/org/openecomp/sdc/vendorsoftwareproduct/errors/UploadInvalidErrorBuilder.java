...
public class UploadInvalidErrorBuilder extends BaseErrorBuilder {
    ...
    private static final String UPLOAD_INVALID_DETAILED_MSG = "File uploaded for vendor software product with Id %s and version %s is invalid: %s";
     ...
     */
    public UploadInvalidErrorBuilder(String vendorSoftwareProductId, Version version, Map<String, List<ErrorMessage>> errors) {
        ...
        getErrorCodeBuilder().withMessage(String.format(UPLOAD_INVALID_DETAILED_MSG, vendorSoftwareProductId, version.toString(), toString(errors)));
    }
    ...
    private String toString(Map<String, List<ErrorMessage>> errors) {
        ...
        errors.entrySet().stream().forEach(entry -> singleErrorToString(sb, entry.getKey(), entry.getValue()));
        return sb.toString();
    }
    ...
    private void singleErrorToString(StringBuilder sb, String fileName, List<ErrorMessage> errors) {
        ...
        sb.append(fileName);
        sb.append(sb.append(": "));
        errors.stream().forEach(error -> sb.append(error.getMessage()).append("[").append(error.getLevel()).append("], "));
    }
}
