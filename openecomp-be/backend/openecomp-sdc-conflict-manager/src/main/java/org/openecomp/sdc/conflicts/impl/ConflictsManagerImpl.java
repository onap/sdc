...
public class ConflictsManagerImpl implements ConflictsManager {
    ...
    public boolean isConflicted(String itemId, Version version) {
        Optional<ItemMergeHandler> itemMergeHandler = ItemMergeHandlerFactory.getInstance().createInterface(itemId);
        return conflictsDao.isConflicted(itemId, version) || (itemMergeHandler.isPresent() && itemMergeHandler.get().isConflicted(itemId, version));
    }
    ...
    public ItemVersionConflict getConflict(String itemId, Version version) {
        ...
        ItemMergeHandlerFactory.getInstance().createInterface(itemId)
            .ifPresent(itemMergeHandler -> itemMergeHandler.postListConflicts(itemId, version, conflicts));
        return conflicts;
    }

    @Override
    public void finalizeMerge(String itemId, Version version) {
        ItemMergeHandlerFactory.getInstance().createInterface(itemId).ifPresent(mergeHandler -> mergeHandler.finalizeMerge(itemId, version));
    }
    ...
    public Conflict getConflict(String itemId, Version version, String conflictId) {
        ...
        if (itemMergeHandler.isPresent()) {
            Optional<Conflict> conflict = itemMergeHandler.get().getConflict(itemId, version, conflictId);
            if (conflict.isPresent()) {
                return conflict.get();
            }
        }
        Conflict conflict = conflictsDao.getConflict(itemId, version, conflictId);
        itemMergeHandler.ifPresent(mergeHandler -> mergeHandler.postGetConflict(itemId, version, conflict));
        if (conflict == null) {
    ...
    @Override
    public void resolveConflict(String itemId, Version version, String conflictId, ConflictResolution resolution) {
        if (Resolution.OTHER.equals(resolution.getResolution())) {
            throw new UnsupportedOperationException("Resolution other than 'THEIRS' or 'YOURS' is not supported.");
        }
        Optional<ItemMergeHandler> itemMergeHandler = ItemMergeHandlerFactory.getInstance().createInterface(itemId);
        if (!itemMergeHandler.isPresent() || !itemMergeHandler.get().resolveConflict(itemId, version, conflictId, resolution)) {
            getConflict(itemId, version, conflictId); // validate that the conflict exist
            itemMergeHandler.ifPresent(mergeHandler -> mergeHandler.preResolveConflict(itemId, version, conflictId, resolution));
