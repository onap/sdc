...
import org.openecomp.sdc.ci.tests.validation.VfValidation;
...
public class OnboardingUiUtils {
    ...
    private static void importUpdateVSP(VendorSoftwareProductObject vsp, boolean isUpdate, boolean restore) {
        String vspName = vsp.getName();
        HomePage.searchForVSP(vspName);
    ...
    private static void onboardedVnfMetadataVerification(VendorSoftwareProductObject vsp, boolean isUpdate) {
        if (isUpdate) {
            VfValidation.verifyOnboardedVnfMetadataAfterUpdateVNF(vsp.getName(), vsp);
        } else {
            VfValidation.verifyOnboardedVnfMetadata(vsp.getName(), vsp);
        }
    }
    ...
    public static void doCheckOut() {
        String lifeCycleState = GeneralPageElements.getLifeCycleState();
        ...
        if (needCheckout) {
            try {
                ...
                Assert.assertEquals(LifeCycleStateEnum.CHECKOUT.getValue(), GeneralPageElements.getLifeCycleState(),
                    "Did not succeed to checkout");
            } catch (Exception e) {
    ...
    public static void importVSP(VendorSoftwareProductObject vsp) {
