...
public class PropertiesUtilsTest {
    ...
    public void testProxyInstanceGetPropertiesUndeclaredPropertyWithValue(){
        ...
        List<PropertyDefinition> propertyDefinitions = Arrays.asList(buildPropertyDefinition("undeclaredProperty2"),
                buildPropertyDefinition("undeclaredProperty1", undeclaredPropertyValue));
        when(service.getProperties()).thenReturn(propertyDefinitions);
        when(service.getInputs()).thenReturn(null);
        final List<PropertyDefinition> properties = PropertiesUtils.getProperties(service);
        assertEquals(2, properties.size());
        assertNull(properties.get(0).getValue());
        assertEquals(properties.get(1).getValue(), undeclaredPropertyValue);
    }
    ...
    public void testProxyInstanceGetPropertiesResolvePropertyValueFromInput(){
        ...
        List<PropertyDefinition> propertyDefinitions = Arrays.asList(buildPropertyDefinitionForDeclaredProperty(
                declaredPropertyName, mappedInputName), buildPropertyDefinition("undeclaredProperty"));
        when(service.getProperties()).thenReturn(propertyDefinitions);
        ...
        final List<PropertyDefinition> properties = PropertiesUtils.getProperties(service);
        assertEquals(3, properties.size());
        assertEquals(properties.get(1).getValue(), inputValue);
        //Undeclared property value
        assertNull(properties.get(2).getValue());
    }
