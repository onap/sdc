...
public class DataTypeBusinessLogicTest {
    ...
    public void test_getPrivateDataType() throws Exception {
        ...
        Either<DataTypeDefinition, StorageOperationStatus> result = testInstance.getPrivateDataType(COMPONENT_ID, DATATYPE_NAME);
        assertTrue(result.isLeft());
    ...
    public void test_deletePrivateDataType1() throws Exception {
        when(toscaOperationFacadeMock.getToscaElement(eq(COMPONENT_ID), Mockito.any(ComponentParametersView.class))).thenReturn(Either.left(service));
        when(toscaOperationFacadeMock.deleteDataTypeOfComponent(service, DATATYPE_NAME)).thenReturn(StorageOperationStatus.OK);

        Either<DataTypeDefinition, StorageOperationStatus> result = testInstance.deletePrivateDataType(COMPONENT_ID, DATATYPE_NAME);
        assertTrue(result.isLeft());
        DataTypeDefinition dataType = result.left().value();
        assertEquals(service.getDataTypes().get(0), dataType);
    }
    ...
    public void test_deletePrivateDataType2() throws Exception {
        ...
        Either<DataTypeDefinition, StorageOperationStatus> result = testInstance.deletePrivateDataType(service, DATATYPE_NAME);
        assertTrue(result.isLeft());
        DataTypeDefinition dataType = result.left().value();
        assertEquals(service.getDataTypes().get(0), dataType);
    }
