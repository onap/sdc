...
public class ResourceBusinessLogicTest {
    ...
    ArtifactsBusinessLogic artifactManager = new ArtifactsBusinessLogic(artifactCassandraDao, toscaExportHandler,
        csarUtils, lifecycleBl,
    ...
    protected ComponentDescriptionValidator componentDescriptionValidator = new ComponentDescriptionValidator(
        componentsUtils);
    protected ComponentProjectCodeValidator componentProjectCodeValidator = new ComponentProjectCodeValidator(
        componentsUtils);
    protected ComponentIconValidator componentIconValidator = new ComponentIconValidator(componentsUtils);
    protected ComponentContactIdValidator componentContactIdValidator = new ComponentContactIdValidator(
        componentsUtils);
    protected ComponentTagsValidator componentTagsValidator = new ComponentTagsValidator(componentsUtils);
    protected ComponentNameValidator componentNameValidator = new ComponentNameValidator(componentsUtils,
    ...
    public void setup() {
        ...
        when(toscaOperationFacade
            .validateComponentNameExists("tosca.nodes.Root", ResourceTypeEnum.VFC, ComponentTypeEnum.RESOURCE))
        ...
        when(toscaOperationFacade.validateToscaResourceNameExists("tosca.nodes.Root"))
            .thenReturn(validateDerivedExists);
    ...
    public void handleNodeTypeArtifactsTest() {
        ...
        assertTrue(
            bl.handleNodeTypeArtifacts(resourceResponse, nodeTypeArtifactsToHandle, new ArrayList<>(), user, true, true)
                .isRight());
    }

    @Test
    public void getUiComponentDataTransferByComponentIdTest() {
        when(toscaOperationFacade.getToscaElement(eq(""), Mockito.any(ComponentParametersView.class)))
    ...
    public void shouldUpgradeToLatestDerivedTest() {
        ...
        when(toscaOperationFacade.shouldUpgradeToLatestDerived(genericVFC))
            .thenReturn(Either.right(StorageOperationStatus.OK));
    ...
    public void testUpdateUnhappyScenario() {
        ...
        when(toscaOperationFacade.getToscaElement(resource.getUniqueId()))
            .thenReturn(Either.left(setCanWorkOnResource(resource)));
        ...
        try {
            Resource createdResource = bl
                .validateAndUpdateResourceFromCsar(resource, user, null, "", resource.getUniqueId());
    ...
    public void testDeleteMarkedResources() {
        ...
        when(toscaOperationFacade.getAllComponentsMarkedForDeletion(ComponentTypeEnum.RESOURCE))
            .thenReturn(eitherNoResources);
        ...
        when(toscaOperationFacade.deleteMarkedElements(ComponentTypeEnum.RESOURCE))
            .thenReturn(Either.left(deletedComponents));
