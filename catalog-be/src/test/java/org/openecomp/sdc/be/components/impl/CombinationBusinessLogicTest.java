...
public class CombinationBusinessLogicTest {
    ...
    private String COMBINATION_UNIQUE_ID;
    ...
    public void testCreateCombinationFromServiceNoComponents() {
        ...
        when(mockTitanDao.getVertexById(COMBINATION_UNIQUE_ID)).thenReturn(getVertexEither);
        UiCombination combination = new UiCombination();
        combination.setDescription("Hello Word");
        combination.setName("Isaaccomb");
        combination.setUniqueId("Combination5");

        Either<Combination, ResponseFormat> createResponse = bl
                .createCombination(combination, serviceNoCompInstances.getUniqueId(), user.getUserId() );

        // expect an invalid response 400 code
        assertEquals(true, createResponse.isRight());
        assertEquals(new Integer(500), createResponse.right().value().getStatus());
    }
    ...
    public void testCreateCombinationFromServiceWithComponents() {
        ...
        Combination combinationNoCompInstances = createCombinationObject(false);
        Combination combinationWithCompInstances = combinationObjectforcreate(true);
        ...
        when(serviceBusinessLogic.getService(anyString(),anyObject())).thenReturn(Either.left(serviceWithCompInstances));
        UiCombination combination = new UiCombination();
        combination.setDescription("My short description");
        combination.setName("My-Combination_Name");
        combination.setUniqueId("Combination5");

        Either<Combination, ResponseFormat> createResponse = bl
                .createCombination(combination, serviceWithCompInstances.getUniqueId(), user.getUserId());
    ...
    public void testCreateCombinationInstanceFromService() {
        ...
        //when(combinationOperation.getCombination(anyString())).thenReturn(Either.left(combinationJson));
    ...
    private Combination combinationObjectforcreate(boolean addComponentInstances) {
        Combination combination = new Combination();
        combination.setName(COMBINATION_NAME);
        combination.setUniqueId(COMBINATION_UNIQUE_ID);
        combination.setDesc("My short description");

        ComponentInstance compinstance = new ComponentInstance();
        compinstance.setPosX("10");
        compinstance.setPosY("15");
        compinstance.setUniqueId("e4cc9f41-919b-46d8-9849-123acf994348.combination");
        compinstance.setComponentUid("");
        componentInstances.add(compinstance);
        RequirementCapabilityRelDef relationship = new RequirementCapabilityRelDef();
        componentInstancesRelations.add(relationship);
        ComponentInstanceProperty attr = new ComponentInstanceProperty();
        List<ComponentInstanceProperty> attrlist = new ArrayList<>();
        attrlist.add(attr);
        componentInstancesAttributes.put("ATTR1", attrlist);
        componentInstancesProperties.put("PROP1",new ArrayList<>());
        componentInstancesInputs.put("INPUTS",new ArrayList<>());
    ...
    private Service createServiceObject(boolean addComponentInstances) {
        ...
        COMBINATION_UNIQUE_ID = service.getUniqueId() + ".combination";
