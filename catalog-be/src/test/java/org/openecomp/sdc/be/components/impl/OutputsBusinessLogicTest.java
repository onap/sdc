...
class OutputsBusinessLogicTest {
    ...
    private OutputDefinition setUpListOutput() {
        ...
        SchemaDefinition listOutputSchema = new SchemaDefinition();
        listOutputSchema.setProperty(new PropertyDataDefinitionBuilder()
            .setType(LISTOUTPUT_SCHEMA_TYPE)
            .setIsRequired(false)
            .build()
        );
        listOutput.setSchema(listOutputSchema);
        return listOutput;
    }
