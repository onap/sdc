 ...
 *
 * In contrast with `Either`, `Try` relies on exception handling and is designed to encapsulate computations resulting in an exception. The advantage compared to `Either` is that the indication of the line of code throwing the exception is preserved, which eases debugging while minimizing the stack trace. On the other it also provides a higher level of composition, so it is pretty much the best of both worlds:
 * ```
 * String unsafe() throws RuntimeException { ... }
 * Try<String> safe = Try.make(() -> unsafe());
 * ```
 * In order to get the value result of a `Try`,one has to call `get`. `get` takes two functions,one that is called in case of success,one that is called in case of errors:
 * ```
 * Try<String> safe= Try.make(() -> unsafe());
 * safe.consume(s -> System.out.println(s), exception -> { throw exception; });
 * ```
 * A safer way is to provide an exception handler:
 * ```
 * safe.get(s -> s), exception-> {
...
public final class Try<A> {
     ...
     */
    public void consume(Consumer<A> onSuccess, Consumer<Exception> onFailure) {
        try {
