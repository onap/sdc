...
public class FileAlterationListenerComponent {
    ...
    protected void runListener() {
            ...
            public void onFileChange(final File file) {
                ...
                if (file.getAbsolutePath().endsWith(keystoreLocation)) {
                    try {
                        ...
                        swappableSslService.updateKeyManager(KeyManagerUtils.createKeyManager(keystore, passCharArray));
                    ...
                    try {
                        ...
                        swappableSslService.updateTrustManager(TrustManagerUtils.createTrustManager(truststore));
        ...
        final String jettyBase = sslConfig.getJettyBase();
        Stream.of(buildPathToWatch(jettyBase, keystoreLocation), buildPathToWatch(jettyBase, truststoreLocation))
    ...
    private Path buildPathToWatch(final String jettyBase, final String location) {
        if (location.startsWith("/")) {
            return Path.of(location).getParent();
        } else {
            return Path.of(jettyBase, location).getParent();
        }
    }
