...
public class TypesUploadServlet extends AbstractValidationsServlet {
    ...
    private <T extends ToscaDataDefinition> void buildStatusForUniqueElementTypeCreate(Wrapper<Response> responseWrapper,
            ...
            try {
                ...
                if (createdDataType != null) {
                    if (Boolean.TRUE.equals(createdDataType.right)) {
                        status = ActionStatus.CREATED;
                        final Object representation = RepresentationUtils.toRepresentation(DataTypeDefinitionDtoMapper.mapFrom((DataTypeDataDefinition)createdDataType.left));
                        responseWrapper.setInnerElement(buildOkResponse(getComponentsUtils().getResponseFormat(status), representation));
                    }
                    else {
                        responseWrapper.setInnerElement(buildErrorResponse(getComponentsUtils().getResponseFormat(ActionStatus.DATA_TYPE_ALREADY_EXIST)));
                    }
                }
