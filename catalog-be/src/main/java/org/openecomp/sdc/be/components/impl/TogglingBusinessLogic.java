...
public class TogglingBusinessLogic extends BaseBusinessLogic {
    ...
    public Map<String, Boolean> getAllFeatureStates() {
        return ToggleableFeature.safeGetValues().stream().collect(Collectors.toMap(Enum::name,
                ToggleableFeature::isActive));
    }

    public boolean getFeatureState(String featureName) {
        ToggleableFeature feature = ToggleableFeature.valueOf(featureName);
        if(Objects.isNull(feature)) {
            throw new RuntimeException(String.format("No feature named %s was found", featureName));
        }
