...
public class ArtifactTypeImportManager {
    ...
    private static final Logger log = Logger.getLogger(ArtifactTypeImportManager.class.getName());
    @Resource
    ...
    public Either<List<ArtifactTypeDefinition>, ResponseFormat> createArtifactTypes(final String artifactTypesYml,
            final String modelName, final boolean includeToModelDefaultImports) {
        Either<List<ArtifactTypeDefinition>, ActionStatus> artifactTypes =
                createArtifactTypeFromYml(artifactTypesYml, modelName);
        if (artifactTypes.isRight()) {
            return Either.right(componentsUtils.getResponseFormat(artifactTypes.right().value(), null));
        }
        final List<ArtifactTypeDefinition> elementTypes =
                createArtifactTypesByDao(artifactTypes.left().value());
    ...
    private Either<List<ArtifactTypeDefinition>, ActionStatus> createArtifactTypeFromYml(
        ...
        if (artifactTypes.isLeft() && StringUtils.isNotEmpty(modelName)) {
