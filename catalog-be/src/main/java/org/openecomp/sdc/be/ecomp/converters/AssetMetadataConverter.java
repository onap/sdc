...
public class AssetMetadataConverter {
    ...
    static final Logger log = Logger.getLogger(DistributionCatalogServlet.class);
     ...
     */
    public Either<List<? extends AssetMetadata>, ResponseFormat> convertToAssetMetadata(
        List<? extends Component> componentList, String serverBaseURL, boolean detailed) {
        ...
        for (Component curr : componentList) {
            Either<? extends AssetMetadata, ResponseFormat> resMetaData = convertToSingleAssetMetadata(curr,
                serverBaseURL, detailed);
    ...
    public <T extends Component> Either<? extends AssetMetadata, ResponseFormat> convertToSingleAssetMetadata(
        T component, String serverBaseURL, boolean detailed) {
        ComponentTypeEnum componentType = component.getComponentType();
        Either<? extends AssetMetadata, ResponseFormat> resMetaData = convertToMetadata(componentType, serverBaseURL,
            detailed, component);
    ...
    Either<? extends AssetMetadata, ResponseFormat> convertToMetadata(ComponentTypeEnum componentType,
        String serverBaseURL, boolean detailed, Component curr) {
        ...
        switch (componentType) {
            ...
            default:
                ...
                ResponseFormat responseFormat = componentsUtils
                    .getResponseFormatAdditionalProperty(ActionStatus.COMPONENT_INVALID_CATEGORY);
                return Either.right(responseFormat);
        }
    }

    Either<? extends AssetMetadata, ResponseFormat> generateResourceMeatdata(String serverBaseURL, boolean detailed,
        Component curr) {
        AssetMetadata metaData;
        metaData = createMetadaObject(detailed, curr.getComponentType());
        metaData = convertToResourceMetadata((ResourceAssetMetadata) metaData, (Resource) curr, serverBaseURL,
            detailed);
        ...
        if (detailed) {
            ...
            if (converted.isRight()) {
                ActionStatus storageResponse = componentsUtils
                    .convertFromStorageResponse(converted.right().value(), ComponentTypeEnum.RESOURCE);
    ...
    AssetMetadata createMetadaObject(boolean detailed, ComponentTypeEnum type) {
    ...
    Either<? extends AssetMetadata, ResponseFormat> generateServiceMetadata(String serverBaseURL, boolean detailed,
        Component curr) {
        AssetMetadata metaData = createMetadaObject(detailed, curr.getComponentType());

        metaData = convertToServiceAssetMetadata((ServiceAssetMetadata) metaData, (Service) curr, serverBaseURL,
            detailed);
    ...
    <U extends AssetMetadata, T extends Component> U convertToAsset(U asset, T component, String serverBaseURL,
        boolean detailed) {
    ...
    <T extends ResourceAssetMetadata> T convertToResourceMetadata(T assetToPopulate, Resource resource,
    ...
    <T extends ServiceAssetMetadata> T convertToServiceAssetMetadata(T assetToPopulate, Service service,
        String serverBaseURL, boolean detailed) {
    ...
    <T extends ResourceAssetDetailedMetadata> Either<T, StorageOperationStatus> convertToResourceDetailedMetadata(
        T assetToPopulate, Resource resource, String serverBaseURL) {
    ...
    <T extends ServiceAssetDetailedMetadata> Either<T, StorageOperationStatus> convertToServiceDetailedMetadata(
        T assetToPopulate, Service service) {
    ...
    <T extends ResourceAssetDetailedMetadata> T populateResourceWithArtifacts(T asset, Resource resource,
    ...
    <T extends ServiceAssetDetailedMetadata> T populateServiceWithArtifacts(T asset, Service service,
        Map<String, ArtifactDefinition> artifacts) {
    ...
    List<ArtifactMetadata> populateAssetWithArtifacts(Component component, Map<String, ArtifactDefinition> artifacts) {
    ...
    ArtifactMetadata convertToArtifactMetadata(ArtifactDefinition artifact, String componentType, String componentUUID,
        ...
        if (resourceInstanceName == null || resourceInstanceName.isEmpty()) {
            metadata.setArtifactURL(
                String.format(COMPONENT_ARTIFACT_URL, componentType, componentUUID, artifact.getArtifactUUID()));
        } else {
            metadata.setArtifactURL(String
                .format(RESOURCE_INSTANCE_ARTIFACT_URL, componentType, componentUUID, resourceInstanceName,
                    artifact.getArtifactUUID()));
        }

        metadata.setArtifactDescription(artifact.getDescription());
        metadata.setArtifactTimeout(
            artifact.getTimeout() != null && artifact.getTimeout() > 0 ? artifact.getTimeout() : null);
    ...
    Either<List<ResourceInstanceMetadata>, StorageOperationStatus> convertToResourceInstanceMetadata(
        ...
        for (ComponentInstance componentInstance : componentInstances) {
            ...
            if (!uuidDuplicatesMap.containsKey(componentUid)) {
                ...
                if (eitherResource.isRight()) {
                    log.debug("convertToResourceInstanceMetadata: Failed getting resource with Uid: {}",
            ...
            if (MapUtils.isNotEmpty(componentInstance.getDeploymentArtifacts())) {
                ...
                for (ArtifactDefinition artifactDefinition : values) {
                    ArtifactMetadata converted = convertToArtifactMetadata(artifactDefinition, componentType,
                        componentUUID, componentInstance.getNormalizedName());
