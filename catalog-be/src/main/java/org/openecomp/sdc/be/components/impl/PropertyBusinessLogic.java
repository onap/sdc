...
public class PropertyBusinessLogic extends BaseBusinessLogic {
    ...
    public void createPropertiesInGraph(Map<String, PropertyDefinition> properties, org.openecomp.sdc.be.model.Component component) {
        ...
        if (!propertiesAfterValidation.isEmpty()) {
            StorageOperationStatus associatePropertiesEither = toscaOperationFacade
                .createAndAssociateProperties(propertiesAfterValidation, component.getUniqueId());
            if (associatePropertiesEither != StorageOperationStatus.OK) {
                throw new ComponentException(componentsUtils
                    .getResponseFormat(componentsUtils.convertFromStorageResponse(associatePropertiesEither)));
            }
        }
    }
    ...
    private Map<String, PropertyDefinition> validateAndMergeProperties(Map<String, PropertyDefinition> properties, Component component) {
        ...
        if (resourceProperties != null && CollectionUtils.isNotEmpty(resourceProperties)) {
    ...
    private Either<PropertyDefinition, ResponseFormat> prepareAndValidatePropertyBeforeCreate(PropertyDefinition newPropertyDefinitionDefinition,
        ...
        if (type != null) {
            ...
            if (newPropertyDefinitionDefinition.getDefaultValue() != null) {
                final String convertedValue = converter.convert(newPropertyDefinitionDefinition.getDefaultValue(), innerType, dataTypes);
