...
public class YamlTemplateParsingHandler {
    ...
    private Map<String, PolicyDefinition> getPolicies(String fileName, Map<String, Object> toscaJson) {
        ...
        if (MapUtils.isNotEmpty(foundPolicies)) {
            Map<String, PolicyDefinition> policies = foundPolicies
                                                          .entrySet()
                                                          .stream()
    ...
    private List<PropertyDataDefinition> validateFillPolicyProperties(PolicyTypeDefinition policyTypeDefinition,  Map<String, Object> policyTemplateJsonMap) {
        ...
        if (propertiesMap != null && !propertiesMap.isEmpty() && policyTypeDefinition.getProperties() != null
                    && !policyTypeDefinition.getProperties().isEmpty()) {
            propertyDataDefinitionList = policyTypeDefinition.getProperties().stream().map(propertyDefinition -> {
                PropertyDataDefinition newPropertyDef = new PropertyDataDefinition(propertyDefinition);
                String propertyName = newPropertyDef.getName();
                if (propertiesMap.containsKey(propertyName) && propertiesMap.get(propertyName) != null) {
                    Object propValue = propertiesMap.get(propertyName);
                    newPropertyDef
                            .setValue(CharMatcher.is('\"').and(CharMatcher.is('\'')).trimFrom(gson.toJson(propValue)));
                }
