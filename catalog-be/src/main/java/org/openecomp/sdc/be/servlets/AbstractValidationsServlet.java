...
public abstract class AbstractValidationsServlet extends BeGenericServlet {
    ...
    private static final List<String> TOSCA_DEFINITION_VERSIONS = Arrays
            .asList(TOSCA_SIMPLE_YAML_PREFIX + "1_0_0", TOSCA_SIMPLE_YAML_PREFIX + "1_1_0",
                    "tosca_simple_profile_for_nfv_1_0_0",
                    TOSCA_SIMPLE_YAML_PREFIX + "1_0", TOSCA_SIMPLE_YAML_PREFIX + "1_1",
                    TOSCA_SIMPLE_YAML_PREFIX + "1_2", TOSCA_SIMPLE_YAML_PREFIX + "1_3");
    ...
    protected void validateZip(final Wrapper<Response> responseWrapper, final File zipFile, final String payloadName) {
        if (StringUtils.isEmpty(payloadName)) {
            ...
            final Response errorResponse = buildErrorResponse(
                    getComponentsUtils().getResponseFormat(ActionStatus.INVALID_CONTENT));
            responseWrapper.setInnerElement(errorResponse);
            return;
        }
    ...
    protected void validateCsar(final Wrapper<Response> responseWrapper, final File csarFile,
    ...
    protected void fillPayloadDataFromFile(Wrapper<Response> responseWrapper,
            UploadResourceInfo uploadResourceInfoWrapper, File file) {
    ...
    protected void validateUserRole(Wrapper<Response> errorResponseWrapper, User user,
            ResourceAuthorityTypeEnum resourceAuthority) {
        log.debug(VALIDATE_USER_ROLE);
    ...
    protected void validateAndFillResourceJson(Wrapper<Response> responseWrapper,
            Wrapper<UploadResourceInfo> uploadResourceInfoWrapper, User user,
        ...
        try {
            ...
            if (resourceInfoObject == null) {
                ...
                if (!resourceAuthorityEnum.isBackEndImport()) {
                    isValid = resourceInfoObject.getPayloadName() != null
                            && !resourceInfoObject.getPayloadName().isEmpty();
    ...
    protected void validateAuthorityType(Wrapper<Response> responseWrapper, String authorityType) {
        ...
        if (authorityTypeEnum == null) {
            ...
            Response errorResp = buildErrorResponse(
                    getComponentsUtils().getResponseFormat(ActionStatus.INVALID_CONTENT));
            responseWrapper.setInnerElement(errorResp);
        }
    }
    ...
    protected void validatePayloadIsTosca(Wrapper<Response> responseWrapper, UploadResourceInfo uploadResourceInfo,
        ...
        if (!isValid) {
            ...
            Response errorResponse = buildErrorResponse(responseFormat);
            getComponentsUtils().auditResource(responseFormat, user, uploadResourceInfo.getName(),
    ...
    protected void validatePayloadNameSpace(final Wrapper<Response> responseWrapper,
            final UploadResourceInfo resourceInfo, final User user,
        ...
        if (!isValid) {
            final ResponseFormat responseFormat = getComponentsUtils()
                    .getResponseFormat(ActionStatus.INVALID_RESOURCE_NAMESPACE);
            final Response errorResponse = buildErrorResponse(responseFormat);
            getComponentsUtils().auditResource(responseFormat, user, resourceInfo.getName(),
                    AuditingActionEnum.IMPORT_RESOURCE);
            responseWrapper.setInnerElement(errorResponse);
        } else {
    ...
    private void validatePayloadIsSingleResource(Wrapper<Response> responseWrapper,
            UploadResourceInfo uploadResourceInfo, User user,
            String toscaPayload) {
    ...
    private void validatePayloadIsNotService(Wrapper<Response> responseWrapper, User user,
            UploadResourceInfo uploadResourceInfo,
            String toscaPayload) {
        log.debug("checking payload is not a tosca service");
        Map<String, Object> mappedToscaTemplate = (Map<String, Object>) new Yaml().load(toscaPayload);
        Either<Object, ResultStatusEnum> toscaElement = ImportUtils
                .findToscaElement(mappedToscaTemplate, TypeUtils.ToscaTagNamesEnum.TOPOLOGY_TEMPLATE,
                        ToscaElementTypeEnum.ALL);
        if (toscaElement.isLeft()) {
            ResponseFormat responseFormat = getComponentsUtils()
                    .getResponseFormat(ActionStatus.NOT_RESOURCE_TOSCA_TEMPLATE);
            Response errorResponse = buildErrorResponse(responseFormat);
            getComponentsUtils().auditResource(responseFormat, user, uploadResourceInfo.getName(),
                    AuditingActionEnum.IMPORT_RESOURCE);
            responseWrapper.setInnerElement(errorResponse);
    ...
    private void validateToscaTemplatePayloadName(Wrapper<Response> responseWrapper,
            UploadResourceInfo uploadResourceInfo, User user) {
        ...
        if (!isValidSuffix) {
            ResponseFormat responseFormat = getComponentsUtils()
                    .getResponseFormat(ActionStatus.INVALID_TOSCA_FILE_EXTENSION);
            Response errorResponse = buildErrorResponse(responseFormat);
            getComponentsUtils().auditResource(responseFormat, user, uploadResourceInfo.getName(),
                    AuditingActionEnum.IMPORT_RESOURCE);
            responseWrapper.setInnerElement(errorResponse);
    ...
    private void validateMD5(Wrapper<Response> responseWrapper, User user, UploadResourceInfo resourceInfo,
    ...
    private void fillToscaTemplateFromJson(Wrapper<Response> responseWrapper, Wrapper<String> yamlStringWrapper,
            User user,
    ...
    void fillPayload(Wrapper<Response> responseWrapper, Wrapper<UploadResourceInfo> uploadResourceInfoWrapper,
            Wrapper<String> yamlStringWrapper,
            User user, String resourceInfoJsonString, ResourceAuthorityTypeEnum resourceAuthorityEnum, File file)
            throws ZipException {
        if (responseWrapper.isEmpty()) {
            if (resourceAuthorityEnum.isBackEndImport()) {
                ...
                if (!resourceAuthorityEnum.equals(ResourceAuthorityTypeEnum.CSAR_TYPE_BE)) {
                    if (responseWrapper.isEmpty()) {
                        validateZip(responseWrapper, file,
                                uploadResourceInfoWrapper.getInnerElement().getPayloadName());
                    }
                    // Fill PayLoad From File
                    if (responseWrapper.isEmpty()) {
                        fillToscaTemplateFromZip(yamlStringWrapper,
                                uploadResourceInfoWrapper.getInnerElement().getPayloadName(), file);
                    }
                } else {
                    if (responseWrapper.isEmpty()) {
                        validateCsar(responseWrapper, file,
                                uploadResourceInfoWrapper.getInnerElement().getPayloadName());
                    }
                    // Fill PayLoad From File
                    if (responseWrapper.isEmpty()) {
                ...
                if (responseWrapper.isEmpty()) {
                    fillToscaTemplateFromJson(responseWrapper, yamlStringWrapper, user,
    ...
    protected void specificResourceAuthorityValidations(final Wrapper<Response> responseWrapper,
        ...
        if (responseWrapper.isEmpty()) {
            // UI Only Validation
            if (!resourceAuthorityEnum.isBackEndImport()) {
                importUIValidations(responseWrapper, uploadResourceInfoWrapper.getInnerElement(), user, request,
                        resourceInfoJsonString);
            }
            // User Defined Type Resources
            if (resourceAuthorityEnum.isUserTypeResource() && !CsarValidationUtils
                    .isCsarPayloadName(uploadResourceInfoWrapper.getInnerElement().getPayloadName())
                    && responseWrapper.isEmpty()) {
                validatePayloadNameSpace(responseWrapper, uploadResourceInfoWrapper.getInnerElement(), user,
                        yamlStringWrapper.getInnerElement());
            }
        }
    }

    void commonGeneralValidations(Wrapper<Response> responseWrapper, Wrapper<User> userWrapper,
        ...
        if (responseWrapper.isEmpty()) {
            validateAndFillResourceJson(responseWrapper, uploadResourceInfoWrapper, userWrapper.getInnerElement(),
                    resourceAuthorityEnum,
                    resourceInfoJsonString);
        }
        if (responseWrapper.isEmpty()) {
            validateToscaTemplatePayloadName(responseWrapper, uploadResourceInfoWrapper.getInnerElement(),
                    userWrapper.getInnerElement());
        }
        if (responseWrapper.isEmpty()) {
            validateResourceType(responseWrapper, uploadResourceInfoWrapper.getInnerElement(),
                    userWrapper.getInnerElement());
        }
    }
    ...
    private void importUIValidations(Wrapper<Response> responseWrapper, UploadResourceInfo resourceInfo, User user,
            HttpServletRequest request,
            String resourceInfoJsonString) {
        if (responseWrapper.isEmpty()) {
            validateMD5(responseWrapper, user, resourceInfo, request, resourceInfoJsonString);
        }
        if (responseWrapper.isEmpty() && request != null && request.getMethod() != null
    ...
    void commonPayloadValidations(Wrapper<Response> responseWrapper, Wrapper<String> yamlStringWrapper, User user,
        ...
        if (responseWrapper.isEmpty()) {
            validatePayloadIsSingleResource(responseWrapper, uploadResourceInfo, user,
                    yamlStringWrapper.getInnerElement());
        }
    }

    void handleImport(Wrapper<Response> responseWrapper, User user, UploadResourceInfo resourceInfoObject,
            String yamlAsString, ResourceAuthorityTypeEnum authority, boolean createNewVersion,
            String resourceUniqueId) {

        System.out.println("[handleImport] ENTER");
        System.out.println("[handleImport] authority=" + authority);
        System.out.println("[handleImport] createNewVersion=" + createNewVersion);
        System.out.println("[handleImport] resourceUniqueId=" + resourceUniqueId);
        System.out.println("[handleImport] yaml present=" + (yamlAsString != null));
        System.out.println("[handleImport] payloadName=" + resourceInfoObject.getPayloadName());
        ...
        try {
            ...
            if (CsarValidationUtils.isCsarPayloadName(resourceInfoObject.getPayloadName())) {
                ...
                System.out.println("[handleImport] Importing resource from CSAR...");
                ...
                System.out.println("[handleImport] Importing user-defined resource...");
                createOrUpdateResponse = resourceImportManager.importUserDefinedResource(
                        yamlAsString, resourceInfoObject, user, false);

            } else {
                log.debug("import normative type resource");
                System.out.println("[handleImport] Importing normative resource...");
                createOrUpdateResponse = resourceImportManager.importNormativeResource(
                        yamlAsString, resourceInfoObject, null, user, createNewVersion, true, false);
            }

            if (createOrUpdateResponse != null) {
                System.out.println("[handleImport] createOrUpdateResponse received.");
                importedResourceStatus = createOrUpdateResponse;
            } else {
                System.out.println("[handleImport] createOrUpdateResponse is null.");
            }

            if (importedResourceStatus != null) {
                System.out.println("[handleImport] importedResourceStatus is NOT null.");
                System.out.println("[handleImport] Building representation from imported resource...");
                ...
                try {
                    ...
                    System.out.println("[handleImport] ERROR while building representation: " + e.getMessage());
                }

                response = buildOkResponse(
                        getComponentsUtils().getResponseFormat(importedResourceStatus.right),
                        representation);

                System.out.println("[handleImport] OK response built.");
            } else {
                System.out.println("[handleImport] importedResourceStatus is NULL — no resource imported.");
            }

            responseWrapper.setInnerElement(response);
            System.out.println("[handleImport] EXIT (normal).");

        } catch (Exception ex) {
            System.out.println("[handleImport] ERROR: " + ex.getMessage());
            ex.printStackTrace(System.out);
            System.out.println("[handleImport] EXIT (error).");
            throw ex;
        }
    }

    private ImmutablePair<Resource, ActionStatus> importResourceFromUICsar(UploadResourceInfo resourceInfoObject,
            User user,
            String resourceUniqueId) {
    ...
    private void getAndValidateCsarYaml(Map<String, byte[]> csarUIPayload, Resource resource, User user,
            String csarUUID) {
        getAndValidateComponentCsarYaml(csarUIPayload, resource, user, csarUUID);
    }

    private void getAndValidateComponentCsarYaml(Map<String, byte[]> csarUIPayload, Component component, User user,
            String csarUUID) {
        ...
        if (getToscaYamlRes.isRight()) {
            ...
            if (component instanceof Resource) {
                ...
                getComponentsUtils().auditResource(responseFormat, user, (Resource) component,
                        AuditingActionEnum.CREATE_RESOURCE);
            } else {
                BeEcompErrorManager.getInstance().logBeDaoSystemError(
                        "Creating service from CSAR: fetching CSAR with id " + csarUUID + " failed");
            }
    ...
    void validateInputStream(final HttpServletRequest request, Wrapper<String> dataWrapper,
            Wrapper<ResponseFormat> errorWrapper) throws IOException {
    ...
    <T> void validateClassParse(String data, Wrapper<T> parsedClassWrapper, Supplier<Class<T>> classGen,
    ...
    protected void validateAndFillServiceJson(Wrapper<Response> responseWrapper,
            Wrapper<UploadServiceInfo> uploadServiceInfoWrapper,
        ...
        try {
            ...
            if (serviceInfoObject == null) {
                ...
                if (!serviceAuthorityEnum.isBackEndImport()) {
                    isValid = serviceInfoObject.getPayloadName() != null
                            && !serviceInfoObject.getPayloadName().isEmpty();
                ...
                log.debug(
                        "get isValid:{},serviceInfoObject get name:{},get tags:{},getContactId:{},"
                                + " getPayloadName:{}",
                        isValid,
    ...
    protected void validateToscaTemplatePayloadName(Wrapper<Response> responseWrapper,
            UploadServiceInfo uploadServiceInfo) {
    ...
    protected void specificServiceAuthorityValidations(Wrapper<Response> responseWrapper,
        ...
        if (responseWrapper.isEmpty()) {
            // UI Only Validation
            if (!serviceAuthorityEnum.isBackEndImport()) {
                importUIValidations(responseWrapper, uploadServiceInfoWrapper.getInnerElement(), request,
                        serviceInfoJsonString);
            }
    ...
    protected void importUIValidations(Wrapper<Response> responseWrapper, UploadServiceInfo serviceInfo,
            HttpServletRequest request,
    ...
    protected void validatePayloadNameSpace(Wrapper<Response> responseWrapper, UploadServiceInfo serviceInfo,
            String toscaPayload) {
        boolean isValid;
        String nameSpace = "";
        Map<String, Object> mappedToscaTemplate = (Map<String, Object>) new Yaml().load(toscaPayload);
        ...
        if (!isValid) {
            ResponseFormat responseFormat = getComponentsUtils()
                    .getResponseFormat(ActionStatus.INVALID_SERVICE_NAMESPACE);
            Response errorResponse = buildErrorResponse(responseFormat);
            responseWrapper.setInnerElement(errorResponse);
        } else {
    ...
    protected void validateMD5(Wrapper<Response> responseWrapper, HttpServletRequest request,
        ...
        if (!isValid) {
            ResponseFormat responseFormat = getComponentsUtils()
                    .getResponseFormat(ActionStatus.INVALID_SERVICE_CHECKSUM);
            Response errorResponse = buildErrorResponse(responseFormat);
            responseWrapper.setInnerElement(errorResponse);
        }
    }
    ...
    protected void handleImportService(Wrapper<Response> responseWrapper, User user,
            UploadServiceInfo serviceInfoObject) {
        Response response = null;
    ...
    private ImmutablePair<Service, ActionStatus> importServiceFromUICsar(UploadServiceInfo serviceInfoObject,
            User user) {
     ...
     * @throws ZipException
     */
    protected void fillServicePayload(Wrapper<Response> responseWrapper,
            Wrapper<UploadServiceInfo> uploadServiceInfoWrapper,
        ...
        if (responseWrapper.isEmpty()) {
            ...
            if (serviceAuthorityEnum.isBackEndImport()) {
                ...
                if (!serviceAuthorityEnum.equals(ServiceAuthorityTypeEnum.CSAR_TYPE_BE)) {
                    ...
                    if (responseWrapper.isEmpty()) {
                        fillToscaTemplateFromZip(yamlStringWrapper,
                                uploadServiceInfoWrapper.getInnerElement().getPayloadName(), file);
                    }
                } else {
                    ...
                    // Fill PayLoad From File
                    if (responseWrapper.isEmpty()) {
                        fillServicePayloadDataFromFile(responseWrapper, uploadServiceInfoWrapper.getInnerElement(),
                                file);
                    }
                }
            } else {
                // Fill PayLoad From JSON
                if (responseWrapper.isEmpty()) {
                    fillServiceToscaTemplateFromJson(responseWrapper, yamlStringWrapper, user,
    ...
    protected void fillServicePayloadDataFromFile(Wrapper<Response> responseWrapper,
            UploadServiceInfo uploadServiceInfoWrapper, File file) {
        try (InputStream fileInputStream = new FileInputStream(file)) {
    ...
    private void fillServiceToscaTemplateFromJson(Wrapper<Response> responseWrapper, Wrapper<String> yamlStringWrapper,
            User user,
        ...
        if (serviceInfo.getPayloadData() == null || serviceInfo.getPayloadData().isEmpty()) {
            ResponseFormat responseFormat = getComponentsUtils()
                    .getResponseFormat(ActionStatus.INVALID_RESOURCE_PAYLOAD);
            Response errorResponse = buildErrorResponse(responseFormat);
            getComponentsUtils().auditResource(responseFormat, user, serviceInfo.getName(),
                    AuditingActionEnum.IMPORT_RESOURCE);
            responseWrapper.setInnerElement(errorResponse);
        } else {
