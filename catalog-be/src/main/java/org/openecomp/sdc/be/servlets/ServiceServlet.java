...
public class ServiceServlet extends AbstractValidationsServlet {
    ...
    @Tags({@Tag(name = "SDCE-2 APIs")})
    @Operation(description = "Completely Delete Service", method = "DELETE", summary = "Return no content", responses = {
            @ApiResponse(content = @Content(array = @ArraySchema(schema = @Schema(implementation = Service.class)))),
            @ApiResponse(responseCode = "204", description = "Service deleted"), @ApiResponse(responseCode = "403", description = "Restricted operation"),
            @ApiResponse(responseCode = "400", description = "Invalid content / Missing content"),
            @ApiResponse(responseCode = "404", description = "Service not found")})
    @PermissionAllowed(AafPermission.PermNames.INTERNAL_ALL_VALUE)
    public Response completelyDeleteService(@PathParam("serviceId") final String serviceId, @Context final HttpServletRequest request) {
        String url = request.getMethod() + " " + request.getRequestURI();
        log.debug(START_HANDLE_REQUEST_OF, url);
        String userId = request.getHeader(Constants.USER_ID_HEADER);
        User modifier = new User();
        ...
        try {
            ...
            loggerSupportability
                    .log(LoggerSupportabilityActions.DELETE_SERVICE, StatusCode.STARTED, "Starting to delete service {} by user {} ", serviceIdLower,
                            userId);
            ResponseFormat actionResponse = serviceBusinessLogic.completelyDeleteService(serviceId, modifier);
            if (actionResponse.getStatus() != HttpStatus.SC_OK) {
            ...
            BeEcompErrorManager.getInstance().logBeRestApiGeneralError("Completely Delete Service");
            log.debug("delete service failed with exception", e);
            throw e;
        }
    }
