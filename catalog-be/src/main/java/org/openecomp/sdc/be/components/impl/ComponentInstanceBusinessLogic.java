...
public class ComponentInstanceBusinessLogic extends BaseBusinessLogic {
    ...
    public Either<Map<String, ComponentInstance>, ResponseFormat> copyComponentInstance(ComponentInstance inputComponentInstance,
        ...
        Either<ComponentInstance, ResponseFormat> actionResponse = createComponentInstance(
                "services", containerComponentId, userId, inputComponentInstance);
        ...
        log.info("destComponentInstance's data is { }", destComponentInstance.toString());

        Either<String, ResponseFormat> copyComponentInstanceWithInfo = deepCopyComponentInstance(
        ...
        resultMap.put("componentInstances", destComponentInstance);
    ...
    private Either<String, ResponseFormat> deepCopyComponentInstance(
        ...
        Either<String, ResponseFormat> copyComponentInstanceWithAdditionalInfo = copyComponentInstanceWithAdditionalInfo(
                sourceComponent, destComponent, sourceComponentInstanceId, destComponentInstance, userId);
        if (copyComponentInstanceWithAdditionalInfo.isRight()) {
            ...
            return Either.right(componentsUtils.getResponseFormat(
                    ActionStatus.USER_DEFINED, "Failed to copy the component instance with additional info as part of deep copy"));
        }
    ...
    private Either<String, ResponseFormat> copyComponentInstanceWithPropertiesAndInputs(
            ...
            ComponentInstance destComponentInstance, String userId) {
        log.info("start to copy ComponentInstance with properties and inputs");
                ...
                for (ComponentInstanceProperty sourceProp : sourcePropList) {
                    ...
                    if (sourceProp.getGetInputValues() != null) {
                        ...
                        if (getSourceInputDefaultValue.left().value() != null) {
                            destInputNewValueMap.put(destComponent.getUniqueId() +
                                    "." + destComponentInstance.getNormalizedName() +
                                    "_" + destPropertyName, getSourceInputDefaultValue.left().value());
                        }
    ...
    private Either<String, ResponseFormat> copyComponentInstanceWithAdditionalInfo(Component sourceComponent,
                                                                                   Component destComponent,
                                                                                   String sourceComponentInstanceId,
                                                                                   ComponentInstance destComponentInstance,
                                                                                   String userId) {
        ...
        String destComponentInstanceId = destComponentInstance.getUniqueId();
        log.info("start to copy component instance with additionalInfo");
        WebApplicationContext wac = ContextLoader.getCurrentWebApplicationContext();
        AdditionalInformationBusinessLogic additionalInformationBusinessLogic =
                (AdditionalInformationBusinessLogic) wac.getBean("additionalInformationBusinessLogic");
        log.info("now get all additional information");
        Either<AdditionalInformationDefinition, ResponseFormat> getAdditionalInfo =
                additionalInformationBusinessLogic.getAllAdditionalInformation(NodeTypeEnum.Service, sourceComponent.getUniqueId(), userId);

        if (getAdditionalInfo.isRight()) {
            log.info("errorMessage {} {}", getAdditionalInfo.right().value().getFormattedMessage(),
                    getAdditionalInfo.right().value().getStatus().toString());
        }
            ...
            if (additionalInfo.getParameters() != null || additionalInfo.getParameters().size() > 0) {
                ...
                for (AdditionalInfoParameterInfo parameterInfo : parameterList) {
                    ...
                    if (parameterInfoUid.contains(sourceComponentInstanceId) && !"".equals(parameterInfo.getValue())) {
                        ...
                        destAdditionalInfo = additionalInformationBusinessLogic.
                                createAdditionalInformation(NodeTypeEnum.Service, destComponent.getUniqueId(),
                                        parameterInfo, userId);

                        if (destAdditionalInfo.isRight()) {
                            log.error("Error in destAdditionalInfo with dest component instance id:{}",
                                    destComponentInstanceId);
                            return Either.right(componentsUtils.getResponseFormat(ActionStatus.INVALID_CONTENT_PARAM,
                                    "Failed to paste component instance to the canvas, additionalInfo copy"));
                        }
                        log.info("The dest additional information is {}", destAdditionalInfo.toString());
                    }
                }
            }
        }
