...
public class DataValidatorFilter implements Filter {
    ...
    private boolean isExcluded(final String path) {
        final List<String> excludedUrlsList = getExcludedUrlsList();
        return CollectionUtils.isNotEmpty(excludedUrlsList) && excludedUrlsList.stream().anyMatch(path::contains);
    }

    private List<String> getExcludedUrlsList() {
        final String excludedUrls = ConfigurationManager.getConfigurationManager().getConfiguration().getExcludedUrls();
        if (StringUtils.isNotBlank(excludedUrls)) {
            return Arrays.asList(excludedUrls.split(","));
        }
