...
public class ComponentInstanceOutputsRedeclareHandler {
    ...
    public ActionStatus redeclareComponentOutputsForInstance(Component container, String newInstanceId, List<OutputDefinition> oldOutputs) {
        ...
        Map<String, List<AttributeDataDefinition>> allAttributesForInstance = getAllGetAttributesForInstance(container, newInstanceId);
        List<OutputDefinition> previouslyDeclaredOutputs = declaredOutputsResolver
            .getPreviouslyDeclaredOutputsToMerge(oldOutputs, container, allAttributesForInstance);
    ...
    private Map<String, List<AttributeDataDefinition>> getAllGetAttributesForInstance(Component newComponent, String instanceId) {
        List<AttributeDataDefinition> allInstanceAttributes = Stream
            .of(newComponent.safeGetComponentInstanceAttributes(instanceId), newComponent.safeGetComponentInstanceOutput(instanceId))
            .flatMap(Collection::stream).map(AttributeDataDefinition::new).collect(toList());
        return singletonMap(instanceId, allInstanceAttributes);
    }
