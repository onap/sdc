...
import static org.openecomp.sdc.be.dao.jsongraph.types.VertexTypeEnum.DATA_TYPES;
...
public class DataTypeServlet extends BeGenericServlet {
    ...
    public Response fetchDataType(@Context final HttpServletRequest request,
        ...
        props.put(GraphPropertyEnum.DATA_TYPES, dataType);
        props.put(GraphPropertyEnum.MODEL, modelName);
        props.put(GraphPropertyEnum.IS_HIGHEST_VERSION, true);
        final Either<List<GraphVertex>, JanusGraphOperationStatus> byCriteria = janusGraphDao.getByCriteria(DATA_TYPES, props);
        if (byCriteria.isRight()) {
            return buildErrorResponse(getComponentsUtils().getResponseFormat(ActionStatus.NO_CONTENT));
        }
        return buildOkResponse(getComponentsUtils().getResponseFormat(ActionStatus.OK), byCriteria.left().value().get(0));
