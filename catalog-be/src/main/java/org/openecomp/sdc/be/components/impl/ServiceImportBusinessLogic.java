...
public class ServiceImportBusinessLogic {
    ...
    public ServiceImportBusinessLogic(IElementOperation elementDao,
                                      ...
                                      NodeFilterValidator serviceFilterValidator,
    ...
    public Service createService(Service service, AuditingActionEnum auditingAction, User user,
    ...
    protected Service createServiceFromCsar(Service service, User user, Map<String, byte[]> csarUIPayload,
                                            String csarUUID) {
        ...
        try {
            ...
            Service cService = createServiceFromYaml(service, csarInfo.getMainTemplateContent(),
    ...
    protected Service createServiceFromYaml(Service service, String topologyTemplateYaml,
        ...
        try {
            ...
            createdService = createServiceAndRIsFromYaml(service, false, nodeTypesArtifactsToCreate, shouldLock,
                inTransaction, csfyp);
    ...
    protected Service createServiceAndRIsFromYaml(Service service, boolean isNormative,
                                                  ...
                                                  CreateServiceFromYamlParameter csfyp) {
        ...
        if (shouldLock) {
            Either<Boolean, ResponseFormat> lockResult = serviceBusinessLogic
                .lockComponentByName(service.getSystemName(), service, CREATE_RESOURCE);
        ...
        try {
            ...
            if (shouldLock) {
                serviceBusinessLogic.graphLockOperation
                    .unlockComponentByName(service.getSystemName(), service.getUniqueId(),
    ...
    protected Either<Resource, ResponseFormat> createOrUpdateArtifacts(
        ...
        if (preparedResource.getResourceType() == ResourceTypeEnum.VF) {
            if (nodeName != null && nodeTypesArtifactsToHandle.get(nodeName) != null && !nodeTypesArtifactsToHandle
                .get(nodeName).isEmpty()) {
                Either<List<ArtifactDefinition>, ResponseFormat> handleNodeTypeArtifactsRes =
                    handleNodeTypeArtifacts(preparedResource, nodeTypesArtifactsToHandle.get(nodeName),
                        createdArtifacts, csarInfo.getModifier(), inTransaction, true);
    ...
    protected Either<Resource, ResponseFormat> handleVfCsarArtifacts(Resource resource, CsarInfo csarInfo,
        ...
        if (csarInfo.getCsar() != null) {
            createOrUpdateSingleNonMetaArtifactToComstants(resource, csarInfo, artifactOperation, shouldLock,
                inTransaction);
            ...
            if (artifacsMetaCsarStatus.isLeft()) {
                return getResourceResponseFormatEither(resource, csarInfo, createdArtifacts, artifactOperation,
                    shouldLock, inTransaction, artifacsMetaCsarStatus);
            } else {
                return csarArtifactsAndGroupsBusinessLogic
                    .deleteVFModules(resource, csarInfo, shouldLock, inTransaction);
            }
        }
        return Either.left(resource);
    }

    protected void createOrUpdateSingleNonMetaArtifactToComstants(Resource resource, CsarInfo csarInfo,
                                                                  ArtifactOperationInfo artifactOperation,
                                                                  boolean shouldLock, boolean inTransaction) {
    ...
    protected Either<Resource, ResponseFormat> createOrUpdateNonMetaArtifacts(CsarInfo csarInfo, Resource resource,
        ...
        try {
            Either<List<CsarUtils.NonMetaArtifactInfo>, String> artifactPathAndNameList = getValidArtifactNames(
                csarInfo, collectedWarningMessages);
            ...
            if (ArtifactsBusinessLogic.ArtifactOperationEnum
                .isCreateOrLink(artifactOperation.getArtifactOperationEnum())) {
                vfCsarArtifactsToHandle = new EnumMap<>(ArtifactsBusinessLogic.ArtifactOperationEnum.class);
                vfCsarArtifactsToHandle
                    .put(artifactOperation.getArtifactOperationEnum(), artifactPathAndNameList.left().value());
            } else {
                    ...
                    if (foundArtifact != null) {
                        if (ArtifactTypeEnum.findType(foundArtifact.getArtifactType()).equals(currNewArtifact
                            ...
                            serviceBusinessLogic.artifactsBusinessLogic
                                .handleAuditing(auditingAction, resource, resource.getUniqueId(),
            ...
            if (responseWrapper.isEmpty()) {
                if (!artifactsToUpload.isEmpty()) {
                    nodeTypeArtifactsToHandle
                        .put(ArtifactsBusinessLogic.ArtifactOperationEnum.CREATE, artifactsToUpload);
                }
                if (!artifactsToUpdate.isEmpty()) {
                    nodeTypeArtifactsToHandle
                        .put(ArtifactsBusinessLogic.ArtifactOperationEnum.UPDATE, artifactsToUpdate);
                }
    ...
    protected Either<Resource, ResponseFormat> processCsarArtifacts(CsarInfo csarInfo, Resource resource,
                                                                    List<ArtifactDefinition> createdArtifacts,
                                                                    boolean shouldLock, boolean inTransaction,
        ...
        if (operation.getArtifactOperationEnum() == ArtifactsBusinessLogic.ArtifactOperationEnum.UPDATE
            ...
            if (serviceImportParseLogic.isArtifactDeletionRequired(artifactId, artifactFileBytes, isFromCsar)) {
                Either<ArtifactDefinition, ResponseFormat> handleDelete = serviceBusinessLogic.artifactsBusinessLogic
                    .handleDelete(resource.getUniqueId(), artifactId, csarInfo.getModifier(), resource,
    ...
    protected Either<Service, ResponseFormat> createOrUpdateArtifacts(
        ...
        Either<Service, ResponseFormat> createdCsarArtifactsEither = handleVfCsarArtifacts(preparedService, csarInfo,
            createdArtifacts,
            new ArtifactOperationInfo(false, false, operation), shouldLock, inTransaction);
        log.trace("************* Finished to add artifacts from yaml {}", yamlFileName);
        if (createdCsarArtifactsEither.isRight()) {
        ...
        if (csarInfo.getCsar() != null) {
            ...
            if (eitherGerResource.isRight()) {
                ResponseFormat responseFormat = componentsUtils.getResponseFormatByComponent(
                    componentsUtils.convertFromStorageResponse(eitherGerResource.right().value()), service,
            ...
            if (artifacsMetaCsarStatus.isLeft()) {
                ...
                if (ArtifactsBusinessLogic.ArtifactOperationEnum
                    .isCreateOrLink(artifactOperation.getArtifactOperationEnum())) {
                    createArtifactsFromCsar = csarArtifactsAndGroupsBusinessLogic
                        .createResourceArtifactsFromCsar(csarInfo, service, artifactsContents, artifactsFileName,
                            createdArtifacts);
                } else {
                    Either<Component, ResponseFormat> result = csarArtifactsAndGroupsBusinessLogic
                        .updateResourceArtifactsFromCsar(csarInfo, service, artifactsContents, artifactsFileName,
                            createdArtifacts, shouldLock, inTransaction);
                ...
                return csarArtifactsAndGroupsBusinessLogic
                    .deleteVFModules(service, csarInfo, shouldLock, inTransaction);
            }
        }
    ...
    protected Either<Service, ResponseFormat> processCsarArtifacts(CsarInfo csarInfo, Service resource,
                                                                   List<ArtifactDefinition> createdArtifacts,
                                                                   boolean shouldLock, boolean inTransaction,
                                                                   Either<Service, ResponseFormat> resStatus,
                                                                   EnumMap<ArtifactsBusinessLogic.ArtifactOperationEnum, List<CsarUtils.NonMetaArtifactInfo>> vfCsarArtifactsToHandle) {
        for (Map.Entry<ArtifactsBusinessLogic.ArtifactOperationEnum, List<CsarUtils.NonMetaArtifactInfo>> currArtifactOperationPair : vfCsarArtifactsToHandle
            .entrySet()) {
            Optional<ResponseFormat> optionalCreateInDBError =
    ...
    protected Either<Boolean, ResponseFormat> createOrUpdateSingleNonMetaArtifact(Service service, CsarInfo csarInfo,
        ...
        if (operation.getArtifactOperationEnum() == ArtifactsBusinessLogic.ArtifactOperationEnum.UPDATE
            ...
            if (serviceImportParseLogic.isArtifactDeletionRequired(artifactId, artifactFileBytes, isFromCsar)) {
                Either<ArtifactDefinition, ResponseFormat> handleDelete = serviceBusinessLogic.artifactsBusinessLogic
                    .handleDelete(service.getUniqueId(), artifactId, csarInfo.getModifier(), service,
                        shouldLock, inTransaction);
                if (handleDelete.isRight()) {
                    result = Either.right(handleDelete.right().value());
                }
    ...
    protected Either<Service, ResponseFormat> createGroupsOnResource(Service service,
        ...
        if (updatedResource.isRight()) {
            ResponseFormat responseFormat = componentsUtils.getResponseFormatByComponent(
                componentsUtils.convertFromStorageResponse(updatedResource.right().value()), service,
                ComponentTypeEnum.SERVICE);
            return Either.right(responseFormat);
        }
    ...
    protected List<GroupDefinition> updateGroupsMembersUsingResource(Map<String, GroupDefinition> groups,
                                                                     Service component) {
        List<GroupDefinition> result = new ArrayList<>();
        List<ComponentInstance> componentInstances = component.getComponentInstances();
        if (groups != null) {
            Either<Boolean, ResponseFormat> validateCyclicGroupsDependencies = serviceImportParseLogic
                .validateCyclicGroupsDependencies(groups);
    ...
    protected Resource createRIAndRelationsFromYaml(String yamlName, Resource resource,
        ...
        try {
            ...
            resource = createResourceInstancesRelations(csarInfo.getModifier(), yamlName, resource,
    ...
    protected Resource createResourceInstancesRelations(User user, String yamlName, Resource resource,
        ...
        if (((MapUtils.isEmpty(uploadResInstancesMap) || CollectionUtils.isEmpty(componentInstancesList)) &&
            ...
            BeEcompErrorManager.getInstance().logInternalDataError("createResourceInstancesRelations",
                "No instances found in a resource or nn yaml template. ", BeEcompErrorManager.ErrorSeverity.ERROR);
            throw new ComponentException(
                componentsUtils.getResponseFormat(ActionStatus.NOT_TOPOLOGY_TOSCA_TEMPLATE, yamlName));
        }
        ...
        log.debug(
            "enter ServiceImportBusinessLogic createResourceInstancesRelations#createResourceInstancesRelations - Before get all datatypes. ");
        Either<Map<String, DataTypeDefinition>, JanusGraphOperationStatus> allDataTypes = serviceBusinessLogic.dataTypeCache
            .getAll();
        if (allDataTypes.isRight()) {
            ...
            BeEcompErrorManager.getInstance().logInternalFlowError("UpdatePropertyValueOnComponentInstance",
                "Failed to update property value on instance. Status is " + status,
    ...
    protected void processProperty(Resource resource, ComponentInstance currentCompInstance,
                                   Map<String, DataTypeDefinition> allDataTypes,
                                   Map<String, InputDefinition> currPropertiesMap,
        ...
        String validPropertyVAlue = serviceBusinessLogic
            .validatePropValueBeforeCreate(property, value, isValidate, allDataTypes);
    ...
    protected void handleSubstitutionMappings(Resource resource,
                                              Map<String, UploadComponentInstanceInfo> uploadResInstancesMap) {
        if (resource.getResourceType() == ResourceTypeEnum.VF) {
            Either<Resource, StorageOperationStatus> getResourceRes = toscaOperationFacade
    ...
    protected Resource createResourceInstances(String yamlName, Resource resource,
        ...
        if (MapUtils.isEmpty(uploadResInstancesMap)
            && resource.getResourceType() != ResourceTypeEnum.PNF) { // PNF can have no resource instances
        ...
        uploadResInstancesMap
            .values()
            .forEach(i -> createAndAddResourceInstance(i, yamlName, resource, nodeNamespaceMap, existingNodeTypeMap,
    ...
    protected void handleNodeTypes(String yamlName, Resource resource,
        ...
        try {
            ...
            componentsUtils
                .auditResource(responseFormat, csarInfo.getModifier(), resource, AuditingActionEnum.IMPORT_RESOURCE);
            throw e;
        } catch (StorageException e) {
            ResponseFormat responseFormat = componentsUtils
    ...
    protected Resource handleNestedVfc(Resource resource,
                                       ...
                                       List<ArtifactDefinition> createdArtifacts, Map<String, NodeTypeInfo> nodesInfo,
                                       CsarInfo csarInfo,
                                       String nodeName) {
    ...
    protected Resource handleComplexVfc(Resource resource,
        ...
        if (oldComplexVfcRes.isRight() && oldComplexVfcRes.right().value() != StorageOperationStatus.NOT_FOUND) {
            ...
            Either<Boolean, ResponseFormat> eitherValidation = serviceImportParseLogic
                .validateNestedDerivedFromDuringUpdate(
    ...
    protected void createNodeTypes(String yamlName, Resource resource, boolean needLock,
        ...
        while (nodesNameValueIter.hasNext()) {
            ...
            Map<ArtifactsBusinessLogic.ArtifactOperationEnum, List<ArtifactDefinition>> nodeTypeArtifactsToHandle =
                nodeTypesArtifactsToHandle == null
            ...
            if (nodeTypesInfo.containsKey(nodeType.getKey())) {
                ...
                ImmutablePair<Resource, ActionStatus> resourceCreated = createNodeTypeResourceFromYaml(yamlName,
                    nodeType, csarInfo.getModifier(), mapToConvert,
    ...
    protected ImmutablePair<Resource, ActionStatus> createNodeTypeResourceFromYaml(
        ...
        user = serviceBusinessLogic
            .validateUser(user, "CheckIn Resource", resourceVf, AuditingActionEnum.CHECKIN_RESOURCE, true);
    ...
    protected Service createRIAndRelationsFromYaml(String yamlName, Service service,
                                                   ...
                                                   String nodeName) {
        log.debug("************* Going to create all nodes {}", yamlName);
        handleServiceNodeTypes(yamlName, service, topologyTemplateYaml, false, nodeTypesArtifactsToCreate,
        ...
        service = createServiceInstancesRelations(csarInfo.getModifier(), yamlName, service,
            uploadComponentInstanceInfoMap);
        log.debug("************* Going to create positions {}", yamlName);
    ...
    protected Service createServiceInstancesRelations(User user, String yamlName, Service service,
        ...
        if (((MapUtils.isEmpty(uploadResInstancesMap) || CollectionUtils
            .isEmpty(componentInstancesList)))) { // PNF can have no resource instances
            log.debug(
                "#createResourceInstancesRelations - No instances found in the resource {} is empty, yaml template file name {}, ",
        ...
        log.debug(
            "enter ServiceImportBusinessLogic  createServiceInstancesRelations#createResourceInstancesRelations - Before get all datatypes. ");
        Either<Map<String, DataTypeDefinition>, JanusGraphOperationStatus> allDataTypes = serviceBusinessLogic.dataTypeCache
            .getAll();
        if (allDataTypes.isRight()) {
            JanusGraphOperationStatus status = allDataTypes.right().value();
        ...
        if (eitherGetResource.isRight()) {
            throw new ComponentException(componentsUtils.getResponseFormatByComponent(
                componentsUtils.convertFromStorageResponse(eitherGetResource.right().value()), service,
    ...
    protected void processProperty(Service resource, ComponentInstance currentCompInstance,
                                   Map<String, DataTypeDefinition> allDataTypes,
                                   Map<String, InputDefinition> currPropertiesMap,
                                   List<ComponentInstanceInput> instPropList, List<UploadPropInfo> propertyList) {
        UploadPropInfo propertyInfo = propertyList.get(0);
    ...
    protected ResponseFormat addPropertyValuesToRi(UploadComponentInstanceInfo uploadComponentInstanceInfo,
        ...
        if (propMap != null && propMap.size() > 0) {
            for (List<UploadPropInfo> propertyList : propMap.values()) {
                ...
                String validatePropValue = serviceBusinessLogic
                    .validatePropValueBeforeCreate(property, value, isValidate, allDataTypes);
    ...
    protected ResponseFormat addPropertyValuesToRi(UploadComponentInstanceInfo uploadComponentInstanceInfo,
                                                   Service service, Resource originResource,
                                                   ComponentInstance currentCompInstance,
                                                   Map<String, List<ComponentInstanceProperty>> instProperties,
                                                   Map<String, DataTypeDefinition> allDataTypes) {
        Map<String, List<UploadPropInfo>> propMap = uploadComponentInstanceInfo.getProperties();
    ...
    protected void processComponentInstanceCapabilities(
        Either<Map<String, DataTypeDefinition>, JanusGraphOperationStatus> allDataTypes,
        Map<ComponentInstance, Map<String, List<CapabilityDefinition>>> instCapabilties,
    ...
    protected void updateCapabilityPropertiesValues(
        Either<Map<String, DataTypeDefinition>, JanusGraphOperationStatus> allDataTypes,
        Map<String, List<CapabilityDefinition>> originCapabilities,
    ...
    protected void updatePropertyValues(List<ComponentInstanceProperty> properties,
    ...
    protected Resource getOriginResource(String yamlName, Map<String, Resource> originCompMap,
                                         ComponentInstance currentCompInstance) {
        Resource originResource;
    ...
    protected void handleSubstitutionMappings(Service service,
                                              Map<String, UploadComponentInstanceInfo> uploadResInstancesMap) {
        if (false) {
            Either<Resource, StorageOperationStatus> getResourceRes = toscaOperationFacade
            ...
            if (getResourceRes.isRight()) {
                ResponseFormat responseFormat = componentsUtils.getResponseFormatByComponent(
                    componentsUtils.convertFromStorageResponse(getResourceRes.right().value()), service,
                    ComponentTypeEnum.SERVICE);
                throw new ComponentException(responseFormat);
            }
            getResourceRes = updateCalculatedCapReqWithSubstitutionMappings(getResourceRes.left().value(),
    ...
    protected Either<Resource, StorageOperationStatus> updateCalculatedCapReqWithSubstitutionMappings(Resource resource,
        ...
        if (updateRes == null) {
            ...
            status = toscaOperationFacade
                .associateOrAddCalculatedCapReq(updatedInstCapabilities, updatedInstRequirements,
                    resource);
            if (status != StorageOperationStatus.OK && status != StorageOperationStatus.NOT_FOUND) {
                updateRes = Either.right(status);
            }
    ...
    protected void addRelationsToRI(String yamlName, Service service,
                                    Map<String, UploadComponentInstanceInfo> uploadResInstancesMap,
                                    List<ComponentInstance> componentInstancesList,
    ...
    protected Service createServiceInstances(String yamlName, Service service,
        ...
        uploadResInstancesMap
            .values()
            .forEach(i -> createAndAddResourceInstance(i, yamlName, service, nodeNamespaceMap, existingNodeTypeMap,
                resourcesInstancesMap));
        ...
        if (CollectionUtils
            .isEmpty(eitherGetResource.left().value().getComponentInstances())) { // PNF can have no resource instances
            log.debug("Error when create resource instance from csar. ComponentInstances list empty");
            BeEcompErrorManager.getInstance().logBeDaoSystemError(
                "Error when create resource instance from csar. ComponentInstances list empty");
            throw new ComponentException(componentsUtils.getResponseFormat(ActionStatus.NOT_TOPOLOGY_TOSCA_TEMPLATE));
    ...
    protected void createAndAddResourceInstance(UploadComponentInstanceInfo uploadComponentInstanceInfo,
                                                String yamlName,
                                                Resource resource, Map<String, Resource> nodeNamespaceMap,
                                                Map<String, Resource> existingnodeTypeMap,
        ...
        try {
            ...
            if (containerNodeType.equals(NodeTypeEnum.Resource)
                ...
                serviceImportParseLogic.setCapabilityNamesTypes(refResource.getCapabilities(),
    ...
    protected void createAndAddResourceInstance(UploadComponentInstanceInfo uploadComponentInstanceInfo,
                                                String yamlName,
                                                Service service, Map<String, Resource> nodeNamespaceMap,
                                                Map<String, Resource> existingnodeTypeMap,
                                                Map<ComponentInstance, Resource> resourcesInstancesMap) {
        Either<Resource, ResponseFormat> eitherResource;
        log.debug("*************Going to create  resource instances {}", uploadComponentInstanceInfo.getName());
    ...
    protected Resource validateResourceInstanceBeforeCreate(String yamlName,
                                                            UploadComponentInstanceInfo uploadComponentInstanceInfo,
                                                            Map<String, Resource> nodeNamespaceMap) {
        ...
        try {
            ...
            if (!ModelConverter.isAtomicComponent(refResource)
                && refResource.getResourceType() != ResourceTypeEnum.VF) {
    ...
    protected boolean validateResourceNotExisted(String type) {
        try {
            Either<Resource, StorageOperationStatus> latestResource = toscaOperationFacade
                .getLatestResourceByToscaResourceName(type);
    ...
    protected Resource updateResourceFromYaml(Resource oldRresource, Resource newRresource,
                                              ...
                                              String yamlFileName, String yamlFileContent, CsarInfo csarInfo,
                                              Map<String, NodeTypeInfo> nodeTypesInfo,
                                              Map<String, EnumMap<ArtifactsBusinessLogic.ArtifactOperationEnum, List<ArtifactDefinition>>> nodeTypesArtifactsToHandle,
        ...
        try {
            ...
            preparedResource = createResourceInstances(yamlFileName, preparedResource, instances,
            ...
            ResponseFormat responseFormat =
                e.getResponseFormat() == null ? componentsUtils.getResponseFormat(e.getActionStatus(), e.getParams())
                    : e.getResponseFormat();
            log.debug("#updateResourceFromYaml - failed to update resource from yaml {} .The error is {}", yamlFileName,
                responseFormat);
            componentsUtils.auditResource(responseFormat, csarInfo.getModifier(),
                preparedResource == null ? oldRresource : preparedResource, actionEnum);
            throw e;
        } catch (StorageException e) {
            ResponseFormat responseFormat = componentsUtils
                .getResponseFormat(componentsUtils.convertFromStorageResponse(e.getStorageOperationStatus()));
        ...
        preparedResource = serviceImportParseLogic
            .getResourceWithGroups(updateArtifactsEither.left().value().getUniqueId());

        ActionStatus mergingPropsAndInputsStatus = resourceDataMergeBusinessLogic
            .mergeResourceEntities(oldRresource, preparedResource);
    ...
    protected Resource createResourceFromYaml(Resource resource, String topologyTemplateYaml,
                                              String yamlName, Map<String, NodeTypeInfo> nodeTypesInfo,
                                              CsarInfo csarInfo,
                                              Map<String, EnumMap<ArtifactsBusinessLogic.ArtifactOperationEnum, List<ArtifactDefinition>>> nodeTypesArtifactsToCreate,
                                              boolean shouldLock, boolean inTransaction, String nodeName) {
        List<ArtifactDefinition> createdArtifacts = new ArrayList<>();
    ...
    protected Resource createResourceAndRIsFromYaml(String yamlName, Resource resource,
                                                    ParsedToscaYamlInfo parsedToscaYamlInfo,
                                                    AuditingActionEnum actionEnum, boolean isNormative,
                                                    List<ArtifactDefinition> createdArtifacts,
                                                    String topologyTemplateYaml,
                                                    Map<String, NodeTypeInfo> nodeTypesInfo, CsarInfo csarInfo,
                                                    Map<String, EnumMap<ArtifactsBusinessLogic.ArtifactOperationEnum, List<ArtifactDefinition>>> nodeTypesArtifactsToCreate,
        ...
        if (shouldLock) {
            Either<Boolean, ResponseFormat> lockResult = serviceBusinessLogic
                .lockComponentByName(resource.getSystemName(), resource,
                    CREATE_RESOURCE);
            if (lockResult.isRight()) {
                serviceImportParseLogic
        ...
        try {
            ...
            resource = serviceImportParseLogic
                .getResourceWithGroups(createArtifactsEither.left().value().getUniqueId());
    ...
    protected void updateGroupMembers(Map<String, GroupDefinition> groups, GroupDefinition updatedGroupDefinition,
    ...
    protected Map<String, Resource> createResourcesFromYamlNodeTypesList(String yamlName, Service service,
                                                                         Map<String, Object> mappedToscaTemplate,
                                                                         boolean needLock,
                                                                         Map<String, EnumMap<ArtifactsBusinessLogic.ArtifactOperationEnum, List<ArtifactDefinition>>> nodeTypesArtifactsToHandle,
                                                                         List<ArtifactDefinition> nodeTypesNewCreatedArtifacts,
        ...
        try {
            ...
            mapToConvert.put(TypeUtils.ToscaTagNamesEnum.TOSCA_VERSION.getElementName(), toscaVersion.left().value());
            Map<String, Object> nodeTypes = serviceImportParseLogic.getNodeTypesFromTemplate(mappedToscaTemplate);
            createNodeTypes(yamlName, service, needLock, nodeTypesArtifactsToHandle, nodeTypesNewCreatedArtifacts,
                nodeTypesInfo, csarInfo, mapToConvert, nodeTypes);
            return csarInfo.getCreatedNodes();
        } catch (Exception e) {
