...
public class OutputsBusinessLogic extends BaseBusinessLogic {
    ...
    public Either<List<ComponentInstanceOutput>, ResponseFormat> getComponentInstanceOutputs(final String userId,
        ...
        if (!ComponentValidations.validateComponentInstanceExist(component, componentInstanceId)) {
            ...
            log.debug("Failed to found component instance outputs {}, error: {}", componentInstanceId, actionStatus);
            loggerSupportability.log(LoggerSupportabilityActions.CREATE_OUTPUTS, component.getComponentMetadataForSupportLog(),
                StatusCode.ERROR, "Failed to found component instance outputs componentInstanceId: {}", componentInstanceId);
            return Either.right(componentsUtils.getResponseFormat(actionStatus));
        }
