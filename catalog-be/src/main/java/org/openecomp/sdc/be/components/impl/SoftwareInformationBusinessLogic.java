...
public class SoftwareInformationBusinessLogic {
    ...
    public Optional<PropertyDefinition> setSoftwareInformation(final Resource resource,
        ...
        if(resource.getResourceType() == ResourceTypeEnum.PNF && versionList.size()>0){
            final String default_sw_version_Const = "default_software_version";
            final String sw_version = versionList.get(0);
            PropertyDefinition default_Sw_Ver_PropertyDefinition = resource.safeGetInputs().stream().filter(s->s.getName().equals(default_sw_version_Const)).findFirst().get();
            if(default_Sw_Ver_PropertyDefinition!=null) {
                default_Sw_Ver_PropertyDefinition.setDefaultValue(sw_version);
            }
        }
