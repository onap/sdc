...
public class ComponentInstanceAttributesMergeBL {
    ...
    public ActionStatus mergeComponentInstanceAttributes(List<ComponentInstanceAttribute> oldInstAttributes,
        ...
        toscaOperationFacade.getToscaElement(instanceId);
        ...
        newInstAttributes.forEach(newAttribute -> oldInstAttributes
                .stream().filter(oldAttribute -> newAttribute.getName().equals(oldAttribute.getName())).forEach(instAttribute -> {
                    if (instAttribute.getValue() == null && newAttribute.getDefaultValue() != null) {
                        instAttribute.setDefaultValue(newAttribute.getDefaultValue());
                    }
                    instAttribute.setDescription(newAttribute.getDescription());
                    commonInstAttributes.add(instAttribute);
                }));
