...
class ModelImportManager:
    ...
    def __handle_model_non_node_types(self, action, model, model_payload_dict, with_metadata=False):
        if self.ACTION_UPGRADE != action or not os.path.isdir(self.__get_tosca_path(action, model)):
    ...
    def __handle_model_node_types(self, action, model, model_payload_dict, upgrade=False):
        if self.ACTION_UPGRADE != action or not os.path.isdir(self.__get_tosca_path(action, model) / self.NODE_FOLDER):
            action = self.ACTION_INIT
