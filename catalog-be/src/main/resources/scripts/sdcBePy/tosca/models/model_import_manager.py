...
class ModelImportManager:
    ...
    def create_models(self):
        for model_folder_name in self.__get_model_init_list():
            ...
            tosca_path = str(self.__get_base_action_path(self.ACTION_INIT)) + "/" + model_folder_name + "/tosca/"
            self.__model_client.import_model_elements(model_payload_dict, tosca_path)
            if os.path.isdir(tosca_path + "node-types/"):
    ...
    def update_models(self):
        for model_folder_name in self.__get_model_upgrade_list():
            ...
            tosca_path = str(self.__get_base_action_path(self.ACTION_UPGRADE)) + "/" + model_folder_name + "/tosca/"
            if os.path.isdir(tosca_path + "node-types/"):
