...
class ModelImportManager:
    ...
    def create_models(self):
        for model_folder_name in self.__get_model_init_list():
            ...
            self.__init_model_node_types(model_folder_name, model_payload_dict, True)
    ...
    def update_models(self):
        ...
        for model_folder_name in self.__get_model_upgrade_list():
            ...
            self.__upgrade_model_non_node_types(model_folder_name, model_payload_dict, existing_models)
            self.__upgrade_model_node_types(model_folder_name, model_payload_dict, existing_models, True)
            self.__upgrade_model_non_node_types(model_folder_name, model_payload_dict, existing_models, True)
    ...
    def __upgrade_model_non_node_types(self, model, model_payload_dict, models, with_metadata=False):
        path = self.__get_tosca_path(self.ACTION_UPGRADE, model)
        if os.path.isdir(path):
            self.__model_client.import_model_elements(model_payload_dict, str(path) , with_metadata)
        elif not any(m for m in models if model_payload_dict['name'] == m['name']):
            self.__init_model_non_node_types(model, model_payload_dict, upgrade)
    ...
    def __upgrade_model_node_types(self, model, model_payload_dict, models, upgrade=False):
        path = self.__get_tosca_path(self.ACTION_UPGRADE, model / self.NODE_FOLDER)
        if os.path.isdir(path):
            self.__model_client.import_model_types(model_payload_dict, self.__get_node_type_list(path), upgrade)
        elif not any(m for m in models if model_payload_dict['name'] == m['name']):
            self.__init_model_node_types(model, model_payload_dict, upgrade)
