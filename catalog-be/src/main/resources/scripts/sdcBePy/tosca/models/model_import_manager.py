...
class ModelImportManager:
    ...
    def __import_model_node_types(self, model_folder_name, model_name, action):
        ...
        payload_json_path = self.__get_node_types_payload_file_path(model_name, path)
    ...
    def __get_node_types_payload_file_path(self, model_name, node_types_folder):
        payload_json_path = node_types_folder / 'payload.json'
        if not os.path.isfile(payload_json_path):
            error_msg = "Missing payload.json file for model '{}'. Expected path '{}'".format(model_name,
                                                                                              payload_json_path)
            raise Exception(error_msg)
        return payload_json_path
