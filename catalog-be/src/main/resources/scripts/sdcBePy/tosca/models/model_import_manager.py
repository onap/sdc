...
class ModelImportManager:
    ...
    def update_models(self):
        ...
        for model_folder_name in self.__get_model_upgrade_list():
            ...
            self.__upgrade_model_node_types(model_folder_name, model_payload_dict, existing_models)
    ...
    def __upgrade_model_non_node_types(self, model, model_payload_dict, models, with_metadata=False):
        if models and any(m for m in models if model_payload_dict['name'] == m['name']):
            path = self.__get_tosca_path(self.ACTION_UPGRADE, model)
            if os.path.isdir(path):
                self.__model_client.import_model_elements(model_payload_dict, str(os.path.join(path, '')), with_metadata)
        else:
            self.__init_model_non_node_types(model, model_payload_dict, with_metadata)
    ...
    def __upgrade_model_node_types(self, model, model_payload_dict, models, upgrade=True):
        if models and any(m for m in models if model_payload_dict['name'] == m['name']):
